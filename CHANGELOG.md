...
**The DCIM import stops being silent, and stops dropping a relationship the model already knows how to hold.** Measured against a real NetBox, four stacked switches arrived as eight unrelated boxes, forty-one console ports and sixty-two of seventy-five power feeds vanished without a word, and a device being decommissioned came in looking exactly like one in production. None of that needed a new model: a virtual chassis is a stack InfraNet has represented since 2.4, and everything else was a matter of saying out loud what the import does not take.
