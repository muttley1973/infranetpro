...
### Added
- **An outlet can now say which group it belongs to, which is the only question a UPS exists to answer.** A power strip answers "what is fed"; a UPS has to answer "what stays on when the power goes, and what can I sacrifice to last longer" — and that is a property of the GROUP, not of the single outlet. The model had nowhere to write it. Measured across the vendors rather than invented: APC has a Main Outlet Group plus one to three Switched Outlet Groups, Eaton calls them load segments with a Primary group, Vertiv splits programmable from non-programmable receptacles, CyberPower and Ubiquiti separate Battery/Surge from Surge-only, Bastion and SNR write Segment1_3. Different words, two axes: whether the group can be switched on its own, and whether the battery holds it. Both now exist, on every device that has outlets — the group is not a UPS oddity, it is how outlets are organised, so a strip with banks gets it too. Each group carries a colour that the rack repeats as a thin band on the outlet, above the fill that shows state: the group is a declaration, the state a measurement, and two different things do not share one visual channel. The handover dossier prints the groups and tags every outlet with its own. One honesty note is written into the panel: this is DECLARED, not measured. RFC 1628 has no outlet groups at all — every brand keeps them in its own private MIB — so anyone who wants to read them off the wire will write a per-vendor driver, and until then nobody pretends otherwise. The assignment is stored separately from what a catalogue will one day read off an outlet name like "Group 2 - Output 1": your word is written where the catalogue cannot overwrite it.
- **A UPS from the catalogue now arrives with its outlets, and with the groups its maker already named.** The runtime catalogue — the slim projection the app actually reads — kept only network ports: of 53 UPS models, not one carried a single outlet, so applying a catalogue model to a UPS gave you nothing, while the full canonical file next to it described all of them outlet by outlet. They now come across, up to the same 48 the DCIM import allows, each with its name and its socket type. The group comes with them where the maker wrote it in the name: measured over the real catalogue, that is 56% of UPS outlets — «Group 1 Outlet 3», «Group 2 - Output 1», «Segment1_3», «Primary Group - Outlet1», «Power Outlet - Non Programmable 1», «Battery Backup/Surge 4», «Surge Only 2». What the name does not say is not guessed: «Outlet 1…8» arrives without a group, because position is not evidence. Two manual-first rules govern the transfer, because applying a model must never cost you work: what you wrote ON an outlet — the device it feeds, the state you declared, the group you gave it — survives, since the model brings the hardware and not your documentation of it; and groups you declared yourself are never renamed by the catalogue, which only fills in when you have none. Outlets you documented beyond what the model declares are kept too: a model listing fewer of them is not proof that yours do not exist. Rebuilding the slim catalogue no longer needs the network either: `--from-canonical` reprojects it from the canonical file already on disk, and leaves the manifest and the diff untouched, because the source did not change — only the projection did.
...
- **You can now ask what changed in the DCIM since you imported — and nothing gets written.** Every import creates a fresh project: that is the guarantee that no merge can ever clobber your work, and the price is that when NetBox moves on, the only way to bring it back in was another photocopy, throwing away the hand work — positions on the floor, cables you declared, names you corrected. For a manual-first tool that was the biggest hole in the integration. The first half of the answer, and the only one worth having before deciding anything, is the reading: a **Compare with the open project** button on the preview, and a panel of rows — this device's name differs, this network is new, this VLAN is gone — with the document's value and the DCIM's side by side. Three honesty rules shape it. A device you added by hand has no DCIM id and is therefore never reported as "gone from the DCIM": that would accuse you of your own work, so it is counted separately and declared, and it is exactly why there is no "apply all" button. Only declared fields are compared, never measured ones: putting a measurement next to a declaration produces differences that mean nothing, the same error that keeps `platform` away from the firmware field. And "different" is not "wrong" — with no timestamp per field, nobody can say whether you corrected the name or NetBox did, so the row shows both values and stays silent about blame, except where the document carries a manual flag, which is a fact and is said. Identity comes from the DCIM id already written on every imported device and network, from the VLAN's own number, and from the rack's name — never from an id this import generated, which means nothing across two readings. The panel also says which scope it compared: measured against a real NetBox, comparing everything against a single-site project reports 181 new devices, all true and all useless, and that sentence is what stops it reading as a document 181 devices behind.

### Changed
- **One site, one project — the import now opens on choosing the site instead of on the whole-DCIM dump, and the project remembers where it came from.** The product already wanted this in three independent places: the project name is proposed from the site and falls back to a neutral name only when the devices span several — the degenerate case; the manual states it outright, because rack names collide across sites; and the customer profile is a single-site mid-sized company. The wizard nevertheless opened on "import everything", which made the fallback the recommended path. Now step one is the site list, **Next stays disabled until you choose**, and importing the whole DCIM survives as a demoted, clearly-labelled detour that says what it costs. The piece that was actually missing, though, is smaller and matters more: an imported project now records the slice of the DCIM it was born from — measured from the sites of the devices that really came in, not copied from the request, so choosing three sites where only one has devices records the one. "Compare with the open project" reads that and re-reads exactly that slice, which is the difference between a useful answer and 181 true, useless novelties. A project imported before this release has no such record: the comparison falls back to the scope you pick by hand and says so, rather than pretending to know. The instance's own address is deliberately not written into the document — project JSON gets exported and passed around, and an internal DCIM URL should not travel with it.
- **The import preview says each thing once, at the weight it deserves.** The screen had grown four ways of stating the same numbers — a context line, four KPI cards, a strip of nine chips and the "I will import…" estimate — and two competing attention signals whose counts disagreed, because the yellow banner counted devices while "To decide" counts decisions. Now the estimate is the only statement of numbers: what does not come in joins it as chips (each one a button that jumps to the row explaining it — and before these labels existed, an import that lost something with no choices to make was summarised as "nothing lost"), the second-rank figures live behind a "View details" fold, and the freshness line moved next to the title it qualifies. The banner is gone; the only rows that block creation carry a "blocks creation" tag on themselves, and the Create button stays the other half of that one signal. Decisions whose default is already a good answer collapse to a single line stating the current choice — open them only to change it; losses and declared limits collapse to their title line too, keeping the "data lost" tag on the closed row (and, for losses, the chip up top), so the fact stays announced while the explanation waits to be asked. The loss rows sit after the choices since a chip already carries them in the estimate. Two variables composing HTML that was never rendered left with the redesign.
...
- **Renaming a radio now changes the labels that name it.** A radio's name lives on the device (`node.radios[i].label`): you type it in the Wi-Fi panel, and the DCIM import copies into it the interface name NetBox declares. Nothing that composes a label ever read it. The cable's automatic label, the port name in the topology and in the cable panel, the CSV, the PDF label sheet and the Verify's snapshot each worked the name out by slicing the internal port id, so a radio came out as "Pradio2" whatever you had called it — six independent derivations of one concept, none of which knew radios exist. Radio ports also carry no port record, so the usual ifName/alias fallback had nothing to fall back on. One function now answers the question (`lib/radio.js`) and every label goes through it: a named radio prints its name, without the "P" that is the convention for physical ports, and an unnamed one prints exactly what it printed before. The names an import had already brought in — radio0, radio1, wlan0 — were being written and never shown; they show now, with nothing to re-import.
