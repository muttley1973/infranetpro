...
- **Changed** — the icon set now draws from **two public-domain (CC0) sources**: **Simple Icons** (monochrome, tinted via `currentColor`) and **VectorLogoZone / gilbarbara/logos** for full-colour marks (the sprite carries each logo's own `viewBox` and colours). First colour logo added: **Azure**. New monochrome logos: Nutanix, Unraid, QEMU. The green/yellow vSphere Client mark is in no CC0 set, so VMware stays monochrome. Trademarks remain their owners'; the logos are used nominatively to identify the OS/platform.
