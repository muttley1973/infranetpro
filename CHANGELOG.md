...
**A network is a thing of its own, and every address is checked — not just the first IPv4.** The subnet stops being a field of the VLAN: a network is now first-class and its VLAN is optional, the shape every real IPAM uses. Project format goes to schema 2, migrated in place on open; nothing moves for a project with one subnet per VLAN. This release also declares and audits IPv6 like IPv4, folds in a deep correctness pass over the address and topology engines, and hands the AI assistant the whole documentation instead of a third of it — the networks you declared included.
...
- **`prefixes` in the REST inventory** (`/api/v1/projects/:id`): every network with its VLAN (`null` when none), name, gateway, DNS and provenance. `vlans[].subnet` keeps its meaning.

### Changed
- **"Networks" comes before "VLAN"** in the project panel — the addressing plan is the document; a VLAN is a label a network may carry.
- **The VLAN card shows its networks and takes you to them; it does not write them.** A network has at most one VLAN, so that field lives on the network. The card still sets the routing device, which is an attribute of the VLAN.
- **The two gateways are named**: "Gateway (IP)" is the address, on the network; "Routed by" is the device, on the VLAN. "No device holds this address" moved next to the address.
- **The networks list reads as a table** with a per-row occupancy bar (no bar for a /64); a VLAN shows as a colour-dot pill, not coloured text that read like an error.
- **The L3 map is one row per network, not one per VLAN** (a VLAN with no network stays, saying so), wider (780px) so device names are not cut; the CSV follows.
- **Addresses, CIDRs and MACs use the interface font** with tabular figures; code, logs, tokens and `sysDescr` stay monospace. Every editable field is sized to the data it writes.

### Fixed
- **Presence no longer depends on how a MAC is written.** The Verify indexed measured addresses with a bare `.toLowerCase()`, which works only while every source emits the colon form. The day one does not — a pasted Windows ARP table (`AA-BB-CC-…`), a lease driver, the Cisco dotted form — the documented device stopped matching the measured one and went grey forever, silently. Both sides of every comparison now use one canonical MAC key.
- **A MAC in Cisco dotted notation is recognised like any other.** The topology planner kept its own normaliser that only replaced dashes, so on `aabb.ccdd.eeff` the questions "is this a virtual NIC?" and "is this a randomised MAC?" both answered no — and the device behind that port was typed as a plain switch. The five MAC normalisers the project had accumulated are now one (`lib/mac-class.js`), and the two lists of virtual OUIs are one list.
- **A declared network's real mask decides, instead of an assumed /24.** "Which segment is this address in?" was rewritten in five places as "the first three octets", and in three of them that assumption decided a verdict. With two /25s inside one /24, probing one half declared the other half covered — so a device nobody had looked for was reported **absent** rather than unverifiable. With a /22, two addresses of the same network looked like opposite sides of a router, and the planner proposed a router that does not exist; on the discovery side, half the collector's own hosts looked off-segment and dead hosts came back as candidates. The question is now answered in one place (`segmentKey` in `lib/cidr.js`): the declared prefix containing the address wins, and only where nothing is declared does the /24 convention still apply — stated once, instead of assumed five times. The collector reads its own interfaces' real masks, which the OS had all along.
- **The assistant is told the whole documentation, not a third of it.** A census of the context that leaves for the model found 23 of 30 documented parameters missing — and since the model is instructed to answer only from that context, anything absent came back as "not in the documentation", about data you had entered by hand. Now included: **the declared networks** (with their CIDR, whether or not they carry a VLAN, IPv6 among them — before, a project whose addressing has no VLANs, the norm after a NetBox import, reached the model as an empty list), **the VLAN's declared role** (guest/voice/management), **IPv6 on the device and IP/IPv6 on the port** (a router's WAN side lives on the port), **the VMs documented on a host** (only their count went out), **a PDU's outlets with the device each one feeds** ("what goes dark if this PDU fails"), and **warranty / end-of-life dates**. Secrets are unaffected: every new block is an explicit allowlist, free prose (device notes) and the backup pointer stay out by design, and each category obeys the existing scope switches — verified against the real lab project, five secret values present, none in the context.
- **A replaced device now reaches the assistant.** The browser collected `identityChanged` — a measured serial or model different from the declared one — but the server's allowlist did not know the category and dropped it silently: the most serious finding of a Verify never arrived. Same for `unverified`, presence that could not be proved because the sweep never covered that subnet, which is half of the verdict.
- **The anti-invention check knows what it let out.** It read the context at fixed paths, so a model citing a real port address or the gateway of a VLAN-less network — data InfraNet itself had supplied — was flagged "reference not found": the check punished the right answers. It now indexes networks, port addresses and VM addresses too. An outlet also names its powered device exactly as the handoff dossier prints it, so document and assistant cannot disagree about the same outlet.
- **No IPv6 gateway was ever checked.** The L3 report walked VLANs and read each one's first IPv4, so a dual-stack VLAN's /64 and every VLAN-less network went unexamined. It now walks `ipam.prefixes[]` — one row per declared network, both families — and so does the overlap audit, which had been blind to more than half of a real plan.
- **IPv6 is compared by identity, not by text**, in one place (`lib/cidr.js`): two spellings of one address are one address. This fixes a v6 gateway never matching its router, the same v6 on two devices not flagged as a duplicate, a /64 counting one address twice, and — in the topology — a router left unlinked because the document's spelling differed from the measured one.
- **An IPv6 network no longer reads as empty**: the address collector dropped anything that was not IPv4 and never looked at `ip6`; both families count now, VM interfaces included.
- **"Out of subnet" and "wrong family" are two answers now** — an IPv4 on an IPv6 network is the wrong kind of address, not a digit off — and the network/broadcast check is gated on the family, leaving a /64's all-zeros subnet-router anycast alone (RFC 4291).
- **An address written on a port counts everywhere.** The per-interface address (2.8.2) was read by nothing: occupancy under-counted, "next free" could hand back a used address, and the L3 map called a gateway on a port "held by no device".
- **A cable inferred twice is removed even when the real one is corroborated** — the dedup trusted `LLDP`/`CDP` but not the fused `LLDP+MAC`, so the phantom survived in the confirmed case.
- **The topology crawl no longer overwrites a pinned name or adopts a next-hop's MAC** — the two guards the Discovery path already applies, now on the crawl too.
- **A trunk set by hand stays a trunk** even when its VLANs collapse to one, instead of being demoted to access and labelled "derived".
- **Smaller correctness fixes**: a link-local is no longer a duplicate (`fe80::1` is unique per link, RFC 4007); an empty VLAN key no longer migrates to "VLAN 0"; an IPv4 with leading zeros keys to its canonical form; SNMP no longer stamps "VLAN 1" on a port it never measured; and a "change IP" row now carries which device and which new address, so ignoring one change no longer hides a later one.
- **The DCIM import stops throwing prefixes away** (the 51-of-90 VLAN-less prefixes on a real NetBox, and a VLAN's second prefix) and says how many arrived without a VLAN. Overlap detection works on IPv6 and never reports two families as a conflict. Adopt from leases reaches a network with no VLAN, and the collected decision examples finally show.
- **The import declares the addresses that stay out** instead of showing "IP addresses 0" next to "Prefixes 90" — a count with a few examples, printed only when it can be proved.
- **Typography leaks fixed**: `--font-mono` is now defined (Windows was rendering Courier New), the VLAN name field inherits the UI font, three Italian strings no longer print stray backticks, and a DCIM-imported network no longer shifts its row by a column.

### Security
- **The AI chat is admin-only.** It spends the admin's LLM key and sends the project's topology to an external provider — an egress like every scan and poll, and a viewer could drive it.
- **A colour from an imported project can no longer run as code.** Any `vlanColors` value that is not a plain `#rrggbb` is snapped back to neutral grey on load, before it reaches the `style` attribute it is written into.
