...
### Fixed
- **A cable being drawn now starts at the port, not off the edge of the screen.** Starting a link selects the port, which redraws the Properties panel — and that panel puts the *same* `data-pid` on a dozen inputs, selects and buttons. A bare `querySelector` returns the first one in the document, so the preview anchored to a panel control instead of the port LED; with the Rack tab in front those controls are hidden, giving a 0×0 rect at the origin, and the line came in from the top-left corner as if it started on the floor plan. `data-pid` and `data-id` are dual-use, so asking for "the element that represents this port" is now a single call (`portAnchorEl`), scoped to the visual anchors the way the cable renderer already was — and the five other places that asked the bare way (port drag, rack drag, resize, VM drop target, port repaint, cabling-route highlight) ask it too. When the port is not drawable here and now — a different rack, a closed panel — no preview is drawn at all, rather than one starting from an invented point.
- **Hiding the floor-plan grid now frees every placement, not just dragging.** The manual promises that hiding the grid makes snapping free to the pixel, and dragging honoured it — but the three paths that *put* something on the map without dragging it rounded to a 20px step of their own, never reading the setting: "Place on floor plan" for a cabinet, the shared-segment node, and the racks the sub-bar places to draw a topology. The grid disappeared for the eyes and stayed for the hands. The rule now lives once in `lib/floor-snap.js` and every caller asks it, so the switch means the same thing everywhere. Non-finite coordinates (a canvas not yet measured) return 0 instead of writing `NaN` into a device position.
