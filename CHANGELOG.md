...
- **At-a-glance verdicts, severity accent, since-last-read delta, honest labels.** Each column now answers its question with a health dot (green/amber/red) and a sober phrase — the level (ok/warn/bad + issue count) computed in the pure lib, the words in the glue. Red is reserved for a never-synced project (flying blind), so a project read recently but with discrepancies stays amber rather than alarmist — the "Verifica completa" Fase-2 placeholder no longer paints every project red. The section's entry-point tile takes a severity-coloured accent bar, and a −N / +N chip shows problems closed or opened since the previous SNMP read (baseline in `localStorage` per project, anchored to the last sync, never in `state` — viewing must not dirty the document). Labels made truer: the Conformance mismatch verdict is "to be defined", not "don't match"; the provenance pill "measured" reads "from scan"; "Proper name" → "Device name"; a missing *name* is "to confirm" (the device exists, only a proper name is missing) while a missing IP/MAC stays "missing". The breadcrumb follows the active view (Floor plan / Topology / Overview) instead of always reading "Floor plan". `lib/overview.js`, `src/app-overview.js`, `src/app-subbar.js`, `lib/i18n.js`, `styles/11-overview.css`, +tests.
