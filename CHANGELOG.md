...
- **SFP uplinks align with the switch data ports on high-density chassis**: in the ultra-dense (`dense-xxl`) front-panel layout the two SFP rows were packed tighter than the two copper rows and sat visually higher; the SFP block now uses the copper row pitch so its LEDs land on the same horizontal lines as the switch ports.
