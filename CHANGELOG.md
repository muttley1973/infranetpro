...
- **Added** — a **hypervisor / homelab's Platform** (`hvPlatform`) now drives its OS logo in the panel header too: VMware ESXi → VMware, plus Proxmox, Docker, TrueNAS (Debian-based → Linux), Nutanix, Unraid and KVM (the QEMU/KVM logo); **Hyper-V and Azure Local** (Stack HCI, both Windows-based) → the **Windows** logo. Only XCP-ng has no public-domain logo → shown as none. New Simple Icons pulled in: Nutanix, Unraid, QEMU. (Proprietary colour logos like the vSphere Client mark or the Azure "A" aren't CC0, so they're not bundled — we stay on Simple Icons + in-house glyphs.)
