...
- **And the legend says what those cables are, not what they lack.** One entry, «no VLAN colour», stood for every neutral cable — a label defined by absence, in a model where a cable that switches now always has a colour. Absence is no longer a category: two states remain and each is a precise statement, so each gets its own entry under the same neutral swatch — **trunk** (carries several VLANs, none prevails) and **routed** (in no VLAN at all). Either appears only when at least one cable actually falls there, because an entry explaining a colour that is nowhere on the map is noise; on the lab that means the legend shows «trunk» alone.
- **And the neutral stops looking like VLAN 1.** With the invented ones gone, cables without a VLAN colour are numerous and real — trunks, routed links, undeclared ports — and until now they fell through to the site's native VLAN, so one grey covered several different states. They now take a neutral of their own, distinct in hue rather than merely in shade, with a legend entry that appears only when at least one cable actually falls there and says which three cases it covers.
