...
- **The project list is paid once per save instead of once per read.** Every call re-read and re-parsed every project file on disk, synchronously, in the server's single process — so for that whole time no other request was served, for anybody. Measured over forty runs before touching it: 9.9 ms on a real store of nineteen projects, and 68 ms median with 84 at the 95th percentile on twelve projects of a thousand devices each. Those are not two faults but the same code at two scales, and the second is the one that matters, because the per-site device counts on the multi-site map come out of this same call — the cost grows with the number of sites, which is precisely what makes an installation large. A project's row in that list depends only on that project's file, so it is now kept and rebuilt when the file changes: at rest the call costs about 1 ms at either scale, and after a save it re-reads only the project that was saved. Whether a file changed is decided by the same two facts — last-written time and size — that this app already trusts to notice somebody overwriting a project under a save in progress, now read from one place so the two cannot drift apart. The server also knows when it is the one writing, so a save drops its own row directly, which covers the single case no signature can see: two writes landing in the same millisecond at the same size.

- **The Save button was at its loudest when there was nothing to save.** With the document clean it was the accent-filled primary — the strongest button on the toolbar, 10.7:1 against the page — and the moment you had unsaved work it became an amber outline measuring 1.19:1 against that same background, which at that point is not a shape but a hairline. Nine times the emphasis, on the wrong side of the question. Unsaved work now fills the button in amber, with the ink derived from that amber rather than assumed (10.8:1), and a saved document leaves it weighing exactly what its neighbours weigh — the same argument this toolbar already made when it stopped painting Delete red at rest, because a warning that is always lit stops warning. It also removes the bar’s second primary: the redesign had decided the primary action is Verify, and two primaries are not a primary. The button no longer changes width between the two states either, so the header stops flinching the moment you touch anything — and the small amber dot beside the label is gone with it: it was the same colour as the fill that replaced it, so it had become an invisible disc whose only remaining effect was to make the unsaved button 14 pixels wider than the saved one. The two states are now the same shape and differ only in colour — and that colour is the amber the topology legend already uses for ENDPOINT, so the toolbar does not introduce a fourth amber into an app that had three.

- **A media converter on the floor plan was labelled «Media».** The short label that fits a floor node said neither what the device is nor what it converts, and in Italian it was the same word the app uses for *medium* recognition confidence and for *medium* VM criticality — one word for a device type and for two opposite axes of intensity, which is how a reader concludes the two scales are the same one. It now reads «Converter» in both languages, which is the tail of its own catalogue name («Media Converter»), the same shape that already turns «Dispositivo IoT» into «IoT».

- **The badge warning you not to trust a cable was the hardest one in the panel to read.** The state badges on a cable painted their text white on whatever colour the state called for, and four of the twelve colours could not carry white: «Inferred · to verify» sat at 2.03:1 where readable text needs 4.5, followed by «Weak» at 3.14, LAG at 3.35 and CDP at 3.36. The ink is now derived from the background's own luminance instead of being fixed, which takes those four to 9.34, 6.02, 5.64 and 5.64 — with every colour of the app left exactly as it was. It is a rule and not four patches: a badge colour added tomorrow gets a readable ink for free, and a test reads the colour tables out of the source so a new one cannot slip in below the threshold.
...
- **A port's VLAN that could not be read was written down as VLAN 1.** The standard reading — the port's PVID, from the bridge MIB every managed switch implements — was collected as «whatever the decoder returned, or 1». That decoder returns zero when it cannot make sense of what the agent sent, so a failed read landed in the document as a port sitting in VLAN 1, in a form nothing downstream could tell apart from a switch that had genuinely answered 1. Zero is not a value a PVID can take: the standard defines it as *no VLAN*, so a zero at that point is a decoding error and nothing else, and turning it into a 1 turns a failure into a measurement. It now stays absent, which the rest of the app already knows how to handle — a port whose VLAN nobody managed to read goes on taking it from what has been declared. The difference is not cosmetic: on a device that switches VLANs a measured PVID carries authority, so that invented 1 could outrank a VLAN somebody had documented by hand. Values outside the valid range are refused the same way, which also covers the reserved 4095, and the two vendor-specific readings underneath still have no fallback of their own.

- **A save in flight can no longer be overwritten by another one.** Saving writes to a temporary file, flushes it to disk and then renames it over the real one, so a crash mid-write leaves the previous version intact. That temporary was named after the process, not after the write, which meant every save of the same project inside one server shared a single filename. Nothing could go wrong while those writes were strictly sequential, but the moment any of that I/O became asynchronous two saves would have overwritten each other’s temporary before the rename — and since the rename itself is atomic, the result would have been a perfectly valid file containing half of one save and half of the other, with no error anywhere. The name now identifies the write. A unique name also creates an obligation the fixed one did not have — a failed write used to leave a temporary the next write would reuse — so a write that fails now removes its own temporary instead of leaving it behind.

- **An import can now say which networks a link makes reachable — where NetBox actually says so.** Two of the coherence checks on the multi-site model need that answer, and no import had ever supplied it: on every imported project they declared themselves blind with the reason «no reach», which is honest but meant a whole check had never once run. The gap was not an oversight in the import, and the reason is now measured rather than assumed: on a real NetBox 4.6.7 a tunnel carries its encapsulation, its IPsec profile and its id, and nothing at all about the networks it protects; a circuit carries its code, its committed rate and its two terminations, and nothing about what it transports. Deducing the encryption domain of an IPsec from the networks of the site would be inventing the single most delicate field in the model. The one chain that does exist is read now: an L2 service terminating on a VLAN, and the prefixes NetBox declares on that VLAN. Networks are unioned across every termination at the same site rather than taken from the first, so a service landing on two VLANs no longer leaves the second one accused of not being carried; a prefix the archive has marked deprecated is left out, because repeating a withdrawn statement is worse than saying nothing; and where nothing is declared the field stays empty rather than becoming an empty list, since «we do not know» and «it reaches nothing» are different answers.
