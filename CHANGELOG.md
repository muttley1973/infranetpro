...
- **Imported objects keep the identity they have in the DCIM.** Ports, patch-panel slots and racks now carry the id of the object they came from, in a field of their own. A port's only anchor back to its interface used to be `ifName` — a string, the one kind of comparison this project forbids everywhere else, and enough on its own to make writing a cable back impossible, since a cable is addressed by its two interfaces and not by its two devices. A patch-panel slot carries both ends, front and rear, because a cable can terminate on either. The reference lives in a field and never inside the InfraNet id: ids get renumbered, and an identity that lives in a string dies with the first rename.
- **The identity is cheap on purpose, and the price was measured.** The object type lives in the name of the field rather than beside the value: 13 bytes per port instead of 57, which on 40 devices of 48 ports is a document that grows by a third rather than more than doubling. Only one module knows those field names — everything else reads the same shape it always did, so the encoding can change again without chasing callers.
