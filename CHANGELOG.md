...
> **Deferred (with rationale, tracked in the audit memo):** the PoE positional fallback (load-bearing where the PoE index ≠ ifIndex), per-run cache invalidation (needs a generational design so a muted device's topology isn't dropped), a few L2/L3 label/multi-Po nuances, the authenticated-SMB-in-stealth scan-policy decision, and the export presence-colour model (`export.js` still colours a floor node by port fault, not by the on-screen presence buckets — a substantial, compliance-adjacent pass of its own).
