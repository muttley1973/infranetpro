...
- **A trend on the verification timeline.** The *Checks* tab opened on a list of snapshots; it now opens on their reading — where you are, as the last check split into consistent, mismatched and not-verifiable devices, and where you are going, as the shape of the series. It **refuses a verdict** when the share it could not look at has grown, never compares a manual check against an automatic poll, breaks the series if the estate changes size, and below three samples says so: the confirmed share matures with use.
- **«Silent to this key».** With SNMPv2c a wrong community raises no error — the agent drops the request and the probe times out — so silence and *no SNMP* are indistinguishable from outside, and a whole estate gets filed as unmanaged after knocking with the wrong key. Discover says it now and counts them, but only where something authoritative expected an answer: a neighbour that declared the device over LLDP/CDP, or the project documenting it with an SNMP driver. Such a neighbour, when it stayed quiet, used not to appear at all.

### Changed

- **The assistant's context tells the truth about itself.** Cutting a list is honest; cutting a relation is not — one missing link makes two connected halves look separate, and from the inside you cannot tell. Topology, ports, PDU outlets, VMs and SSIDs now carry how much was shown and how much there was, and the prompt forbids answering past that line. Passive cabling comes out summarised by type while its names stay in the topology, where a path reads hop by hop: 56% off the context, at the cost of one question in twenty.
...
live, then the surfaces the first pass had not touched. Everything short enough to fix
safely was fixed, each finding reproduced first and each covered by a test. Nothing here
changes what the app documents; what changes is what it refuses to do.

### Added

- **Anyone can change their own password** — `POST /api/auth/password`, viewers included, who had no way to. It asks for the current one, keeps the session that made the change and expires the account's others.
- **Every secret can live on the data volume** — `INFRANET_API_TOKENS_FILE`, `INFRANET_AI_CONFIG_FILE`, `INFRANET_DCIM_CONFIG_FILE`, `INFRANET_SESSION_SECRET_FILE` — and `INFRANET_AI_ENDPOINT` pins the AI endpoint, so a key set by the operator cannot be redirected from the interface.

### Changed

- **The Docker image runs as a non-root user** and keeps the secrets out of its layers, on the `/data` volume.
- **Signing in is stricter**: the session id is regenerated on login, a new password needs eight characters, and throttling counts per address *and* account — behind one proxy, ten failures used to lock out everyone.
- **The topology crawl stays in internal address space** unless you declare otherwise, with depth and device count capped; a VLAN id outside 1–4094 is refused instead of quietly clamped; and every `/api/*` answer carries `Cache-Control: no-store`.

### Fixed

- **A management URL could carry credentials out of the app.** With a `javascript:` scheme it ran in the app's own origin when opened; written without a scheme (`admin:pw@10.0.0.1`) it travelled with its password into the REST DTOs, the Ansible inventory and the context sent to the AI provider.
- **Revoking a session survives the next login** — a demoted administrator who left a tab open used to get their rights back simply by signing in elsewhere.
- **The export stopped being a way in.** It could read files off the server through an image reference, hang forever on a malformed PNG, and freeze the single thread the server has for nineteen seconds on one long cell.
- **Work no longer disappears quietly**: a malformed save rewrote a project as empty and answered *200 OK*; undo and redo did not light «Save»; deleting a rack left its HA partner pointing at something gone; closing the tab said nothing; and an import from discovery went unrecorded in the document's journal.
- **Values are checked before they are used** — a subnet prefix outside its range, a `projectId` that is not a positive integer, a non-string username (a 500, now a 400), a snapshot id before it becomes a path, and the saved DCIM token, which now travels only to the instance it was saved for.
- **Three strings that had never reached either language** now follow the chosen one, and `<html lang>` follows it too.
...
- **The project list is paid once per save instead of once per read.** Every call re-read and re-parsed every project file on disk, synchronously, so nobody else was served meanwhile. It now costs about 1 ms instead of 68 on a large installation, and a save makes the list re-read only the project that was saved.
- **A full canvas render reuses the DOM instead of recreating it.** 55 ms at 500 devices and 92 at 1000, where it was 119 and 203; the floor-only render no longer grows with the project at all, and dragging a device no longer produces a single long frame.
- **Selecting a device no longer rebuilds the whole canvas.** A click cost 122 ms at 500 devices and 205 at 1000; it now costs the same at either size and repaints only what depends on the selection.
- **Two hot lookups are no longer paid on every render.** Resolving a port to its device is remembered, and the rack-unit height no longer forces a whole-document style recalculation on each redraw.
- **Save is loud when there *is* something to save.** With the document clean it used to be the strongest button on the toolbar, and unsaved work showed as a faint amber outline. The emphasis is the other way round now, and the button no longer changes width between the two states.
- **Notifications stay until you dismiss them.** They used to share one box and fade on a timer, so a second message overwrote a first nobody had read yet. Each is its own card now, stacked in a column you empty yourself.
- **The status badges no longer shove the toolbar.** The four of them sit in one bay after the search field: with all four lit, the action buttons move by zero pixels instead of 267.
- **The toolbar reads as groups instead of one long row.** Thirteen dividers became the five that mark what the bar actually does, Delete is no longer painted red at rest, and History gets its word back.
- **Cable badges are readable.** Four of the twelve wrote white text on a background that could not carry it — the worst, at 2.03:1, was the one warning you *not* to trust that cable. The ink is now derived from each background instead of fixed.
- **A media converter on the floor plan is labelled «Converter».** In Italian its old label was also the word for *medium* recognition confidence and *medium* VM criticality: one word for a device type and for two unrelated scales.

### Fixed

- **A port's VLAN that could not be read was written down as VLAN 1.** Zero is not a value a PVID can take — the standard uses it to mean *no VLAN* — so a zero there was a decoding failure, and writing 1 over it made a failure look like a measurement. On a device that switches VLANs that invented 1 could outrank a network documented by hand. It stays absent now.
- **A save in flight can no longer be overwritten by another one.** The temporary file used during a save was named after the process rather than after the write, so every save of the same project inside one server shared a single filename.
- **An import can now say which networks a link makes reachable — where NetBox actually says so.** Two coherence checks needed that answer and had never once run. A tunnel and a circuit carry nothing about the networks they serve, so the field stays empty there deliberately rather than being guessed; the one chain that does exist — an L2 service on a VLAN, and the prefixes declared on it — is read.
- **Extreme switches were showing the generic icon.** A word boundary does not fall inside `ExtremeXOS`, so the pattern that recognises network operating systems never matched it.
