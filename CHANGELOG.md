...
- **Added** — the Overview ends with a footnote, **"What I'm not looking at"**, naming the dimensions this summary does not judge — WAN/circuits, L3 routing, spanning-tree, firewall/ACL, AAA/syslog/NTP, warranty/EOL, restore proof — plus two measured elsewhere but not judged here (UPS power, live health). It shows under every lens, and the list is data in the engine: retiring an entry once a dimension gets modelled is a one-line change.
