...
- **The «no device holds this address» warning is one line now.** It said the verdict and both remedies in one ninety-four-character sentence, inside a panel column that is 410 pixels wide by default: it wrapped onto three lines and read as a paragraph rather than an alert. The line keeps the verdict — «no documented device has this IP» — and the two things that can have happened, the device missing from the project or a mistyped address, moved into its tooltip, where somebody looks once they have decided to act. **«documented» stays in the sentence on purpose**: the project has a real presence measurement, and «no device has this IP» would read as one. The hint boxes around it also moved up from `--fs-sm` to `--fs-md`, the panel body size, because what they hold are whole sentences — a verdict is read at the size of what it judges, and their secondary rank is already carried by the muted colour. ⚠️ Measured in the browser across panel widths: one line from 400 pixels up; dragged to its 320-pixel minimum it still wraps onto two, and no wording that keeps the word «documented» fits there.
- **And the same treatment for the line under «Interface address».** It carried two sentences — when to fill the field in, and where the management address goes instead — and wrapped onto two or three lines under a one-line input. The line now holds the condition alone, «only if the port has an address of its own», and the examples plus the pointer to the device card moved into its tooltip. It also said «presa» in Italian, which in this app is a PDU outlet or a wall socket; the thing that has an address is a **port**. Its size was written inline in three places as `0.73rem` — off the type scale, below the smallest token there is — so the three copies could drift apart the way this project has watched happen twelve times; they are one `.prop-hint` rule now, at `--fs-sm`: one step under the field's label instead of two under legibility. ⚠️ Measured across panel widths: one line in both languages all the way down to the 320-pixel minimum.
- **Every info box in the properties panel now states its rule in one line, at one size.** Fifteen of them held two or three sentences each — what a group of outlets means and that no SNMP standard exposes it; that a radio port is its own kind of link and does not connect to network interfaces and tops out at eight; that a manual outlet state stays separate from the imported one and can be restored — and each wrapped onto two or three lines inside a 410-pixel column, which turns an aside into a paragraph the eye has to wade through. Each line now carries the rule alone and its second half lives in the box's tooltip, reachable in the one moment somebody wants it. Nothing was deleted: every sentence that left the line is in a tooltip, and the golden render pins both halves for the outlet case, because a box that dropped its second half would be lying by omission.
- **And they are all the same size, which they had never been.** The boxes carried five different hand-written sizes — `.72rem`, `.73rem`, `.74rem`, `.78rem`, `--fs-sm` — four of them **off the type scale entirely**, below the smallest token there is, two of them two lines apart in the same stylesheet. Sizes written by hand drift; that is the same defect as a definition living in two layers, and it had produced boxes that sat directly above one another at visibly different sizes. They are `--fs-md` now — the panel body — because what these boxes hold are whole sentences, and the secondary rank they need is already carried by the muted colour. ⚠️ Measured in the browser: all thirteen box classes resolve to the same 14.4px, and every line stays on one line at the default 410-pixel column and above. Dragged to the 320-pixel minimum, nine of them wrap onto two — that is arithmetic, not layout: at that width a box holds about 38 characters.
- **One of those boxes was written in Italian inside the renderer.** The «power outlet state» group — its label and its whole explanatory line — never went through the dictionary, so an English user read Italian. It has keys in both languages now, like everything else on that panel. Two more of the same kind were fixed in passing: the interface-address hint said «presa», which in this app is a PDU outlet or a wall socket, and the thing that holds an address is a **port**.
- **A character budget guards the microcopy.** The failure mode here is not a typo, it is growth: a sentence gains a clause, nobody notices until it wraps on somebody's screen. Sixteen strings now carry a measured cap in both languages, and — except the two that are already complete in one sentence — the test also demands the matching `…Tip` key, since shortening a line is only honest if the rest is still reachable.
