...
### Added
- **A "Networks" panel** — every declared network in one list, sorted by address, VLAN as a badge, overlaps spelled out. Add several at once (comma-separated); edit a network's address in place, delete one with the **×** on its row, or clear the whole plan — both "clear all" actions ask first.
- **A VLAN can carry more than one network** (dual-stack, or a second range on the same SVI), and **a network can have no VLAN at all** — point-to-point links, transit, out-of-band: on a real NetBox, most of the plan.
- **IPv6 can be declared**, not just measured. A bare IPv6 normalises to its /64; a /64 shows the addresses actually seen instead of a meaningless percentage.
- **`prefixes` in the REST inventory** (`/api/v1/projects/:id`): every network with its VLAN (`null` when none), name, gateway, DNS and provenance.

### Changed
- **"Networks" comes before "VLAN"** in the project panel: the addressing plan is the document, a VLAN is a label a network may carry. The VLAN card shows its networks and takes you to them; it no longer writes them.
- **The two gateways are named**: "Gateway (IP)" is the address, on the network; "Routed by" is the device, on the VLAN.
- **The L3 map is one row per network, not per VLAN**, and wider so device names are not cut; the CSV follows.
- **Addresses, CIDRs and MACs use the interface font** with tabular figures; code, logs and `sysDescr` stay monospace.

### Fixed
- **Presence no longer depends on how a MAC is written.** The Verify compared with a bare `.toLowerCase()`, which holds only while every source spells a MAC the same way; a dash-separated or Cisco dotted address stopped matching and the device went grey forever, silently. The five MAC normalisers are now one, so a dotted MAC is also recognised as virtual or randomised like any other.
- **A declared network's real mask decides, instead of an assumed /24.** That assumption was written in five places and decided three verdicts: with two /25s in one /24, probing one half declared the other covered — a device nobody looked for was reported **absent** instead of unverifiable; inside a /22 the planner proposed a router that does not exist; and half the collector's own hosts looked off-segment, so dead hosts came back as candidates. One rule now answers it, and the collector reads its interfaces' real masks.
- **No IPv6 gateway was ever checked**, and an IPv6 network read as empty. The L3 report and the overlap audit now walk every declared network, both families; IPv6 is compared by identity rather than by spelling, so a v6 gateway matches its router, a duplicate is seen, and a /64 stops counting one address twice. "Out of subnet" and "wrong family" are two different answers.
- **An address written on a port counts everywhere** — occupancy, "next free" and the L3 map had all ignored it, so a gateway living on an interface was "held by no device".
- **The assistant is told the whole documentation, not a third of it.** 23 of 30 documented parameters never reached the model, which is instructed to answer only from what it is given — so it said "not in the documentation" about data you had entered by hand. It now sees the declared networks (VLAN-less and IPv6 included), the VLAN's guest/voice/management role, IPv6 and port addresses, the VMs on a host, a PDU's outlets with what they power, and warranty / end-of-life dates. A replaced device and "presence not verifiable" reach it too. Secrets are unaffected: every block is an explicit allowlist, device notes and the backup pointer stay out by design, the scope switches still decide what leaves.
- **Topology and cabling**: a cable inferred twice is removed even when the real one is corroborated; the crawl no longer overwrites a pinned name or adopts a next-hop's MAC; a trunk set by hand stays a trunk when its VLANs collapse to one.
- **The DCIM import stops throwing prefixes away** — 51 of 90 on a real NetBox were VLAN-less — and now declares what stays out, with counts and examples, instead of an unexplained "IP addresses 0".
- **Smaller correctness fixes**: a link-local is not a duplicate (RFC 4007); an empty VLAN key no longer becomes "VLAN 0"; an IPv4 with leading zeros keys to its canonical form; SNMP no longer stamps "VLAN 1" on a port it never measured; a "change IP" row carries which device and which address; `--font-mono` is defined (Windows was falling back to Courier New).
...
- **A colour from an imported project can no longer run as code.** Any `vlanColors` value that is not a plain `#rrggbb` is snapped back to neutral grey on load, before it reaches the `style` attribute.
