...
- **Added** — a **shared corpus and two goldens for the classifier**: 53 rows (the real sysDescr strings this project has measured, anonymised, plus the cases that exercise every rule below, plus 16 invariants) run through both engines — the server's FusionScorer and the browser's `_guessType` — on every test run. There was no regression net at all before: a weight could move and a printer become a switch with nothing failing.
- **Fixed** — a **word no longer beats a measurement**: `ios` in a description was worth 55 points towards *router*, while the device declaring over SNMP that it routes (sysServices, RFC 1213) was worth 45. A Catalyst announcing "IOS XE" whose measured layers were L2-only came out a router. The word now sits below the measurement — with a concordant measurement the sum still classifies, without one the text still classifies, but at an honest confidence.
- **Changed** — the **HP factory-hostname rule** (`HP` + MAC suffix) drops from 65 to 50: it is an inference from an identity string, not a measurement of behaviour, and at 65 it outranked sysServices — the same inversion.
- **Fixed** — **`apc` is out of the UPS rule**. APC makes UPSs, rack PDUs, transfer switches and racks; the brand in the rule meant an APC PDU that declared itself a PDU still came out a UPS. Its UPS lines say "UPS" on their own (Smart-UPS, Back-UPS); Symmetra, which does not, is now listed as a line.
- **Fixed** — **`raritan` is out of the PDU rule**: Raritan also makes KVM-over-IP, and a Dominion KX was being typed a power strip. Its actual PDU lines (PX, Dominion PX) replace the brand.
- **Changed** — the two **Juniper-named rules become model rules**. They were the only branches in the shared table headed by a vendor — not because Juniper is special, but because someone added them for Juniper and not for anyone else. "SRX345" says firewall with or without "Juniper Networks" in front, which is how every other row in that file already works. The old `juniper.*mx` branch was also loose: it matched any "mx" anywhere in the text. **`vsrx` is named explicitly**: there is no word boundary before `srx` inside it, so a model-only rule could not see it — and the stock Junos sysDescr for a virtual firewall reads "vsrx internet router", where the word *router* would have won.
- **Added** — **network emulation platforms are a class, not an inventory**: the list read `pnetlab|eve-ng|unetlab`, which is exactly the bench it was written on. GNS3, containerlab and Cisco Modeling Labs are the same thing and were coming out **switch**.
- **Fixed** — the browser stops being a **second classifier**. `_guessType` had its own scale of regexes, divergent from the table whose own header calls itself the single source: bare `cisco` → switch, `sony` → tv, `apc` → ups, `huawei` → pc. On the shared corpus the two engines disagreed on **17 rows out of 53**; they now disagree on 6, and each of those six is structural (the browser is never given the sysServices measurement) rather than a second table. Concretely: a Windows PC polled over SNMP was typed **switch**, an LG webOS TV **switch**, a washing machine **switch** — the browser's cascade only ever looked for TVs and appliances in the vendor/banner/hostname, never in the sysDescr.
- **Added** — the browser now reads the **sysObjectID** for the types it had no regex for: a Fortinet identified by its own OID was falling through to the final fallback and coming out a switch.
- **Fixed** — **"switch" stops being the answer for everything the app doesn't recognise**. An SNMP responder that declared no OSI layer and matched no rule was *asserted* to be a switch — in the engine and in the browser both, the engine's own comment calling that branch "historical". It was the product's default answer for the unknown, stated with no evidence, on the field that renames the thing on your screen. It now comes out as the **generic device type**: still managed, still rack-placeable, but without a function nobody measured. The confidence was already 25, meaning *I don't know*; now the type says so too. Responders that do publish their layers still resolve to switch, router or server as before.
- **Fixed** — the **operating system stopped being counted twice**. Two votes — the OS fingerprint and the Unix/Linux rule — drew on the same single piece of evidence and stacked: `Linux Synology …` scored 115 for *server* against 90 for the declared NAS function. A NAS runs Linux by construction, as do firewalls and access points: the OS says what a device is made of, not what it does. One evidence, one vote.
- **Fixed** — three more **brands that decided a type**: bare `sony` was worth 58 towards *tv* (Sony also makes cameras, projectors and sensors — the `!camera|!projector` guard bolted onto that rule was the proof the token was too wide; only the `bravia` product line remains), and `cyberpower`/`liebert`/`vertiv` sat in the UPS rule, where a Liebert precision-cooling unit came out a UPS. Their UPS **lines** replace them; anything neither line nor the word "UPS" covers is still caught by the sysObjectID vote and the UPS-MIB, which are measurements.
- **Changed** — the phone-vendor list in the OS fingerprint (`google|samsung|xiaomi|huawei|…`) drops from 62 to **40**. Announced Android is a measurement and keeps its weight; the manufacturer's name alone is the weakest kind of inference, and these firms make switches, TVs, printers and thermostats too — at 62 it outranked the sysServices measurement, the same inversion corrected elsewhere.
- **Fixed** — a **Synology identified by its product line** rather than only by its vendor name: `DiskStation`/`RackStation` is what the unit writes in its own sysDescr, while "Synology" only appears there if someone named the host that way. Recognition used to depend on which of the two strings happened to be present.
- **Fixed** — in that pairing, an SNMP switch or router shows **the MAC its interfaces report**. On managed gear the MAC lives on `ifPhysAddress`, not on the chassis field, so reading only the chassis printed a dash next to devices the tile's own counter was calling documented — the row contradicting itself, which is the one thing this view exists not to do. The tooltip says where the address came from.
- **Changed** — in the Overview, **IP and MAC are one tile, not two**. They are the two halves of one network identity, and separated they forced you to open two details and cross-reference them in your head to answer "who is 192.168.1.13". The detail is now the project's **ARP pairing** — device, address, and the MAC underneath — with the gaps kept on top: no IP first, then no MAC. The big number still counts IP addresses, so the tile promises exactly what it promised before.
- **Added** — **"Spazio rack" and "Porte in fibra libere" open a list**, like every other tile. Free U per rack (with the ones that don't declare their height marked as derived — their total rests on the 42U fallback) and free-vs-declared fibre per device.
- **Added** — **"Porte libere" gains a "By speed" tab**: 166 free ports does not tell you how much room you have until you know whether they are 1G or 10G. Ports whose speed nobody knows are counted **separately** — a port with no declared speed is not a slow port.
- **Fixed** — the **two counters on the "Verifiable over SNMP" tile share a line**. `13 of 13` and `20 not verifiable` needed ~176px in a ~158px cell, so the second pair dropped below and the `20` read like part of the ratio. That tile now spans two grid cells and the pair sits next to it — no shrinking the text to fit.
- **Fixed** — the **header stopped shifting to two rows on its own**. The freshness chip changes width as its own text updates — «now» → «2 min» → «13 gg», and after a Sync «0/13» → «13/13» — swinging ~45px, which in a wrapping header was enough to push the button cluster onto a second line while you watched. Its width is now reserved for the widest case, so the measurement updates and the layout stays put.
...
- **Fixed** — the page no longer scrolls **sideways** on narrow screens (82px at 1024) with nothing visible sticking out. The cause was the **tooltips**: absolutely-positioned pseudo-elements at `opacity: 0` — invisible, but still part of the layout — and the ones on the rightmost controls reached past the window edge, widening the scrollable area. The header now clips horizontally, deliberately keeping vertical overflow visible so its five dropdowns still open downwards.
- **Fixed** — the tooltips of the right-hand toolbar cluster **stay inside the window**. Anchored to the control's right edge instead of centred on it, they grow leftwards: the SNMP freshness chip was losing 71px of its text off-screen at 1024, and off-screen text is not a tooltip.
...
- **Fixed** — a **transfer switch (ATS) that doesn't speak the APC PowerNet profile is no longer silently mute.** Unlike UPSs, transfer switches have no equivalent of RFC 1628, so that vendor profile is the only documented path — and a device from another make simply didn't answer those OIDs, producing a card of dashes indistinguishable from one that answers "I don't know" to everything. The reading now reports whether *anything* answered, and the panel says "not readable with this profile" and explains why. **No OIDs were invented** for the other makes: without a datasheet and hardware to test against they wouldn't be a measurement, they'd be a hope — which is the very thing this pass is removing.

- **Fixed** — on the **server**, a MAC's vendor now comes from the refreshable registry (IEEE + per-vendor plugins, `npm run update-oui`) instead of a table written into the scanning code. That mattered beyond tidiness: the scan row's vendor is the first link the classifier reads, so when the table was wrong it beat sysObjectID, OS fingerprint and registry combined — and it was wrong, listing `18:60:24` as Hewlett Packard where the IEEE assigns it to **Canon**. The four Cisco prefixes the bundled snapshot genuinely lacks now live in Cisco's own plugin. In the browser a short local table stays, deliberately: the "Add to map" dialog works on MACs read from switch forwarding tables and has to show a make **straight away**, without demanding a scan first. It decides no classification, a server-resolved vendor overrides it, and the Canon entry is corrected there too — a stopgap table may be incomplete, it may not be false.
- **Fixed** — a stacked switch's **port label no longer imposes one vendor's naming**. `2/0/24` is Cisco's fixed-config shape, and it was written for every device in a stack. Now the shape is the one **measured** on the interfaces (the engine already recognised four families and threw the result away), slot included; when the stack was declared by hand and nothing was measured, the label is `2/24` — member and port are known, the slot is not, so it isn't written.
- **Changed** — the DHCP live-pull dialog asks for the credentials **the driver declares**, instead of keeping its own per-vendor list to hold in sync: adding a driver on the server and forgetting that line meant asking for the wrong field. No vendor name is left in that file.
- **Changed** — VMware's enterprise OID is **no longer hardcoded** in the shared classifier: it was the only vendor with an OID written into the generic scorer, while Proxmox, XCP-ng and Nutanix all went through plugins. It now goes through its plugin too, and the rule reads the same for everyone. A port-numbering option also stopped naming a specific switch model.

- **Fixed** — a VM that answers over SNMP no longer **rewrites the state you set by hand**. Proving it is powered on is a measurement, so it now lives with the other measurements and shows up as a highlighted row when it disagrees with what you declared — and the transfer into the document stays the explicit gesture it is for every other measured field, undo included. Before, it was written straight into the declared field with no undo at all.
- **Changed** — the **"Add to map"** dialog no longer arrives with every observed candidate already ticked. These entries become real devices in your document; arriving pre-ticked made adopting the lot the default gesture and choosing the one you meant the exception. "Select all" is still one click.
- **Changed** — when an SNMP read finds **more interfaces than the port count you declared**, the drawing still shows them all, but the number you wrote is kept and the panel says so ("you had declared 24 ports"). It used to be raised silently, and the Overview then counted the new figure as declared.
- **Fixed** — LAG markers stripped at load because a cable leads to a **passive element** (wall port, patch panel, media converter) are now reported instead of disappearing in silence. The rule is right — you cannot aggregate towards a socket — but a marker you placed by hand vanishing at every load, with nothing said, left no way to understand why it never stuck.

- **Fixed** — the **SNMP indicator and the per-device LEDs now have an age.** `snmpStatus` survives the save, so a project reopened after months showed a green light and a rack of green bars on devices that might have been off for weeks. The count stays what it was — those devices really did answer — but the green now demands a *recent* answer: past six hours the toolbar dot turns amber and says why ("13 whose answer is not recent, last one 4 d"), and a device's bar goes **dim green**: it answered, just not now. Not red, which would claim a fault, and not amber, which would mean waiting.
- **Changed** — the 7-day staleness gate now also covers **Expansion**, whose verdict rests on measurement (ports the device reports active, PoE classes read from the switch): after weeks with no contact, "ample headroom" is a memory. It deliberately does **not** cover **LAN** — that column asks whether the *document* describes everything, and a document does not age — nor **Recoverability**, which already has its own 30-day backup clock; two clocks on one verdict read as two facts. **Security** turns out not to need it either: its three inputs (the driver you chose, the community you typed, the VLAN you marked) are all declared, and declarations don't go stale because you haven't synced.

- **Fixed** — a summary row that forgets to state where its number comes from no longer claims a human wrote it. The engine's default was "declared", so the mistake always fell on the side that promises more; it is now "not declared", and a test on the source keeps the door shut. Three rows were wrong because of it: **cables** called a set declared even when most of it was inferred by auto-link (15 of 17 on a real project), **MAC addresses** were "declared" though nobody types 48 bits from memory, and **free ports** are a subtraction the app performs, not a figure anyone wrote.
- **Fixed** — **"0 VLANs named" is no longer a green** on a project with no VLANs in use: nothing to name is not the same as everything named.
- **Fixed** — the dossier cover prints a **dash** for a counter that was never supplied. "0 virtual machines" at 22pt is an assertion, and on a missing number it is an invented one.
- **Fixed** — the PDF's free-ports page distinguishes **"0 fibre ports free"** from **"no fibre ports declared"**, as the Overview already does — same metric, one truth. A device with no fibre shows a dash instead of a zero.
- **Fixed** — the dossier no longer opens the Recoverability page with **"0/0 recoverable"** when there is nothing to assess; and the 30-day backup and 90-day lifecycle thresholds now come **from the engine** instead of being retyped in the report — two numbers for one question were two truths waiting to diverge.
- **Fixed** — the L3 map's **CSV export** and the delete-rack confirmation now follow the interface language; they were hardcoded Italian.
- **Fixed** — with no valid subnet, the IPAM engine reports free addresses and occupancy as **unknown rather than zero**. Every caller already guarded against it, but "0 free" reads as "the network is full" — the worst answer to leave loaded for the next caller who forgets.

- **Security** — the **backup pointer** no longer reaches the PDF dossier with credentials inside it. The Recoverability page was the only consumer of `backup.ref` that printed it verbatim, while the REST DTO has always stripped it — and a dossier gets forwarded by email, where a leaked credential cannot be recalled. The pointer itself stays readable: stripping is not deleting.
- **Security** — credential detection in the backup pointer had **two ways through**: the scp/rsync form without a scheme (`user:pass@host:/path`) and a password containing `/`. Both are now caught, without flagging the legitimate pointers that also carry `:` and `@` — `git@github.com:org/repo.git`, a `:port`, an `@` inside a path, a Windows folder named after an address.
- **Security** — that check now also runs **on the server**, on save and on project import. It lived only in the field you type into, so a request built by hand or a JSON imported from elsewhere walked past it — and from there the secret would have reached the REST DTO, the Ansible inventory and the dossier. The save is not refused: the secret is removed and the pointer kept.
- **Security** — the **SNMP community is masked** like the v3 passphrases already were, in the device panel and in the two scan dialogs. The server has always treated it as a secret (it is blanked for non-admin readers); the screen was the one place still spelling it out — including in screenshots.

- **Security** — SNMP secrets are now redacted for non-admin readers on **virtual machines** too (`vm.integration` and the legacy `vm.snmp`), not only on hosts: a viewer opening a project with virtualized hosts used to receive the VMs' community and v3 passphrases in clear.
- **Fixed** — a LAG created **by hand** is no longer overwritten by the LLDP/CDP inference, and renaming a LAG group is no longer reverted at the next Sync: what you declared and what was measured are both off-limits to automation (manual-first).
- **Fixed** — the cable, Wi-Fi and cabling-chain validators now speak **English too**: their warnings were hardcoded in Italian inside the pure engines, invisible to the parity test. The engines emit codes and data, the words live in the translation files — with a test that fails if a new rule ever ships untranslated.
- **Fixed** — a port that was **never observed** is no longer presented as "off": the status menus offer "— not determined" and the endpoint's read-only box no longer states "Inactive". Measured-down and never-measured are different facts.
- **Fixed** — a rack that never declared its height no longer gets **42U written into the document**, and a rack-space total resting on that assumption is reported as derived — never as "rack full".
- **Fixed** — **"0 suspect ports · all coherent"** no longer appears as a measured fact on a project never read over SNMP: the row stays dashed until there is a reading.
- **Fixed** — when the IPAM hygiene engine fails, the Overview says **"not assessed"** instead of a green "no conflicts": a failed measurement is not a clean result.
- **Fixed** — the PDF's **Recoverability (DR)** page no longer counts a device whose declared serial differs from the measured one as recoverable: it marks the row "replaced?" and tallies it, matching the Overview's DR lens.
- **Changed** — the catalog's per-type default make (switch → "Cisco", router → "Juniper", …) is **no longer written into the device** on drop, discovery import or drift adoption — it stays a placeholder. A make on a device is now either declared by you or measured, so no invented `ansible_network_os` reaches the Ansible inventory, the PDF asset register or the AI context.
- **Changed** — in the classifier, an **LG MAC prefix on its own** now scores within the ≤45 ceiling the engine declares for every OUI signal; the "webOS" product string keeps its full weight.
- **Added** — the Overview ends with a footnote, **"What I'm not looking at"**, naming the dimensions this summary does not judge — WAN/circuits, L3 routing, spanning-tree, firewall/ACL, AAA/syslog/NTP, warranty/EOL, restore proof, temperature and interface errors. It shows under every lens, and the list is data in the engine: retiring an entry once a dimension gets modelled is a one-line change.
- **Added** — the Overview gains a sixth lens, **"Health"**, answering *what problems are there right now?* — the only lens that speaks about the present. It composes the telemetry devices already returned (**RAM and disks** from HOST-RESOURCES, **printer consumables**, **UPS** battery/runtime/load) through the same documented thresholds the AI assistant reads, so "RAM full" means one thing across the product. Each row's denominator is who actually answered with that telemetry — a NAS that doesn't speak Printer-MIB is not "a printer with no alerts" — and **CPU load is reported, never judged**: a threshold on an instantaneous reading would be invented. The verdict is grey when nothing was read, and a green one **degrades to amber once the oldest reading passes 6 hours**: health read yesterday is not today's health.
- **Fixed** — **a negative PoE headroom is a debt, not a margin.** A switch whose declared budget doesn't cover the measured port classes used to read "−40 W headroom" in green, and the fleet total hid it by netting it against other switches' spare watts. The debt now comes out separately and per device, turns the row amber, and counts in the column's verdict.
- **Changed** — two entries **left** "What I'm not looking at": UPS power and device health now enter a verdict in the new lens, so declaring them unwatched would be false. What genuinely stays outside is named as such — temperature sensors and interface error counters, which are never polled.
- **Changed** — the Overview's first column is now titled **"LAN"** (was "Project"): the question it asks — *does the document describe everything?* — is about the network, not about the file you have open. The Conformance column's "Project networks" row follows suit: **"LAN networks"**.
- **Added** — every device's Properties panel gains two declared dates next to serial and firmware: **Warranty until** and **End of life (EOL)**. Nothing measures them — no OID tells you when a contract expires — so an empty field means "I don't know", never "unlimited".
- **Added** — the Recoverability lens gains a **"Lifecycle"** row summing those dates worst-first: **end-of-life** (past EOL, you can't buy it again), **out of warranty**, **expiring** within 90 days, with the drill-down naming each device and its date. The denominator is whoever declares a date, not the fleet — and the verdict says how many are left without any, so "12 of 12 covered" is never a green bought with ignorance. It is advisory: an end-of-life device with a fresh backup is still recoverable, because EOL says you can't *re-buy* it, not that you can't bring it back up. Warranty/EOL therefore leaves "What I'm not looking at".
- **Added** — the PDF's **Recoverability (DR)** page carries the same lifecycle: a **"Lifecycle"** column per device (EOL · out of warranty · expiring, with the date that drove it) and an **"N end-of-life"** counter in the header verdict. Same thresholds as the lens — a dossier that contradicted the app would be worse than no dossier.
- **Fixed** — the **Conformance** column can turn green again. "Verifiable over SNMP" counted PCs, phones and printers in its denominator — devices nobody polls — so the column stayed amber forever with no action able to close it, and the count never moved either. It now counts **what you actually poll**, and endpoints are named in the verdict ("all reachable — 20 endpoints by presence") because Verify checks them by presence, not by SNMP.
- **Added** — the dossier opens with the **Overview**: one page, right after the floor plan, carrying the three questions and their verdicts — the same ones on screen, with the same provenance dot next to every number and its legend, and the **"What I'm not looking at"** line at the bottom. Until now the verdicts never left the app: the PDF handed over raw data and kept the judgement inside. It lists no devices (that's the asset register) so it stays one page, and it can be switched off like any other section. The words come from the app itself — the pure engine stays the only source of the numbers, the glue the only source of the words, and the server just draws.
- **Fixed** — characters outside WinAnsi no longer render as garbage in the PDF: the typographic minus "−" used on screen came out as a quotation mark. The Overview page normalises what the interface actually uses (minus, arrows) and turns anything else outside CP1252 into "?" — an honest character beats an invented glyph.
- **Changed** — "Verifiable over SNMP" carries **two counters** — how many answer among those you poll ("12 of 13") and, in amber beside it, how many you don't poll at all ("20 not verifiable"). Two populations that never add up, so they never look like one ratio. And it writes **names, not counts**, and one thing decides who is in it: **the driver**. An SNMP driver set means "I poll it, it must answer"; no driver means "I don't poll it" — and that holds for *any* device type, because a printer speaks Printer-MIB just as a switch speaks IF-MIB. With several racks, "2 not responding" meant leafing through every rack page to find out which: the row says *"no answer: OfficeJet8715"* straight away (up to three names, then "+N").
- **Changed** — once everything you poll answers, that row turns **green** with the second counter keeps saying how many stay out, and one click opens the **full list of the not-verifiable ones** — your standing list of in-person checks. It covers every device the panel offers SNMP Integration for: printers, NAS, cameras and UPSs too, not just switches and routers. A device configured but silent is a *failed measurement*: it shows in red, keeps the green away and counts in the column's tally. One that isn't polled is a decision, not a problem: it never blocks the verdict, but it never disappears either — and with zero accesses configured the row is dashed, not green.
- **Fixed** — the **Security** lens no longer shows a green "compliant" dot next to the words *"no SNMP access measured"*. With not a single access configured there is nothing to assess, so the verdict is grey: the app cannot know whether those devices don't speak SNMP or whether nobody has written the access down yet, and a declared management VLAN doesn't buy the green either — it describes segmentation, not the exposure of an access that doesn't exist.
- **Changed** — the "What I'm not looking at" footnote now always fits on **one line**. It used to wrap onto two or three rows and push the results down; the chips are compact and their labels short, with the full name opening each tooltip so nothing is lost.
