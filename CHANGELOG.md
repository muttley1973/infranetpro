...
### Fixed
- **An inferred FDB uplink on a LAG port no longer masquerades as a confirmed "LAG".** When the Sync cables a documented switch to the port where its MAC is learned (`MAC-UPLINK`), the *remote* port is a default (`<node>-1`) and the remote end — often an unmanaged, SNMP-blind switch (e.g. Zyxel GS1200) — was never observed: we don't know which of its ports the cable enters, nor whether it forms a LAG. But because the *local* port is a LAG member and both ends are switches, the link picked up `lagLogicalKey` and `linkState()` showed it as **"LAG" (confirmed)** instead of *"Inferred · to verify"*. A `MAC-UPLINK` candidate is now never promoted to a LAG member (guarded at both the candidate-build and the dedup/`_refreshLagMeta` passes): it stays **one** *"Inferred · to verify"* cable with a correctable remote port — attach one, confirm and adjust the rest by hand. As a bonus the dedup pass now self-heals any such link previously mislabelled "LAG". Vendor-neutral (keyed on the protocol, not the vendor); genuine LLDP/CDP LAGs and MAC-inferred LAGs between two managed switches are untouched. `src/app-autolink.js`.
