...
### Added
- **A port can now carry its own address.** An address belongs to an interface, not to a box: a router has one per port, and while the model held a single address per device the second interface could only exist as a second device — which is how a MikroTik already on the map came back as an unrelated neighbour announced over LLDP. The device card keeps the management address; each port can declare the address it answers on, and it shows in the port tooltip so it reads from the map without opening anything. Passive ports — patch panels, wall outlets — do not offer the field: copper passing through is not an interface, and offering the field there invites a false entry. What you type is stored exactly as typed, manual-first; if it does not look like an IPv4 the panel says so rather than refusing it.
- **The Name column in Discover no longer repeats the IP address.** On a real network about half the devices announce neither a model nor a usable hostname, and the column fell back to printing the address — the same address already sitting two columns to the right. It now composes a readable line from what was actually measured, the type and the brand ("IoT-AzureWave"), using the same library the floor plan uses for exactly this problem; a composed line is printed in a lighter weight and says so on hover, because nobody declared it. A real name always wins and is left untouched.
- **The Discover table has draggable columns.** Vendor names were being cut off — "Zyxel Communic…", "AzureWave Tech…" — with no way to see the rest. Drag the edge of any fixed column to widen it, double-click that edge to put it back; the widths are remembered per browser. Name stays elastic and takes whatever space is left, so one column always absorbs the slack instead of every column shifting on each drag. Widen past the dialog and the table scrolls sideways, with the header staying put. The full vendor string is also on the cell as a tooltip, so it is readable without dragging anything.

### Fixed
- **A device that will not answer SNMP no longer looks like a device that is gone.** On the floor plan the ring around the tile was red and dashed: red is the colour of proven absence, and dashes already mean something else entirely in this app — *inferred, to be confirmed*, the language of cables deduced from LLDP and MAC tables. One sign was carrying two meanings and borrowing the wrong colour. The ring is now orange, and it is drawn exactly like the red halo of an absent device — same weight, same glow, hugging the tile the same way — because the two are readings of the same device and should look like it. What changes is the colour, which is the news. Hovering the tile now says which one it is and why — a coloured ring with no words made people ask.
...
- **Discovery sightings keep 10,000 records instead of 1,000, and the verification timeline 10,000 rows instead of 2,000.** Both numbers sat next to an ageing rule that says what retention actually means — sightings unseen for 90 days, timeline rows older than a year — and in both cases the count ran out first, so the ageing rule never got to decide. The old sighting limit was justified by its weight in the project file, which is no longer where it lives; and a sighting is an endpoint *seen*, not a documented device, so a real project of six nodes already carried 204 of them. What fell off was the oldest record — the first-seen date the confidence score is built on. The timeline told the same kind of half-truth: one row per full verification, hourly monitoring, 8,760 rows a year against a ceiling of 2,000, so a stated year of history stopped at eighty-three days. A row measures 270 bytes. Both ceilings are now backstops against unbounded growth, which is all they were meant to be, and a test keeps each one clear of the rule it was quietly overriding.
- **When the sighting ceiling does bite, it now gives up the sightings seen least recently.** It used to drop from the front of the list, and the list is not in date order — a known sighting is updated in place, so the order is the order things were *first* seen. Cutting from the front therefore discarded the longest-established records, the ones with a high count and a distant first-seen date, which is precisely what the confidence score is made of. Undated records go first, since they carry no evidence of recency at all; ties are broken by arrival, so the cut is repeatable; and the survivors keep the order they were already in.
- **The change journal keeps 10,000 entries instead of 1,000.** It no longer lives inside the project file, so its length costs the document nothing — and throwing away who changed what is the opposite of what a network document is for. The ceiling was declared in two places, and the copy written by hand at the call site was the one that won, so raising it in the library alone would have changed nothing. There is one declaration now. The history list draws the most recent 500 and says how many are behind them; the filter still searches the whole journal and the CSV export still holds all of it.
