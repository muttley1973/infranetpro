...
## [Unreleased] — post-2.8.0

### Fixed
- **A device you switch off stays red after a reload.** The check measures who is there and who is not and writes that onto every device — then waited for someone to press Save. But autosave is off by default and the automatic monitor runs a check every hour, so each measurement lived only in the open page: reload, and the floor was repainted from the last time the document was saved, turning a device that had been off for hours green again. Measured on a real network: four checks in one afternoon, the last one proving four devices absent, while the file on disk still carried the presence of nine hours earlier. The presence is now stored the moment it is measured, beside the project's history rather than inside the project file — because it is not an edit. The document is what you write (where a device sits, what it is called, how it is wired); presence is what the network writes, and making it wait for a manual save is what lost it. Nothing else changes: your unsaved edits stay unsaved, the project file is not touched behind your back, and when a check reopens a project the fresher of the two measurements wins, so a stored reading never drags a newer one backwards. "Absent" still never travels without the hard evidence that justifies it.
