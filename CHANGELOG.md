...
### Security
- **The AI chat is admin-only.** It uses the admin's own LLM key and sends the project's topology to an external provider — an egress, like every scan and poll, and those are all admin-gated. A logged-in viewer could drive that key and force that egress; now the route refuses a non-admin, in line with the rest.
- **A colour from an imported project can no longer run as code.** VLAN colours are written straight into a `style` attribute; from the interface they are always `#rrggbb`, but a hand-edited project file could carry `red" onmouseover="…` and break out of the attribute. Opening a project now snaps any colour that is not a plain `#rrggbb` back to the neutral grey — one place, before it reaches any panel.

### Fixed
- **An address written on an interface is finally an address.** A router answers on one address per port (the field added in 2.8.2); nothing that counts addresses was looking at it. Occupancy under-counted, the "next free" suggestion could hand back an address already on a port, and the L3 map called a gateway that lives on a port "held by no device". All three now read the port's address, the same identity rule as everywhere else.
- **A cable inferred twice is removed even when the real one is corroborated.** The pass that kills the phantom "second cable to an invented port" trusted `LLDP` and `CDP` but not the fused `LLDP+MAC` — so the duplicate survived in exactly the case where the adjacency was confirmed by two sources. It now shares the one definition of a trusted protocol.
- **The overview reads every declared network, not one per VLAN.** It walked the per-VLAN view, so a declared network with no VLAN — most of a real NetBox — was counted as "undeclared", and a VLAN's second (dual-stack) prefix never showed. It now reads `ipam.prefixes[]`, the authority, so those networks count and their missing gateways surface.
- **The topology crawl stops overwriting a pinned name and adopting a next-hop's MAC.** The LLDP crawl rewrote a device's name without checking the manual lock, and skipped the "don't take a gateway/VRRP MAC as identity" filter that the Discovery path already applies — the same two guards, now on both paths.
- **A trunk you set by hand stays a trunk.** When the carried VLANs collapse to one (native equals tagged), a manually-forced trunk was quietly demoted to access and labelled "derived", as if no one had chosen it. The explicit choice now wins.
- **The same IPv6 spelled two ways is one address, in the topology too.** Neighbour matching and the address index compared IPv6 by raw text, so a management address written expanded in the document didn't match the canonical form measured over ND — a router left unlinked. Both now compare by identity.
- **A link-local is no longer a duplicate.** `fe80::1` is unique per link (RFC 4007), so the same one on two routers on different segments is normal, not a conflict; the hygiene audit skips it.
- **An IPv6 network with no VLAN can't become "VLAN 0".** An empty VLAN key migrated to `0` (`+"" === 0`), the same trap already closed elsewhere; an empty key now means no VLAN.
- **An IPv4 written with leading zeros is the same address.** The field accepts `192.168.001.005` (decimal), but the occupancy and duplicate engines keyed it by raw text, so it counted twice and a real duplicate slipped through. The key is now the canonical form.
- **A "change IP" row carries which device and which new address.** Its key had neither, so ignoring one change hid a later change to a different address for good, and two devices sharing a VRRP MAC collided onto one row. The key now includes the node and the new address, like every other category.
- **SNMP no longer writes "VLAN 1" when it measured nothing.** A port with no VLAN reading and no prior value was stamped `1` as if measured; it now stays unset, and the native VLAN is derived from whoever is authoritative.

### Added
- **A "Networks" section in the project panel**: one list of every declared network, sorted by address, with its VLAN as a badge rather than a container, and overlapping pairs marked with the conflict spelled out. Sorting by VLAN would hide exactly what you need to see, since two colliding networks are neighbours in the address space, not in the VLAN list. Click a row and its detail opens right under it — VLAN, name, gateway, DNS, occupancy, Adopt. The field to add one sits after the list; type several separated by commas and they all land in one step, while what does not parse stays in the field, in red.
...
### Changed
- **"Networks" comes before "VLAN" in the project panel**, and the panel's subtitle says so. The addressing plan is the document; a VLAN is a label a network may carry. The subtitle used to list "Map · VLAN · …" while the map was third — it now follows the real order.
- **The networks list reads as a table.** Named columns, and a miniature occupancy bar on every row: six fractions with different denominators (2/254, 21/254, 0/126) do not compare at a glance, six bars do. Same colours and same amber threshold as the "Occupancy" block in the detail, so the small one and the large one cannot drift apart. No bar for an IPv6 prefix, where there is no capacity to fill. The whole list moved up the type scale — it sat at the size the system reserves for footnotes, smaller than the form nested inside it — and the columns now line up across rows, which they did not.
- **A VLAN shows as a pill with a colour dot, not as coloured text.** Written in colour, "VLAN 99" (red in the palette) was indistinguishable from an error in a list that marks real conflicts in red. The palette colour is arbitrary and says nothing about health.
- **IP addresses, CIDRs and MACs are written in the interface font**, with tabular figures so digits still line up — the one thing monospace was actually providing. Applies everywhere an address appears: Discover, Drift, the L3 map, the shared segment, Adopt, the overview, and interface names in the topology tooltip. Code, logs, API tokens, raw `sysDescr` and configuration textareas stay monospace.
- **The "L3 map / Gateway" report is wider (780px) and larger.** It is the only report with a six-column table; at 680px device names were being cut. The other reports keep their width.
- **The L3 map is one row per network, not one per VLAN.** The network is the identity of the row and the first column, because it is the thing being routed and the only one always present — a VLAN is optional. A VLAN with no network stays in the list, saying so; it has nothing to route, and dropping it silently would be worse. "Routed by" on the VLAN card still asks one question and gets one answer: the SVI is a single interface even when it carries two addresses. The CSV follows the same shape.
- **Every editable field in the properties panel is the size of the data it writes.** Said once instead of rule by rule: the fields outside `.prop-group` — VLAN name, new-VLAN id, "Add network" — were each forgetting it separately.

### Fixed
- **The import stopped being silent about IP addresses that stay out.** InfraNet documents the address *of* a device, so only addresses assigned to an interface come in — but on a real NetBox 180 out of 180 were attached to nothing, and the preview said "IP addresses 0" next to "Prefixes 90". Correct, and it read like a fault. A line now says how many stayed out and why. Same reason `cable.outOfScope` exists: nothing is lost, but silence was asserting something untrue. The count comes from two `limit=1` requests, so it costs two calls and not 180 pages; and it is only printed when it can be proved — an address that came in *is* attached, so the unattached ones cannot outnumber (total − imported), and a figure above that ceiling means the filter was ignored by that NetBox version, not that the addresses are free. In that case nothing is printed: no number beats an invented one. The line also shows a few of the addresses, because a count on its own cannot be judged — "180 stay out" is a question, "180 stay out, for example 10.0.5.7/24" is an answer.
- **The examples collected for a decision were never shown.** The mapper had been gathering five sample values per issue and the decision's documented shape declared them, but nothing carried them past `buildDecisions` and the panel never printed them. They now reach the row — which also gives "networks with no VLAN" the examples it has been collecting unseen for weeks.
- **Three Italian strings printed backtick characters on screen** ("puo\`", "piu\`", "e\`"): the ASCII convention used in code comments had leaked into user-facing copy in the "Networks" panel.
- **`--font-mono` was never defined.** Seven rules asked for it and twenty more declared a bare `monospace`, so on Windows 27 of 30 rendered in Courier New and 3 in Consolas. There are now two font tokens, `--font-ui` and `--font-mono`, and every declaration points at one of them.
- **The VLAN name field rendered in the system form font**, the only field in the app in a different typeface: form controls do not inherit `font-family` unless told to, and that one was never told.
- **A network imported from DCIM shifted its own row by one column**, because the provenance tag was a second grid child instead of sitting inside the address cell.
...
- **No IPv6 gateway was ever checked by anything.** The L3 report walked the VLANs and read each one's *primary* prefix — the first IPv4 — so the second prefix of a dual-stack VLAN never reached it. A VLAN with a /24 and a /64 reported "1 network, no issues" while the v6 gateway went unexamined: not out of subnet, not orphaned, not anything. It is examined now, along with the networks that carry no VLAN, which the per-VLAN walk could not reach either.
- **An IPv6 gateway never matched the router that answers it.** Devices resolved by `ip` alone, so a gateway of `2001:db8:0:20::1` was an orphan even with the router's `ip6` set to exactly that — and the network detail said "no device holds this address", which was false. Addresses are now compared by identity, not by text: two spellings of one IPv6 (`2001:DB8:0:20:0:0:0:1` and `2001:db8:0:20::1`) are one address. That rule lives in `lib/cidr.js`, next to the address arithmetic, so the report and the audit cannot drift apart on it.
- **An IPv6 network always read as empty.** The collector that gathers documented addresses dropped anything that failed to parse as IPv4, and never looked at `ip6` at all — so a /64 with six devices in it showed "1 IP", the gateway that the occupancy engine adds on its own. Both families now count, VM interfaces included, and the same address written two ways counts once.
- **The same IPv6 on two devices was not a duplicate.** The hygiene audit read `ip` only. It reads both families now, in canonical form, which is the case that matters: one address arrives from SNMP already canonical, the other was typed by a person.
- **An IPv6 network counted one address twice.** Occupancy deduplicated by raw string, so the gateway written on the prefix and the `ip6` of the router holding it — the same address in two spellings — were two occupants. A /64 with two devices in it reported three. On IPv4 the key *is* the string, so nothing there changes.
- **"Gateway out of subnet" was the answer to two different questions.** An IPv4 written on an IPv6 network is not an address a digit off — it is the wrong kind of address, and it says so now, separately.
- **The network/broadcast-address check no longer relies on an accident.** It is 32-bit arithmetic on fields that do not exist on an IPv6 prefix; it happened to produce `0.0.0.0`, which nobody writes as a gateway. It is now gated on the family explicitly, and the all-zeros address of a /64 is left alone: that is the subnet-router anycast (RFC 4291), not an unusable address.
