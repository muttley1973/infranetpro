...
- **Fixed** — a MAC's vendor now comes from the **refreshable registry** (IEEE + per-vendor plugins, `npm run update-oui`) instead of two hand-written tables in the code — one in the scanning module, its twin in the browser, the same 35 prefixes, which were the test bench's inventory. That mattered beyond tidiness: the scan row's vendor is the first link the classifier reads, so when the table was wrong it beat everything else — and it was wrong, listing `18:60:24` as Hewlett Packard where the IEEE assigns it to **Canon**. The four Cisco prefixes the bundled snapshot genuinely lacks now live in Cisco's own plugin, and the browser keeps a session cache fed by the server rather than a list of its own: until it has learnt a prefix it says nothing, which is the honest answer.
