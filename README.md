...
- **Chat controls** — a settings **gear** in the chat header (admin) reopens the scope/capability config at any time, a red **trash** button clears the conversation (session-only), and saving the config refreshes the panel instantly (enable → chat, endpoint → 🔒 Local / ☁ Cloud chip)
- **Clickable citations & anti-invention check** — answers surface the devices/VLANs they used as **clickable chips that jump to the node on the map**; a downstream check (`lib/ai-grounding.js`) flags any IP/MAC the model names that **isn't in your data** with a ⚠ *"reference not found"* chip. The model is also fed **live facts** (drift + IPAM occupancy computed in the browser, re-sanitized server-side, gated by the *Drift* scope toggle), so it reasons over the current reality, not just the saved JSON
