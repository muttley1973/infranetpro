...
- **Recoverability (DR) section in the PDF** — an optional page, one row per managed device, that is the runbook to **rebuild from**: **where the config backup lives** (the pointer + method, never the config or credentials), the **date/time** it was last taken, the **serial and firmware** to procure and reflash, and the **rack** it goes back into — under a header verdict *"X/Y recoverable · N without a backup · N without a location"* (recoverable = fresh backup **and** known identity **and** known location, matching the Overview's DR lens). Built from the same **secret-free allowlist DTO** as the asset register; kept off-site, it survives the very LAN it rebuilds
