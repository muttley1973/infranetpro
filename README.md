...
**Discovery — next (from a 2026-07 competitive review of Auvik / netdisco / Lansweeper / NetBox):** the industry model is *neighbour (CDP/LLDP) → `arpnip` (ARP/`ipNetToMediaTable`) → `macsuck` (FDB/port)*. We now cover neighbour + `arpnip`; the ordered next steps:
