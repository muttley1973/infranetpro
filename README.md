...
> 📰 **What's new:** see [CHANGELOG.md](CHANGELOG.md) — latest: **off-segment discovery via SNMP ARP tables** (a host a switch/router has in its `ipNetToMediaTable` is proposed even when it answers no ping/SNMP/LLDP — the standard `arpnip` method, refined with DHCP leases), **no false SNMPv3 label** on non-SNMP hosts (a real remote engineID is required), plus multivendor discovery hardening (ifName‑anchored SNMP port mapping, `vmVlan` access‑VLAN fallback, spaced ping‑sweep retry, LLDP‑neighbour port alignment, crawl vendor preserved), AI assistant (advisory, bring‑your‑own‑key), per‑VLAN IPAM occupancy, DHCP lease import, REST API v1.
