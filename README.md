...
**Discovery — next (from a 2026-07 competitive review of Auvik / netdisco / Lansweeper / device-type):** the industry model is *neighbour (CDP/LLDP) → `arpnip` (ARP/`ipNetToMediaTable`) → `macsuck` (FDB/port)*. We now cover neighbour + `arpnip`; the ordered next steps:
- [ ] **Switch-port mapping (FDB "macsuck")** — surface the MAC↔switch-port table (`dot1dTpFdbTable` / `dot1qTpFdbTable`, already collected for auto-link) as a first-class view — *"this device is on `Gi0/5` of SW-ACC1"* — and auto-draw the cable to the exact port. Headline feature of the discovery leaders (netdisco `macsuck`, Lansweeper switch-port mapping) and the highest-value gap for a documentation tool. Incremental: `buildFdbCandidates` plumbing already exists
