...
> 📰 **What's new (v2.2):** see [CHANGELOG.md](CHANGELOG.md). The read-only **Overview** grew into **five lenses** you switch between — the three summary columns (**Project · Conformance · Expansion**) plus **Recoverability (DR)** (*if it falls tonight, can you bring it back up?* — config backup freshness, hardware identity incl. make/model + firmware, location, HA) and **Security & Services** (SNMP posture v3 vs v1/v2c, default communities, management-VLAN segmentation). The **Conformance** column now surfaces **IPAM conflicts** (duplicate IPs, overlapping subnets) and **VLAN names read over SNMP** (with the declared name kept as law). And the keystone: the **PDF report gained a "Recoverability (DR)" section** — where each device's config backup lives, what to procure and reflash (serial/firmware), and where it goes back — an offline runbook you keep off-site so it survives the LAN it rebuilds. The Overview also closes with a **"What I'm not looking at"** footnote — naming the dimensions it does *not* judge (WAN, routing, spanning-tree, firewall, management services, lifecycle, backup-restore proof, UPS power, live health) so a quiet screen is never mistaken for *all clear*. Every figure still declares its **provenance** and no secret ever leaves the machine.
