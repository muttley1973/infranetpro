...
>
> 📰 **Before that (v2.3.1) — the classifier stops guessing from brand names:** a word no longer outranks a measurement, a brand no longer decides a type, and **"switch" stops being the answer for everything unrecognised**. The Dashboard pairs **IP and MAC in one row** and breaks free ports down **by speed**. Full detail in [CHANGELOG.md](CHANGELOG.md).
