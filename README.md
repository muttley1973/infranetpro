...
**Discovery — next (from a 2026-07 competitive review of Auvik / netdisco / Lansweeper / NetBox):** the industry model is *neighbour (CDP/LLDP) → `arpnip` (ARP/`ipNetToMediaTable`) → `macsuck` (FDB/port)*. We now cover neighbour + `arpnip` + `macsuck` + DHCP-as-source; the ordered next steps:
...
- [ ] **Source-of-truth positioning** — keep discovery **propose-and-reconcile, never overwrite** (NetBox's *"discovered ≠ intent"* model): the manual-first rule is the product thesis, and the differentiator vs the monitoring incumbents
