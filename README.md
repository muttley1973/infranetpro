...
- **L3-lite gateway** — bind each VLAN to the device that routes it (auto-suggested, manual override wins): an L3 badge, a read-only SVI panel section, and a **Report → L3 map** with orphan and out-of-subnet warnings plus CSV.
- **IPAM hygiene** — the L3 map flags the same IP documented on two devices and two VLANs with overlapping subnets (`lib/ipam-audit.js`, document against document, never invented).
...
- **Overview page in the dossier** — one page after the floor plan carrying the three questions, their verdicts, the provenance dots and the *"what I'm not looking at"* line. The executive summary the dossier was missing.
...
- **One click per row** — *update doc*, *ignore* (persisted until the condition changes), *investigate*. The diff is a pure tested function (`lib/drift-report.js`), and the result lands in the Overview's Conformance column as saved state rather than a transient overlay.
