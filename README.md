...
> 🧪 **Post-v2.7.2 development branch — NetBox compatibility hardening.** The DCIM/IPAM import now uses the canonical CC0 catalog with neutral port-slot semantics, explicit reconciliation and revision tracking. Portable JSON exports are versioned and secret-free, history sidecars are crash-safe and cleaned with their project, and the workflow is covered by browser E2E tests. The complete delta from the base release is in [CHANGELOG.md](CHANGELOG.md).
