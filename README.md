...
- [x] **A port shut on purpose vs one merely idle** — `ifAdminStatus` enters as a measure, the two off-states share a monochrome scale in rack, PDF and draw.io, and a cable drawn over a shut port gets a "Port shut" badge. A status you declared keeps your colour, and the reading expires when the switch stops confirming it
