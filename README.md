...
> 📰 **What's new (v2.3.1) — the classifier stops guessing from brand names:** see [CHANGELOG.md](CHANGELOG.md). Deciding **what a device is** was the last important decision in the product with no regression net behind it. There is one now — a shared corpus of 53 rows run through *both* engines on every test run — and behind it the rules changed: a word in a description no longer outranks a measurement over SNMP, a brand name no longer decides a type (one maker builds UPSs, PDUs, transfer switches *and* racks), the operating system is counted once instead of twice, and **"switch" stops being the answer for everything unrecognised** — an SNMP responder that declares no layer and matches no rule now comes out as the generic type, because its confidence already said *I don't know*. The browser stopped being a second, divergent classifier: disagreement between the two engines fell from 17 rows out of 53 to 4. The Overview also pairs **IP and MAC in one row** — reading the address from the interfaces on managed gear, where the chassis field is empty — and breaks free ports down **by speed**.
>
> 📰 **Before that (v2.3) — the honesty pass:** one theme ran through that release: **the app no longer says more than it knows.** A verdict resting on nothing is grey, not green — the Security lens used to show a green dot beside the words *no SNMP access measured*. A measurement now carries its **age**: `snmpStatus` survives the save, so a project reopened after months showed a green light on devices possibly powered off for weeks; past six hours the indicator turns amber and says why, and a stale green verdict in Conformance and Expansion degrades after seven days. Every summary row must **state where its number comes from** — the engine's default was «declared», so a row that forgot to say fell on the side that promises more, and three rows were wrong because of it (cables called a set declared with most of it inferred, MAC addresses «declared» though nobody types 48 bits from memory). Measurement stopped **correcting you behind your back**: a VM answering over SNMP no longer rewrites the power state you set by hand, the port count you declared is kept when a scan finds more interfaces, and «Add to map» no longer arrives with every observed candidate pre-ticked. A backup pointer no longer reaches the **PDF dossier with credentials inside it**, and that check now also runs on the server. A transfer switch that doesn't speak the only documented profile **says so** instead of showing a card of dashes — and no OIDs were invented for the makes we cannot test. The **fibre run is validated at last** (reach depends on the speed, not just the class: OM3 carries 300 m at 10G but 100 m at 40G), while 10G over Cat6 within 55 m stops warning about a compliant run. Plus a sixth Overview lens, **Health** — *what problems are there right now?* — **warranty/EOL** dates feeding a Lifecycle row and the DR page, and the **Overview page opening the handover dossier**: until now the verdicts never left the app while the PDF handed over raw data. Every figure still declares its **provenance** and no secret ever leaves the machine.
...
- **Device labels say what a thing is, not just where it answers** — the label carries the name on top and the address underneath. When Discover finds no usable hostname it stores the IP as the device name, so half a real map would otherwise read `192.168.1.133`; in that case the readable line is **derived for display** from what was actually measured — the classified type and the OUI vendor (`IoT-AzureWave`, `NAS-LaCie`) — while `node.name` is never rewritten, and a declared name always wins and renders at full weight. The same rule feeds the drift report, audit log, L3 map, cabling editor, cable labels and the dossier's asset register, so a device reads identically everywhere (`lib/node-label.js`, pure and tested; vendor strings are reduced from the IEEE company name to the brand, and OUI placeholders for randomised MACs are never printed as a manufacturer)
...
- **Operating-system logos** — the device and VM property-panel **headers** (and the VM list) show the **OS logo**: Windows, the Linux distros (Ubuntu, Debian, RHEL, Fedora, openSUSE), macOS, Android, FreeBSD/BSD, Raspberry Pi, plus hypervisor platforms (VMware, Proxmox, Hyper-V → Windows, Azure Local, Nutanix, Unraid, KVM → QEMU). Drawn from public-domain / permissive icon sets (Simple Icons + VectorLogoZone, CC0; dashboard-icons, Apache-2.0), monochrome in dense lists and full brand colour at accent points. Same honesty rule as everywhere: a specific logo only from an **authoritative source** (SNMP `sysDescr`, a manual OS/platform field, a VM's guest-OS), a **grey family glyph** when it's only a TTL hint, and **nothing** when the OS is unknown — never a guessed default (`lib/os-icon.js`, pure + tests)
...
- **Overview view** — a read-only view switch (like Topology; it never touches the document) that answers three standing questions in three columns — **Project** (is it complete?), **Conformance** (does it still match reality?), **Expansion** (how much can I grow?). Each cell carries its own number *and* a plain-word verdict, and declares the **provenance** of every figure (declared / measured + date / derived / not declared): a missing datum shows as a dashed *not declared* cell, never a zero — the honesty rule made visible. Each column also opens with an **at-a-glance verdict** — a health dot (green/amber/red) plus a sober phrase answering its question — the section's most urgent tile takes a severity-coloured **entry-point accent**, and a **since-last-read delta** (−N / +N) shows problems closed or opened versus the previous SNMP read (baseline in `localStorage` per project, never in the document). Red is reserved for *flying blind* (never synced), so a synced-but-imperfect project reads amber, not alarmist. Every row **drills down in place**: click it to list the devices or subnets behind it — LLDP/CDP neighbours resolve a bare chassis-MAC to the project device's name, free addresses count capacity on the **declared subnet prefix** whenever you declared one — a /16 is ~65,000 addresses, not 254 — with each row tagged *declared* / *undeclared*: networks you use but haven't declared surface as **undeclared** and the verdict nudges *N to declare* (a /24 is only the fallback when nothing is declared). What you write in the VLAN/subnet panel is treated as law — the addressing plan drives the numbers, and whatever falls outside a declared subnet is surfaced, never hidden. Free ports split into **In-rack / Outside-rack / By-speed** tabs — free of total per device, then the same free ports broken down by link speed, with the ones whose speed nobody declared counted apart (a port with no declared speed is not a slow port). **IP and MAC share one row**, because they are two halves of one network identity: the drill-down is the project's **ARP pairing** — device, address, MAC underneath — and on managed gear the MAC shown is the one its *interfaces* report, since that is where it lives. Composes existing engines only (`spare-ports`, `project-networks`, `hw-capabilities`) — no new measurement; the chosen view lives in `localStorage`, never in the project, so looking at the summary never marks it unsaved (`lib/overview.js`, pure + tests; `src/app-overview.js`). A **lens switch** at the top adds three opt-in full-width lenses beyond the summary: **Recoverability (DR)** — *"X of Y recoverable"* from config-backup freshness (≤30 days), hardware identity (make/model **or** serial, with firmware advisory), rack location, presence and declared HA — and **Security & Services** — *how exposed is management?*: encrypted SNMP (v3) vs cleartext (v1/v2c), how many devices use a **default community** (counted, the value never leaves the engine), and how many VLANs are marked *management* — and **Health**, the only lens that speaks about the present: *what problems are there right now?*, composing the telemetry devices already returned (RAM and disks from HOST-RESOURCES, printer consumables, UPS battery/runtime/load) through the same documented thresholds the AI assistant reads, with CPU load reported but never judged, a grey verdict when nothing was read, and a green one that degrades to amber once the oldest reading passes 6 hours. The **Conformance** column also gains an **IPAM conflicts** row (the same IP on two documented devices — VM IPs included — or two VLANs whose subnets overlap) and a **VLAN-names** row that reconciles the names read over SNMP against what you declared (fillable gaps and conflicts surfaced; the declared name is never overwritten). Under every lens the Overview closes with a quiet **"What I'm not looking at"** footnote that names the dimensions it does **not** judge — *not modelled* — WAN, L3 routing, spanning-tree, firewall/ACL, AAA/syslog/NTP, backup-restore proof, temperature and interface error counters (never polled), trunk symmetry (the per-side egress lists that would settle it are not read yet) — so the absence of an alarm is never read as *all clear*; the list is data in the engine (`blindSpots`), retired one line at a time as dimensions get modelled. A **staleness gate** carries the same honesty into time: after seven days with no Sync or Verify, a green verdict in **Conformance** and **Expansion** — the two that rest on measurement — degrades to amber and says *read N days ago*. It deliberately spares **Project**, which asks whether the *document* describes everything (a document does not age), and **Recoverability**, which already has its own 30-day backup clock. Tiles pack **3 per row while they fit**, dropping to 2 when the column narrows — labels and verdicts wrap rather than clip, because a truncated label is the name of the thing whose number you are reading. Each carries a vertical fill gauge
- **Wireless links (Packet-Tracer style)** — mark any connection as wireless and it's drawn as a **sine-wave** instead of a straight cable (in both floor map and topology), skips physical-cable validation, and is auto-suggested when one end is an AP. Wi-Fi-capable devices (AP / router / firewall, per-device toggle in *Network & Access*) expose a **radio port**: a virtual connector that hosts many wireless clients without consuming physical ports and stays out of the *Free ports* report — drag a client onto the radio badge to associate it. Any other Wi-Fi-capable device (a PC/server hotspot) can opt into **"AP mode"** to broadcast an SSID **without changing its type** (capability, not role) — the "Wi-Fi" and "AP mode" toggles sit side by side in *Network & Access*. A **WLAN** toggle in the topology legend hides all wireless connections at once
...
- **Overview page in the dossier** — one page, right after the floor plan, carrying the three questions and their verdicts — the same ones on screen, with the same **provenance dot** next to every number and its legend, and the *"What I'm not looking at"* line at the bottom. It is the executive summary the dossier was missing: the verdicts used to live only in the app while the PDF handed over raw data. It lists no devices (that is the asset register), so it stays **one page**, and it switches off like any other section. The words come from the app itself — the pure engine stays the only source of the numbers, the glue the only source of the words, the server just draws
- **Recoverability (DR) section in the PDF** — an optional page, one row per managed device, that is the runbook to **rebuild from**: **where the config backup lives** (the pointer + method, never the config or credentials), the **date/time** it was last taken, the **serial and firmware** to procure and reflash, the **lifecycle** (declared warranty and end-of-life dates, summed up worst-first: EOL · out of warranty · expiring within 90 days) and the **rack** it goes back into — under a header verdict *"X/Y recoverable · N without a backup · N without a location · N end-of-life"* (recoverable = fresh backup **and** known identity **and** known location, matching the Overview's DR lens). Built from the same **secret-free allowlist DTO** as the asset register; kept off-site, it survives the very LAN it rebuilds
...
- **Wireless association discovery** — the Sync draws **over-the-air associations** (radio↔radio, not cables) from two SNMP signals: the **bridge FDB** (a client MAC learned on a device's radio interface — IF-MIB `ifType ieee80211`, or a vendor-neutral interface-name fallback) and the **L3 neighbour table** (`ipNetToMedia` / `ipNetToPhysical` — a MAC whose ARP/ND entry is on a radio interface), which is universally implemented and covers devices that don't expose the bridge FDB (including PC/SoftAP hotspots and L3 APs/routers). The discovered client gets a station radio and attaches to the AP's radio, with the **BSS/SSID chosen by VLAN match** (ambiguous → left for you to pick). Works on the common **all-in-one** box (router + AP + switch in one). The L3 path fires only for devices broadcasting an SSID, so a station is never mistaken for an AP. Manual-first, with symmetric pruning; pure engine `lib/wifi-assoc.js`
