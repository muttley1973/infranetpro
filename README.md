...
- **Hypervisors & home‑lab VMs** — model VMs under a host (`node.vms[]`) on hypervisor and home‑lab devices. A VM that shows up on the network as a loose tile (a MAC *or* an IP is enough — SNMP‑monitored devices on another subnet have no MAC) can be **absorbed into its host** by dragging it onto the *Virtual machines* section of the host's Properties (the whole section is the drop area; the dashed invite sits above the list): it inherits the tile's name/IP/MAC (when known), so it stops being flagged as undocumented. Absorb fires only on release **inside** the section; releasing elsewhere repositions the device (or snaps it back, with a toast explaining an ineligible drop). Undoable
