...
- **LACP mode & cross-end coherence** — each LAG group has a **mode** (LACP active / passive / static "on"), set by hand and stored in `state.lagModes`. InfraNet resolves the peer LAG from the cabling and warns on the two classic failures: **both ends passive** (neither starts negotiation → bundle never forms) and **LACP vs static** (incompatible ends). The mode also flows into the AI context; `lib/lag-audit.js` `checkLagPair`. SNMP auto-derivation of active/passive is future work
