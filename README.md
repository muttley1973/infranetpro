...
> 📰 **What's new:** see [CHANGELOG.md](CHANGELOG.md) — latest: **Discover names Windows PCs, and Stealth randomizes its scan order** — a Windows box (no SNMP, silent mDNS) is now named from a single NetBIOS **NBSTAT** query sent directly over UDP 137 (~40 ms, cross‑platform — the `nbtstat` CLI is 10–30 s+ on hosts with many virtual NICs) on the Normal/Safe cadences (off on Stealth), and the host running InfraNet from `os.hostname()`; and Stealth no longer sweeps IPs in sequence — it shuffles the target order (a sequential sweep is a scan signature just like a fixed interval) on both the ping sweep and the SNMP phase, verified end‑to‑end on the real route; earlier: **the Verifica report no longer repeats itself** (the *Non verificabili* devices are folded into the *Reti del progetto* section — nested under the unreachable subnet, with a per-network *presence* badge — instead of a duplicate device bucket) and the **PDF asset register** now lists IT assets only, excluding structural cabling (wall ports and electrical panels); earlier: **closed‑port devices finally identify themselves** — an opt‑in **mDNS / SSDP / ONVIF** listen pass makes phones, smart TVs, smart appliances and IP cameras that answer nothing on TCP or SNMP announce their own **type and model** (a Chromecast, a "Samsung QN90A", a "LG ThinQ Washer", a Reolink camera named from ONVIF `GetDeviceInformation`, an SNMP switch's exact model from the standard ENTITY‑MIB), and the Discover *Name* column shows that identifier — never an opaque mDNS UUID; the announced type is a measured, vendor‑neutral signal (link‑local only, opt‑in, SSRF‑guarded), and any personal name (a UPnP `friendlyName`) is used once and never re‑exposed; earlier: **a whole‑project Sync no longer hangs on one slow or half‑answering SNMP device** (the SNMP walk now honours a wall‑clock deadline and the topology phase runs in parallel batches, so a firewall that answers `sysName` but times out on every table walk can't stall the reconstruction — measured **70 s → ~18 s** on the real device), **a multi‑fabric project is no longer falsely greyed** (the presence audit scopes FDB visibility to the subnets the bridge tables *actually* span, so a real LAN behind a router that the lab switches can't see is reported *unverified* rather than *absent*), a new **"Project networks" panel in the Verifica report** (it derives every `/24` from your devices **and DHCP leases** and flags each one *covered / blocked / open* with a one‑click **"Discover network"** — e.g. it names the SNMPv3 switch that's missing credentials), and a big lift in **device‑type classification precision** — **one authoritative classifier** (the Discover UI now defers to the server engine instead of a thinner client copy), the **vendor company name never decides the type** (a "Gateway Inc." PC is no longer a router, an org named "SWITCH" no longer a switch — vendor is identity, never a type keyword, matching nmap/Fingerbank/netdisco), and a **Windows host typed by behaviour, not NIC vendor** (SMB shares + no print ports → *pc*, beating a printer‑vendor MAC); earlier: **the topology view is dramatically faster on large maps** (the pure line builder gained O(1) index maps — up to **~28×** — and the SVG overlay render is now batched into a `DocumentFragment` with an O(1) tile index, killing the per‑line layout thrashing — **~100×** on the render pattern; identical output either way), **the LLDP/CDP crawl polls devices in parallel** (a level‑synchronised BFS with a bounded, Raspberry‑friendly worker pool `CRAWL_POOL` — deterministic, same device set as sequential — so a slow/unreachable device no longer stalls the whole crawl), an **opt‑in stealth scan cadence** (a 3‑level *Normal / Safe / Stealth* selector in Discover: stealth serialises the base sweep and jitters each probe — nmap's polite/T2 profile — so documenting a network doesn't trip its own IDS/IPS, while the parallel deep polling of already‑known devices isn't a scan signature), the **SNMP walk‑retry scoped to just the forwarding‑table group** (the macsuck cure kept, but slow/dead devices stop paying ~3× their timeout on every walk) and a **fix for the crawl aborting early** (SSE client‑disconnect is detected on the response, not the request); earlier: **macsuck no longer drops port badges under crawl load** (SNMP walks retry a timed-out GETBULK with `max-repetitions` halved — the netdisco bulkwalk-retry strategy — so a busy small-business switch degrades in speed instead of returning a truncated forwarding table and leaving MACs unplaced), **vendor recognition now backed by the full IANA + IEEE registries** (an SNMP `sysObjectID` or a MAC now resolves the manufacturer for ~any registered vendor — `data/pen-db.json` ~66k IANA enterprise numbers + `data/oui-db.json` ~57k IEEE OUIs, refreshed with `npm run update-pen` / `update-oui` — so Arista and the long tail no longer show a blank vendor; validated live on a lab Arista), **duplicate phantoms collapsed** (the same MAC seen at two IPs — a phone caught mid DHCP‑renewal — now shows once, the rest greyed *Inactive*) and **patient web fingerprinting in deep‑scan** (slow embedded web UIs on UPSs/NAS/switches get more time to return their title — feeding vendor+type — while the everyday scan stays fast); **no more phantom low‑confidence IPs in Scopri** (dead/stale ARP rows are gone — ARP‑SNMP is now off‑segment‑only, since on‑segment the local ARP is authoritative, and the local ARP read is Windows‑state‑aware; verified in‑browser: a `/24` scan dropped from 7 phantoms to 0) plus a **live crawl heartbeat** (the Scansiona button stays "Expanding…" with live progress during the ~30s LLDP/CDP phase); also **DHCP leases as a discovery source** (leased‑but‑sleeping phones/tablets/IoT appear as observed candidates with hostname + vendor, even with **zero SNMP**) and **`macsuck` device location** (the LLDP/CDP crawl places each discovered MAC on its switch **access port** — a direct edge like `SW‑CORE · Gi0/5`, or *"behind"* an AP/uplink — validated live on a Zyxel GS1900); also **a faster subnet scan** (single‑ping sweep + ARP‑authoritative liveness, ~3× faster with no more `/24` timeouts, and ICMP‑filtered hosts now appear), **a vendor for BYOD devices** (the brand is derived from the mDNS/hostname the device announces — `iPhone‑…`→Apple, `Galaxy…`→Samsung — with an honest "Private · random MAC" label otherwise), **confidence‑gated pre‑selection** (ping‑only phantoms are no longer auto‑checked) and a **de‑duped, fixed‑layout Scopri table**; earlier: **off-segment discovery via SNMP ARP tables** (a host a switch/router has in its `ipNetToMediaTable` is proposed even when it answers no ping/SNMP/LLDP — the standard `arpnip` method, refined with DHCP leases), **no false SNMPv3 label** on non-SNMP hosts (a real remote engineID is required), plus multivendor discovery hardening (ifName‑anchored SNMP port mapping, `vmVlan` access‑VLAN fallback, LLDP‑neighbour port alignment, crawl vendor preserved), AI assistant (advisory, bring‑your‑own‑key), per‑VLAN IPAM occupancy, DHCP lease import, REST API v1.
...
- **Hypervisors & home‑lab VMs** — model VMs under a host (`node.vms[]`) on hypervisor and home‑lab devices. A VM that shows up on the network as a loose tile (its own MAC/IP) can be **absorbed into its host** by dragging it onto the host's *Import VM* drop‑zone in Properties: it inherits the tile's name/IP/MAC and its MAC joins the documentation set, so it stops being flagged as undocumented. Absorb fires only on release **inside** the drop‑zone; releasing elsewhere repositions the device (or snaps it back). Undoable
...
- **Wireless properties (documentation-grade, documentation-grade hybrid)** — the AP's radio holds **SSID · band (2.4/5/6 GHz) · channel (grouped by UNII sub-band, DFS-marked, with Auto) · security (Open/WPA2/WPA3/OWE) · 802.11 standard (Wi-Fi 4…7)**, validated with educational warnings (channel↔band, 6 GHz needs WPA3/OWE, standard↔band, open network); the wireless **association** inherits those read-only and carries its own **RSSI / distance**. Pure tested logic in `lib/wifi-spec.js`
...
- **draw.io (diagrams.net) rack export** — export any rack to a native `.drawio` diagram (mxGraph XML), one page per rack. Devices, ports and the SNMP status stripe become *real editable cells* inside draw.io's own numbered rack container (they snap to their U slot), coloured by link status — not a pasted image. Custom device skins are honoured: the faceplate is embedded as a background image with a native, status-coloured port overlay (real positions recovered from the artwork). Opens clean and fully editable in diagrams.net
...
**Design principles:**
- **Minimal-tooling frontend** — historically zero-build (plain static assets loaded directly). Now migrating to explicit ES modules bundled by **esbuild** (`npm run build` → `dist/app.bundle.js`) to remove the implicit global coupling and `typeof` guards of the glue layer; the pure logic in `lib/*.js` stays UMD-lite (globals in the browser, `require()`-able in tests, imported as-is by esbuild). **The strangler migration is complete** (merged to `main`): all glue **and** the nucleus `app.js` are ESM modules in `src/`; only the pure `lib/*.js` and `export.js` stay classic *by design*. Retiring the transitional `window` bridge is now underway on two tracked axes — `win.*` reads → real `import` (a monotonic ratchet, mutable state behind `src/store.js`) and inline handlers (`onclick`/`onchange`/`oninput`/`onfocus`/`onkeydown`) → event delegation (`src/app-delegation.js`; `data-act`/`data-change`/`data-input`/`data-focus`/`data-keydown`, one delegated listener per event type — which also catches events from templates rendered at runtime); see [Architecture §10](ARCHITECTURE.md). The static change/input/focus/keydown handlers are done (only the classic `export.js` panel remains); ~55 static `onclick` handlers and ~535 handlers in dynamically-rendered templates are still to migrate. CSS is modularized in `styles/` (partials + design tokens, see `styles/README.md`).
...
> **Design invariant — vendor identity ≠ device type.** The engine keeps the two
> axes strictly separate, exactly as nmap, Fingerbank and netdisco/SNMP::Info do:
> the vendor (from a MAC OUI or an SNMP `sysObjectID`'s IANA enterprise number) is
> **identity/branding only**, and its *company name* is never keyword‑matched for
> the generic type nouns `gateway|switch|router|firewall` — otherwise a real
> "Gateway Inc." PC or an org literally named "SWITCH" would be mistyped. Type
> comes from behaviour/structure: the curated sysObjectID→model map, the
> `sysServices` OSI‑layer bits, product tokens in `sysDescr`, TCP service probes,
> and SMB/NetBIOS role signals (a host with SMB shares and no print ports is a
> *computer*, not a printer, regardless of its NIC vendor). When signals contradict,
...
> tablets) vs `pc` (a desktop OS). An opt-in **mDNS/SSDP listen** pass extends this to *closed-port* devices
> that answer nothing on TCP/SNMP: they announce their services in multicast, and the
> advertised service *is* the type (`_ipp`=printer, `_googlecast`/UPnP MediaRenderer=tv,
> `_hap`=iot, `_apple-mobdev2`=mobile), measured and vendor-neutral but link-local (same
> subnet only). When a device is known *only* by such an inference
...
- [x] **DHCP lease import — cross‑VLAN reality (June 2026)**: paste/load a DHCP lease table (ISC dhcpd / dnsmasq / Kea CSV / generic; pfSense / OPNsense / MikroTik / Synology / Windows) to feed the documentation check with authoritative MAC ↔ IP across **all VLANs** — behind an L3 firewall where ARP is blind. Multi‑server **persisted sources** merged by MAC (freshest wins); treated as an identity map, not a liveness probe (no false *absent*, staleness filtered, manual IP pins respected). Pure parser `lib/dhcp-lease.js`; the file/paste path is built‑in, live vendor pull is a separate optional driver pack. **Endpoint/BYOD transparency** groups user‑device noise with a plain‑language *"why hidden"* + reveal toggle.
- [x] **REST API v1 + Ansible dynamic inventory (June 2026)**: a versioned, **read‑only** `/api/v1/*` surface for external consumers (Ansible, dashboards, wikis, automation) — **bearer‑token** auth (no browser session), **sanitized** output only (never SNMP communities), with endpoints for projects, full inventory, device list, **Ansible dynamic inventory** and an OpenAPI 3.0 description. Token management UI under **Users and access → API tokens** (minted once, prefix‑masked list + revoke; token auth mounted *before* the session guard). `integrations/ansible/` ships a stdlib‑only `infranet_inventory.py` (`--list` / `--host`) that turns every IP device into a host grouped by `type_*` / `vlan_*` / `rack_*` / `brand_*` — InfraNet stays the source of truth, Ansible executes. Validated end‑to‑end on a real PNETLab lab. See the [REST API (v1)](#rest-api-v1) section.
...
- [x] **Floor-plan polish (June 2026)**: per-project **map opacity** control (slider in *Floor properties → Floor plan image*, 5–100%, stored as `bgImageOpacity`, overrides the global 0.4 so a clean vector plan can be shown crisp); **login-page language switcher** (IT/EN inside the login card, shared with the app via `localStorage`); the **Project context panel is now fully translated** (it/en) like the rest of the chrome; the *Type* dropdown in **Discover devices** is now sorted alphabetically; transported-VLAN field uses the same font as the native-VLAN field.
...
- [x] **Project context (formerly “Floor plan context”) → accordion**: collapsible sections (Floor plan image / VLAN / Colori workspace / Etichette) uniformed with device props panel pattern. Default open: image + VLAN. *(Auto-poll SNMP and DHCP IP auto-renewal later moved out of this panel into the «Automazioni rete» header popover — see the top entry.)* Persisted in localStorage. Fix regression from session 17: `Importa mappa` button was only visible when `state.bgImage` existed → impossible to load a map on a fresh project. Now always-visible primary button (label switches to "Sostituisci mappa…" when one is loaded)
- [x] Vector PDF / SVG export
- [x] **draw.io (diagrams.net) rack export** (`lib/drawio-export.js`): native mxGraph rack diagram, one page per rack, devices/ports as editable cells in draw.io's own numbered rack container (snap-to-U), device names outside the rack so the face is all interfaces; geometry calibrated against diagrams.net by round-trip; zero embedded-image artefacts. **Activatable cable layer split per VLAN** — one named draw.io layer per VLAN toggled independently, cables as native edges coloured by VLAN, routed in per-cable vertical lanes + horizontal stagger so they never overlap
- [x] **Audit-ready asset register + bilingual PDF report**: optional per-device inventory page in the PDF (reuses the secret-free `nodeToDevice` allowlist DTO — name/type/brand/model/serial/IP/MAC/VLAN/rack), a "last revised" cover timestamp (project `updated_at`) for NIS2 / ISO 27001 A.5.9 / cyber-insurance "kept-up-to-date" evidence, and full it/en localization of every report page (technical terms unchanged); historical audit values stay in their recorded language (audit integrity)
...
- [x] Manual > automatic priority: user-edited `hostname`, `ip`, `integration.host` and device **type** are never overwritten by SNMP/discovery — on both the Sync **and** the discovery import onto an existing node (flag-protected: `hostnameManual`, `ipManual`, `integration.hostManual`, `typeManual`); a hand-set brand is likewise never clobbered by the OUI/SNMP vendor
...
**Discovery — next (from a 2026-07 competitive review of Auvik / netdisco / Lansweeper / device-type):** the industry model is *neighbour (CDP/LLDP) → `arpnip` (ARP/`ipNetToMediaTable`) → `macsuck` (FDB/port)*. We now cover neighbour + `arpnip` + `macsuck` + DHCP-as-source; the ordered next steps:
- [x] **DHCP leases as a discovery source** — imported leases surface leased-but-quiet hosts (sleeping Wi-Fi/IoT) as observed candidates with hostname + OUI vendor, even with zero SNMP; scored with a `dhcp` evidence, manual-first (not pre-selected)
- [x] **Switch-port mapping (FDB "macsuck")** — `locateMacsOnEdge` places each discovered MAC on its access port (min co-learned-MAC = direct edge; busy non-LAG port = *"behind"* an AP/unmanaged switch; LAG uplink = unplaced), shown as a **location badge** in Scopri. Validated live on a Zyxel GS1900 (5 wired devices exact-port, wireless behind the AP). ⚠️ needs a switch that exposes the FDB over SNMP (real Cisco/Aruba/HPE/Zyxel/Arista; the lab vIOS returns an empty bridge FDB). Next: **auto-draw the cable to the exact port on import** (the edge is already on the row)
- [ ] **IPv6 Neighbour Discovery** — walk the unified `ipNetToPhysicalTable` (RFC 4293, covers both IPv4 ARP and IPv6 ND) so the SNMP-ARP off-segment discovery also finds v6-only hosts. Natural extension of `buildArpCandidates`
- [ ] **Discovered-device de-duplication** — fold an ARP/FDB candidate into an already-discovered device when its MAC / sysName / chassis-id matches, so a router's SVI IP doesn't appear as a separate "observed" host; correlate multiple IPs to one node
- [ ] **Shadow/rogue device signal** — cross-reference SNMP-ARP-discovered hosts against the documentation: an IP+MAC a switch is actively forwarding to but that isn't in the map = an undocumented/shadow device, surfaced through the existing Drift *undocumented* bucket (security/audit angle, low cost given the classifier already in place)
- [ ] **Source-of-truth positioning** — keep discovery **propose-and-reconcile, never overwrite** (device-type's *"discovered ≠ intent"* model): the manual-first rule is the product thesis, and the differentiator vs the monitoring incumbents
...
| LAG consistency | `test/lag-audit.test.js` | `checkLagMembers` speed/VLAN mismatch across LAG member ports (unknowns ignored, dedup + sort); `checkLagPair` LACP cross-end mode coherence (both-passive / lacp-vs-static) |
