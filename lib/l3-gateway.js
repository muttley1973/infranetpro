...
(function (root, factory) {
  ...
  function buildL3Report(model) {
    ...
    //
    // ⚠️ Ma «dichiarate» va preso alla lettera, e la palette non lo prova. `vlans`
    // arriva da `vlanColors`, che si riempie DA SOLO — con ogni VLAN letta via SNMP
    // e, su un progetto appena creato, con cinque VLAN di default (10/20/30/40/99)
    // che nessuno ha mai scritto. Elencarle qui le faceva comparire nel report con
    // la dicitura «VLAN dichiarata, nessuna rete»: dichiarata da nessuno. Un colore
    // non e' un atto. Un NOME lo e', e lo e' l'aver scelto CHI la instrada.
    // ⭐ E' la stessa avvertenza che l'igiene VLAN applica gia' (`vlansInUse` non si
    // confronta con `vlanColors`, o il controllo sarebbe verde ovunque): qui non era
    // arrivata. La palette resta intera — un prefisso che cita la VLAN 40 tiene il
    // suo colore — si stringe solo CHI si guadagna una riga propria.
    for (const v of vlans) {
      const vid = +v.vid;
      if (!Number.isFinite(vid) || withNet.has(vid)) continue;
      const meta = vlanMeta.get(vid) || { name: '', color: '' };
      const entry = ipamByVid[String(vid)] || {};
      if (!meta.name && !entry.gatewayNodeId) continue;
      const explicitId = entry.gatewayNodeId ? String(entry.gatewayNodeId) : '';
      const explicitNode = explicitId ? (nodesById.get(explicitId) || null) : null;
      const warnings = [];
      if (explicitId && !explicitNode) warnings.push('staleBinding');
