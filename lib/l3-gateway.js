...
(function (root, factory) {
  ...
  function findNodeByIp(nodes, ip) {
    if (!ip) return null;
    for (const n of (Array.isArray(nodes) ? nodes : [])) if (_sameIp(n.ip, ip)) return n;
    return null;
  }
  const _findNodeByIp = findNodeByIp;
  ...
  function buildL3Report(model) {
    ...
    for (const v of sorted) {
      ...
      // Il gateway non può essere il network o il broadcast address (eccetto
      // /31 e /32, dove gli estremi sono host validi — RFC 3021): un typo tipo
      // ".0"/".255" prima passava come "in subnet" senza alcun segnale.
      const gwReserved = !!(gateway && cidr && inSubnet && cidr.prefix <= 30 &&
        (gateway === _intToIp(cidr.network) || gateway === _intToIp(cidr.broadcast)));
    ...
    const totals = {
      ...
      reservedGateway: rows.filter(r => r.warnings.includes('gatewayReserved')).length,
