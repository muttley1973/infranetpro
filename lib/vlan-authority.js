...
(function (root, factory) {
   ...
   *
   * ⭐ E prima delle due misure c'e' una DICHIARAZIONE: `declaredRouted`, cioe'
   * la porta messa in modalita' L3 a mano (`ports[pid].mode === 'routed'`). Non
   * e' un terzo indizio da pesare insieme agli altri — e' la parola dell'utente,
   * e in questa app la parola dell'utente viene prima di ogni misura. Quindi
   * batte anche il VETO di `bridges`: se l'apparato dice «commuta» e tu dici
   * «instrada», resta scritto quello che hai detto tu e la contraddizione si
   * SEGNALA nel pannello — non si risolve di nascosto scegliendo un vincitore.
   * ⚠️ Senza questa riga «instradato» sarebbe misurabile e mai dichiarabile: un
   * progetto disegnato a mano non avrebbe MAI un cavo instradato, e quel cavo
   * finirebbe sul pavimento (la nativa di sito) affermando che commuta.
   *
   * @param {{ownsIp?:boolean, bridges?:boolean, declaredRouted?:boolean}} port
   * @returns {boolean}
   */
  function isRoutedPort(port) {
    const p = port || {};
    if (p.declaredRouted === true) return true;
