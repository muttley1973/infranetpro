...
(function (root, factory) {
  ...
  //
  // ⚠️ Le due regex NON sono globali, ed è giusto: `$1` e l'ancora `^` non
  // sopravvivono a un `/g` messo di corsa. Ma UNA passata toglieva la PRIMA
  // credenziale e lasciava le altre — `https://u:p@a/x https://u2:p2@b/y` usciva
  // ancora con un segreto dentro, e da lì andava su disco, nel DTO REST,
  // nell'inventory Ansible e nel dossier. Si ripete fino al PUNTO FISSO: ogni
  // sostituzione accorcia la stringa, quindi il ciclo termina da sé (il tetto è
  // solo una cintura). La proprietà che ne esce — ed è quella da provare, non la
  // sparizione del singolo caso — è: `_hasCreds(stripRefCreds(x))` è SEMPRE falso.
  function stripRefCreds(ref) {
    let s = String(ref == null ? '' : ref).trim();
    if (!s) return '';
    for (let i = 0; i < 32; i++) {
      const prima = s;
      s = s.replace(CRED_SCHEME_RE, '$1').replace(CRED_SCP_RE, '');
      if (s === prima) break;
    }
    return s;
  }
