...
(function (root, factory) {
  ...
  function effLinkVlans(args) {
    const a = args || {};
    // ⚠️ Questo `|| 1` NON è il pavimento del modello: il pavimento è la nativa di
    // SITO (dichiarabile, es. 99) e la risolve il CHIAMANTE, che la passa sempre
    // (`_getLinkTrunk` → `_getLinkVlan`, ultimo gradino `_siteNativeVlan()`). Qui è
    // solo l'ultima difesa di una lib pura contro un argomento invalido. Se lo stai
    // leggendo perché un cavo dice 1 su una sede con nativa 99, il difetto sta a
    // monte, in chi non ha passato la nativa — non si aggiusta un pavimento qui.
    ...
    // Override esplicito a TRUNK: resta trunk ANCHE se le VLAN effettive collassano a
    // una sola (nativa == trasportata). Vince sulla derivazione — prima il ramo scattava
    // solo con `carried` vuoto, quindi un trunk forzato con una VLAN sola scivolava ad
    // access ed era pure marcato `derived:true`, come se nessuno l'avesse deciso.
    if (a.manualMode === 'trunk') {
      return { mode: 'trunk', native, vlans: uniqSort([native, ...carried]), carried, derived: false };
    }
