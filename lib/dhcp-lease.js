...
// MAC e segmento non si ri-normalizzano qui: li definiscono lib/mac-class.js e
// lib/cidr.js (una sola definizione per concetto). Lettura a CALL-TIME nel
// browser, require in Node.
(function (root, factory) {
  const isNode = (typeof module !== 'undefined' && module.exports);
  const api = factory(isNode ? require('./mac-class.js') : root, isNode ? require('./cidr.js') : root);
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  if (typeof window !== 'undefined') Object.assign(window, api);
})(typeof self !== 'undefined' ? self : this, function (mc, cidr) {
  'use strict';
  ...
  // MAC → canonico UPPER colon-sep; '' se non è un MAC → record scartato (per il
  // match serve un MAC vero). Una sola definizione: lib/mac-class.
  function _normMac(v) { return mc.macFormat(v); }
  // Segmento del lease. Un file di lease non porta la maschera, quindi qui vale
  // l'assunzione /24 — che però NON si riscrive: la fa `segmentKey` (lib/cidr),
  // l'unico posto dove quell'assunzione è dichiarata (audit D2).
  function _net24(ip) { return cidr.segmentKey(ip); }
  ...
  // DRIFT-M4 (audit 2026-07-21): 'backup' (ISC failover: lease del pool del server di
  // backup, NON assegnato a un client) e 'abandoned' (conflitto rilevato) NON provano
  // una presenza viva → trattati come morti, come 'free'. Prima 'backup' cadeva nel ramo
  // Infinity di _leaseRank e non-stale in isLeaseStale → falso verde.
  const _DEAD_LEASE_STATES = { expired: 1, released: 1, declined: 1, free: 1, backup: 1, abandoned: 1 };
  ...
  // Unione di PIÙ fonti DHCP (una per server) in un set unico per-MAC. Usa lo STESSO
  // criterio di ranking (_leaseRank) del dedup intra-fonte: un lease infinito (riserva
  // statica, expiry null non-morto) batte qualunque datato, anche released/scaduto.
  // Un'unica autorità di ranking: evita che la fusione multi-fonte reintroduca il bug
  // "expiry?Date.parse:0" (S2.2) in cui la riserva statica perdeva contro un lease morto.
  function mergeLeaseSources(sources) {
    const all = [];
    for (const src of (Array.isArray(sources) ? sources : [])) {
      for (const l of ((src && Array.isArray(src.leases)) ? src.leases : [])) {
        if (l && l.mac) all.push(l);
      }
    }
    return _dedupeByMac(all);
  }
  ...
  function isLeaseStale(lease, now) {
    ...
    if (st === 'expired' || st === 'released' || st === 'declined' || st === 'free'
        || st === 'backup' || st === 'abandoned') return true;   // DRIFT-M4: stati non-vivi
    if (lease.expiry) {
  ...
  return { parseDhcpLeases, detectLeaseFormat, reconcileDhcpLeases, normalizeLeaseRecords, isLeaseStale, mergeLeaseSources };
});
