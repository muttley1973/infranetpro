...
(function (root, factory) {
  ...
  // lib/source-ref.js — l'identità d'origine in un campo. Serve ad appaiare gli
  // oggetti per RIFERIMENTO invece che per nome. Se non si risolve, ci si astiene
  // e si ricade sul nome: un confronto più debole, ma dichiarato.
  const _refApi = (typeof require === 'function') ? require('./source-ref') : null;
  ...
  function diffAgainstProject(imported, current) {
    ...
    // ── Rack ────────────────────────────────────────────────────────────────
    // ⭐ Per RIFERIMENTO, non per nome. Il nome era l'unica chiave disponibile
    // finché il rack non portava la propria identità d'origine, ed era la stessa
    // classe di confronto che questo progetto vieta ovunque: rinominare un rack in
    // NetBox lo faceva risultare «sparito e ricomparso», due righe per un oggetto
    // che non si era mosso.
    // ⚠️ Il ripiego sul nome resta, per i progetti importati PRIMA che il
    // riferimento esistesse — ma è un ripiego dichiarato (`byName` nel conteggio),
    // non un silenzio: chi legge deve sapere che quel confronto è più debole.
    const _rackKey = (r) => {
      const ref = _refApi && _refApi.refOf(r);
      return ref ? _refApi.refKey(ref) : '';
    };
    const rackIndex = (state) => {
      const byRef = new Map(), byName = new Map();
      for (const r of (state && Array.isArray(state.racks) ? state.racks : [])) {
        if (!r) continue;
        const k = _rackKey(r);
        // ⚠️ Un rack bifacciale è DUE rack InfraNet con lo stesso riferimento: si
        // tiene il primo, così il fronte e il retro non si contano come due
        // differenze per un oggetto che di là è uno solo.
        if (k) { if (!byRef.has(k)) byRef.set(k, r); }
        else if (_s(r.name)) byName.set(_s(r.name), r);
      }
      return { byRef, byName };
    };
    const dcimR = rackIndex(dcim), docR = rackIndex(doc);
    const racks = { added: [], removed: [], changed: [] };
    let racksByName = 0;
    const _cmpRack = (r, cur, etichetta) => {
      const a = _s(r.sizeU), b = _s(cur.sizeU);
      if (a && a !== b) racks.changed.push({ id: etichetta, name: etichetta, fields: [{ field: 'sizeU', dcim: a, doc: b }] });
    };
    for (const [k, r] of dcimR.byRef) {
      const nome = _s(r.name) || k;
      // Prima per riferimento; poi, solo se il documento è vecchio, per nome.
      const cur = docR.byRef.get(k) || docR.byName.get(_s(r.name));
      if (!cur) { racks.added.push({ id: nome, name: nome }); continue; }
      if (!docR.byRef.has(k)) racksByName++;
      _cmpRack(r, cur, nome);
    }
    for (const [nome, r] of dcimR.byName) {          // il DCIM non li produce, ma un chiamante può
      const cur = docR.byName.get(nome) || null;
      if (!cur) { racks.added.push({ id: nome, name: nome }); continue; }
      racksByName++;
      _cmpRack(r, cur, nome);
    }
    for (const [k, r] of docR.byRef) {
      if (!dcimR.byRef.has(k) && !dcimR.byName.has(_s(r.name))) racks.removed.push({ id: _s(r.name) || k, name: _s(r.name) || k });
    }
    // ⚠️ E NON si scorre `docR.byName`: un rack senza riferimento o l'hai disegnato
    // tu, o viene da un import precedente al riferimento — e i due casi sono
    // indistinguibili. Dichiararlo «sparito dal DCIM» accuserebbe l'utente del
    // proprio lavoro, che è la prima delle tre onestà in testa a questo file.
    // Restano contati come `handmade`, dove appartengono.
    const racksHandmade = docR.byName.size;
    ...
    return Object.assign({
      ...
      handmade: { devices: docSide.handmade, prefixes: docPfxSide.handmade, racks: racksHandmade },
      // Quanti rack si sono potuti appaiare solo per NOME: sono i progetti nati
      // prima che il rack portasse la propria identità. Non è un errore, è un
      // confronto più debole — e chi legge deve poterlo sapere invece di
      // ereditare una certezza che non c'è.
      weak: { racksByName },
