...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('./netnames'), require('./ipv6'), require('./port-id.js'), require('./cidr.js'));
  } else {
    Object.assign(root, factory(root, root, root, root)); // browser: già su window
  }
})(typeof self !== 'undefined' ? self : this, function (nn, v6, pidApi, cidr) {
  'use strict';
  pidApi = pidApi || (typeof globalThis !== 'undefined' ? globalThis : null);
  ...
  // Forma canonica di un IPv6 (RFC 5952), null se non e' un IPv6. Delega a lib/ipv6.js:
  // l'identita' di un indirizzo v6 ha UNA definizione. Il confronto per stringa RAW
  // faceva mancare l'aggancio quando la grafia del documento ("2001:DB8::1") differiva
  // da quella della misura ND/SNMP (canonica) — un router v6 restava scollegato.
  const _canon6 = (a) => {
    const f = v6 && v6.canonicalizeIpv6;
    return (typeof f === 'function' ? f(String(a == null ? '' : a).trim()) : null) || null;
  };
  ...
  function matchNodeByIdent(hostname, ip, nodes) {
    ...
    const ipC6 = _canon6(ip4);           // non-null solo se `ip` e' un IPv6 (mgmt v6 del vicino)
    ...
    for (const x of (nodes || [])) {
      ...
      if (sH && (nh.split('.')[0] === sH || nn_.split('.')[0] === sH)) {
        if (!byShortName) byShortName = x;                            // P3
        else if (x !== byShortName) shortAmbiguous = true;            // ≥2 nodi stesso short-name
      }
      if (ip4 && !byIp &&
          (x.ip === ip4 || (x.integration?.host || '') === ip4 ||
           (ipC6 && x.ip6 && _canon6(x.ip6) === ipC6)))               // P4: anche l'IPv6 di gestione, per identita'
        byIp = x;
    }
    // Short-name ambiguo (es. vicino "gw" con gw.siteA E gw.siteB documentati):
    // meglio NESSUN aggancio che il primo dell'array a confidenza LLDP 0.97 —
    // coerente con findPortByIfName, che sui tie restituisce null.
   ...
   */
  function buildPortIndex(ports, lagGroups, knownNodeIds) {
    const idx = {};
    // PERF: Set costruito UNA volta (non per porta) → nodeIdOfPort ha lookup O(1).
    const idSet = knownNodeIds instanceof Set ? knownNodeIds
      : (Array.isArray(knownNodeIds) ? new Set(knownNodeIds) : null);
    for (const [pid, pi] of Object.entries(ports || {})) {
      const nodeId = pidApi && typeof pidApi.nodeIdOfPort === 'function'
        ? pidApi.nodeIdOfPort(pid, idSet)
        : (() => { const cut = pid.lastIndexOf('-'); return cut > 0 ? pid.slice(0, cut) : null; })();
      if (!nodeId) continue;
  ...
  function buildMacIndex(nodes, ports) {
    const idx = {};
    const knownNodeIds = new Set((nodes || []).map(n => n && n.id).filter(Boolean));
    ...
    for (const [pid, pi] of Object.entries(ports || {})) {
      ...
      const nodeId = pidApi && typeof pidApi.nodeIdOfPort === 'function'
        ...
        : (() => { const cut = pid.lastIndexOf('-'); return cut > 0 ? pid.slice(0, cut) : ''; })();
      if (nodeId && nodeId !== pid) idx[mac] = nodeId;
    }
    return idx;
  }
   ...
   */
  function buildPortMacIndex(ports, knownNodeIds) {
    const idx = {};
    const idSet = knownNodeIds instanceof Set ? knownNodeIds
      : (Array.isArray(knownNodeIds) ? new Set(knownNodeIds) : null);
    for (const [pid, pi] of Object.entries(ports || {})) {
      ...
      const nodeId = pidApi && typeof pidApi.nodeIdOfPort === 'function'
        ? pidApi.nodeIdOfPort(pid, idSet)
        : (() => { const cut = pid.lastIndexOf('-'); return cut > 0 ? pid.slice(0, cut) : ''; })();
      if (!nodeId || nodeId === pid) continue;
      idx[mac] = { nodeId, portId: pid };
    }
    return idx;
  }
  ...
  function buildIpIndex(nodes) {
    ...
    for (const n of (nodes || [])) {
      ...
      const ip6 = String(n.ip6 || '').trim();
      if (ip1 && !idx[ip1]) idx[ip1] = n;
      if (ip2 && !idx[ip2]) idx[ip2] = n;
      if (ip6) {
        // Indicizza SIA la grafia documentata SIA la forma canonica: la misura ND arriva
        // canonica, il documento può essere espanso/maiuscolo — così l'aggancio v6 non
        // dipende da come è stato scritto l'indirizzo (identità, non stringa).
        if (!idx[ip6]) idx[ip6] = n;
        const c6 = _canon6(ip6);
        if (c6 && !idx[c6]) idx[c6] = n;
      }
    }
    return idx;
  }
  ...
  // Tipi che INOLTRANO traffico per altri device → candidabili come target di un
  // uplink (la uplink-resolution). Vendor-neutral: conta il TIPO, non il numero di
  // porte. Un NAS/server/PC/stampante — anche multi-porta (NIC teaming) — è un HOST
  // terminale, NON un apparato di rete: non deve creare falsa ambiguità né essere
  // scambiato per il device diretto di un uplink. `isLeafEndpointNode` (proxy sul
  // conteggio porte) sbagliava proprio qui: un NAS a 2 porte risultava "non-foglia".
  const FORWARDING_INFRA_TYPES = new Set([
    'switch', 'l3switch', 'coreswitch', 'router', 'firewall', 'gateway',
    'sdwan', 'vpncon', 'wlanctrl', 'ap', 'accesspoint', 'bridge', 'mediaconv', 'hypervisor',
  ]);
  function isForwardingInfraNode(node) {
    return !!node && FORWARDING_INFRA_TYPES.has(String((node && node.type) || '').toLowerCase());
  }
  ...
  function findPortByIfName(nodeId, ifName, portIndex) {
    ...
    for (const e of entries) {
      ...
      // Livello 3 — numeric-only (fallback debole). SALTATO per le query LAG:
      // un'interfaccia AGGREGATA (LAG1/Po1/ae1/Eth-Trunk1…) NON deve mai matchare
      // una porta FISICA solo perché condividono il numero (es. "LAG1" → porta
      // fisica 1, bug uplink reale). Un aggregato risolve solo a una porta che È
      // essa stessa quel LAG (Liv. 1/2) o via lagId/lagGroup (fallback sotto).
      // Vendor-neutral: q.lagToken è il token canonico di _ifNameMeta.
      if (q.numOnly && !q.lagToken) {
   ...
   *   Ogni vicino: { remoteDevice, remoteIP, remoteMac, localPort, remotePort,
   *                  remotePortMac, protocol }
   ...
   * @param {Object} [portMacIndex] - buildPortMacIndex(ports): MAC → { nodeId, portId }.
   *   Serve quando il vicino identifica la porta remota col suo MAC invece che col
   *   nome (LLDP portIdSubtype = macAddress): li' la porta e' LETTA, non dedotta.
   * @param {Object} [opts]
   * @param {Array}  [opts.unresolved] - array che raccoglie i vicini che NON hanno
   *   prodotto un candidato, con il MOTIVO. Un vicino scartato in silenzio è la
   *   peggiore delle uscite: il protocollo ci ha detto che due apparati sono
   *   attaccati, e noi non disegniamo niente senza spiegare perché. I motivi non
   *   sono intercambiabili — chiedono azioni opposte:
   *     · `no-identity`          il vicino non annuncia nulla che si possa cercare
   *       (né nome, né IP, né MAC): è una LACUNA nostra o un annuncio muto, e si
   *       chiude scrivendo codice, non scansionando;
   *     · `device-unknown`       l'identità si legge benissimo, ma nel documento
   *       quell'apparato non c'è: si chiude scansionandolo o aggiungendolo;
   *     · `local-port-unresolved` l'apparato remoto è noto, ma il nome della porta
   *       LOCALE non combacia con nessuna porta del nodo sorgente. È il sintomo di
   *       una porta che non sappiamo nominare (vedi lldpLocPortNum ≠ ifIndex).
   * @returns {CandidateSet}
   */
  function buildNeighborCandidates(srcNodeId, neighbors, nodes, portIndex, macIndex, portMacIndex, opts) {
    const cset = createCandidateSet();
    const mac  = macIndex || {};
    const pmac = portMacIndex || {};
    const unresolved = (opts && Array.isArray(opts.unresolved)) ? opts.unresolved : null;
    // Si riporta ciò che il vicino ha DETTO, non una nostra parafrasi: chi legge
    // deve poter risalire all'annuncio.
    const _txt = x => String(x == null ? '' : x).trim();
    const _skip = (nb, reason, remNodeId) => {
      if (!unresolved) return;
      unresolved.push({
        reason,
        // Quando l'apparato remoto è stato riconosciuto, il suo id viaggia col
        // motivo: due apparati che si annunciano SONO adiacenti anche se la porta
        // non si risolve, e con i due id quell'adiacenza si può disegnare.
        remoteNodeId:  _txt(remNodeId),
        protocol:      _txt(nb.protocol) || 'LLDP',
        srcNodeId:     _txt(srcNodeId),
        localPort:     _txt(nb.localPort),
        remoteDevice:  _txt(nb.remoteDevice),
        remoteIP:      _txt(nb.remoteIP),
        remoteMac:     _txt(nb.remoteMac),
        remotePortMac: _txt(nb.remotePortMac),
      });
    };
    const CONF_LLDP = 0.97;
    const CONF_CDP  = 0.90;
    const CONF_INFERRED_PORT = 0.82;     // porta remota DEDOTTA → sopra soglia auto (0.80), sotto trusted (0.90)
    const usedRemotePorts = new Set();   // evita collisioni sulla porta remota (multi-link/LAG)
    ...
    for (const nb of (neighbors || [])) {
      ...
      // 2-bis. Anche il MAC della PORTA remota identifica l'apparato: quella porta
      //        e' sua. Vale per i vicini che annunciano la porta col MAC e il
      //        chassis con un nome che nel documento non c'e'.
      if (!remNode) {
        const pm = _normMacKey(nb.remotePortMac);
        const hit = pm ? pmac[pm] : null;
        if (hit) remNode = nodes.find(n => n.id === hit.nodeId) || null;
      }
      if (!remNode) {
        // Due esiti diversi, e chiedono cose opposte: se il vicino non ha detto
        // NIENTE di cercabile è un buco nostro; se l'ha detto ed è il documento a
        // non conoscerlo, si scansiona. Confonderli nasconde entrambi.
        const haIdentita = !!(_txt(nb.remoteDevice) || _txt(nb.remoteIP) ||
                              _txt(nb.remoteMac) || _txt(nb.remotePortMac));
        _skip(nb, haIdentita ? 'device-unknown' : 'no-identity');
        continue;
      }
      ...
      // 4. Risolvi porta remota. Il chassis-id LLDP è spesso un MAC → la porta
      //    remota non è desumibile (findPortByIfName torna null). Ma il LINK è
      //    autorevole: i due apparati SONO adiacenti. Quindi si aggancia a una
      //    porta sensata del nodo remoto — la prima non ancora usata in questo set
      //    (gestisce multi-link/LAG). Prima era limitato ai nodi a PORTA SINGOLA:
      //    un router/switch documentato con più porte non veniva MAI cablato
      //    (candidato con dst null → scartato). Vendor-neutral.
      let rp = findPortByIfName(remNode.id, nb.remotePort, portIndex);
      let portInferred = false;   // porta remota DEDOTTA (non risolta dal protocollo)?
      // Il vicino puo' identificare la porta remota col suo MAC invece che col nome
      // (LLDP portIdSubtype = macAddress: e' cosi' che si presenta un MikroTik).
      // Quel MAC e' un'identita' esatta: se lo troviamo su una porta di QUESTO nodo
      // la porta e' letta dal protocollo, non indovinata — e resta autorevole.
      if (!rp) {
        const pm = _normMacKey(nb.remotePortMac);
        const hit = pm ? pmac[pm] : null;
        if (hit && hit.nodeId === remNode.id) rp = hit.portId;
      }
      if (!rp) {
        const remPorts = Number(remNode.ports !== undefined ? remNode.ports : 0) || 0;
        if (remPorts >= 1) {
          for (let i = 1; i <= remPorts; i++) {
            const cand = `${remNode.id}-${i}`;
            if (!usedRemotePorts.has(cand)) { rp = cand; break; }
          }
        } else {
          rp = `${remNode.id}-1`;   // n. porte sconosciuto → default alla prima
        }
        // Un device a porta SINGOLA non è ambiguo (una sola porta possibile) →
        // resta autorevole. Su un device MULTI-porta (o a conteggio ignoto) la
        // porta scelta è una NOSTRA inferenza, non un dato del protocollo.
        portInferred = (remPorts !== 1);
      }
      if (rp) usedRemotePorts.add(rp);

      // LLDP/CDP con porta remota ESATTA (o device a porta singola) = vicino
      // dichiarato → autorevole ('discovered'). Con porta remota DEDOTTA il link
      // è un'inferenza e va proposto "da confermare": protocollo NON-trusted +
      // confidenza sotto 0.90, così linkState() lo classifica 'ambiguous' e non
      // lo etichetta LLDP (non lo è: il protocollo NON ci ha dato la porta).
      let conf, proto;
      if (portInferred) {
        conf  = CONF_INFERRED_PORT;
        proto = 'INFERRED';
      } else {
        conf  = nb.protocol === 'LLDP' ? CONF_LLDP
              : nb.protocol === 'CDP'  ? CONF_CDP
              : 0.70;
        proto = nb.protocol || 'LLDP';
      }
      // ⚠️ `cset.add` scarta in silenzio ciò che ha una punta mancante. Qui la punta
      // che può mancare è la LOCALE: il nodo remoto a questo punto è noto e `rp` c'è
      // sempre (al peggio dedotta). Un `lp` nullo vuol dire che il nome di porta
      // arrivato dal protocollo non combacia con nessuna porta del nodo sorgente —
      // e senza questa riga il cavo spariva senza che nessuno se ne accorgesse.
      if (!lp) { _skip(nb, 'local-port-unresolved', remNode.id); continue; }
      cset.add(lp, rp, conf, proto);
    }
  ...
  function buildFdbCandidates(srcNodeId, fdbTable, arpTable, nodes, portIndex, portMacIndex, opts) {
    ...
    // Indice MAC → nodo DOCUMENTATO (per riconoscere un apparato infra su una porta)
    // e, per ogni interfaccia, l'insieme degli apparati INFRA documentati il cui MAC
    // è appreso lì. Serve alla "uplink resolution" più sotto.
    const nodesByMac = {};
    for (const n of (nodes || [])) {
      const m = _normMacKey(n && n.mac);
      if (m && !nodesByMac[m]) nodesByMac[m] = n;
    }
    const infraByIf = {};
    for (const [macRaw2, ifName2] of Object.entries(fdbTable || {})) {
      const n = nodesByMac[_normMacKey(macRaw2)];
      if (n && n.id !== srcNodeId && isForwardingInfraNode(n)) {
        const k = String(ifName2 || '');
        (infraByIf[k] = infraByIf[k] || new Set()).add(n.id);
      }
    }
    const uplinkSeen = new Set();
    ...
    for (const [macRaw, ifName] of Object.entries(fdbTable || {})) {
      ...
      // "UPLINK RESOLUTION" (niente LLDP): se il MAC appreso qui appartiene a un
      // APPARATO INFRA DOCUMENTATO (router/switch/AP/fw) ed è l'UNICO infra su
      // questa porta, allora QUELLO è il device direttamente collegato (il target
      // dell'uplink); gli altri MAC sono transito OLTRE di lui. Vale ANCHE su porte
      // affollate (>4 MAC), dove l'euristica shared-segment altrimenti si arrende:
      // un router documentato, trovato in scansione, con tanti MAC che gli passano
      // dietro, veniva lasciato scollegato. Dedotto dalla sola FDB + device noto.
      // Porta remota non desumibile dalla FDB → default alla prima (correggibile).
      const infraNode = nodesByMac[mac];
      if (infraNode && infraNode.id !== srcNodeId && isForwardingInfraNode(infraNode)) {
        const set = infraByIf[String(ifName || '')];
        if (set && set.size === 1) {                 // uno solo → non ambiguo
          if (!uplinkSeen.has(lp)) {
            uplinkSeen.add(lp);
            cset.add(lp, `${infraNode.id}-1`, 0.82, 'MAC-UPLINK');
          }
          continue;
        }
      }
  ...
  function buildArpCandidates(arpTable, opts) {
    ...
    // Reti del COLLECTOR (on-segment). `localPrefixes` = i CIDR veri delle sue
    // interfacce (`os.networkInterfaces()` la maschera ce l'ha: non c'è motivo di
    // indovinarla). `localSubnets` resta accettato per compatibilità ed è la
    // vecchia assunzione /24 — su una /22 o due /25 sbagliava il confine, e qui
    // il confine DECIDE se un host è morto o solo off-segment (audit D2).
    const localPrefixes = Array.isArray(o.localPrefixes) ? o.localPrefixes : null;
    const localSubnets = o.localSubnets || null; // Set<"a.b.c"> — legacy
    ...
    for (const macRaw of Object.keys(arpTable)) {
      ...
      // ON-SEGMENT: se l'IP e' in una subnet del COLLECTOR, la sua ARP LOCALE (netsh)
      // e' autorevole -> un IP on-segment non trovato dallo sweep locale e' MORTO, non
      // va resuscitato dall'ARP (spesso stantia) di un altro device (es. la ipNetToMedia
      // di un NAS/router pieno di voci dormienti). L'ARP-SNMP serve per gli host
      // OFF-SEGMENT (dietro un confine L3 che il collector non ARPa direttamente).
      if (localPrefixes) {
        // `segmentKey` con dei prefissi ritorna la loro chiave (contiene '/') solo
        // se uno CONTIENE l'indirizzo: è la stessa domanda, con la maschera vera.
        if (String(cidr.segmentKey(ip, localPrefixes)).indexOf('/') >= 0) continue;
      } else if (localSubnets) {
        const seg = cidr.segmentKey(ip);          // assunzione /24, dichiarata in lib/cidr
        if (seg && localSubnets.has(seg)) continue;
      }
      const mac = _normMacKey(macRaw);
  ...
  // Candidati "solo ND" da una ipNetToPhysicalTable IPv6 (ndTable: { mac -> [ip6…] }).
  // Gemello IPv6 di buildArpCandidates: un host nella neighbor cache IPv6 di uno
  // switch/router SNMP, non trovato via ping/SNMP/LLDP, va comunque proposto —
  // ip6 + MAC (→ vendor OUI) sono noti senza ICMP. Solo indirizzi-host ROUTABILI
  // (global/ULA): link-local/multicast/loopback/unspecified non sono un "IP" utile.
  // PURA e vendor-neutral. `v6` = helper lib/ipv6.js (Node: modulo require; browser:
  // window, letto a call-time — questa funzione è comunque server/test-only).
  // Ritorna [{ ip6, mac, viaFrom }] (mac normalizzato aa:bb:..).
  function buildNdCandidates(ndTable, opts) {
    const o = opts || {};
    const knownMacs = o.knownMacs || null;   // Set<normMac> già scoperti (evita doppioni)
    const knownIp6  = o.knownIp6  || null;   // Set<ip6> già noti
    const fromIp    = o.fromIp    || '';
    const out = [];
    if (!ndTable || typeof ndTable !== 'object') return out;
    const classify = (v6 && typeof v6.ipv6Class === 'function') ? v6.ipv6Class : null;
    for (const macRaw of Object.keys(ndTable)) {
      const mac = _normMacKey(macRaw);
      if (!_isRealUnicastMac(mac)) continue;
      if (knownMacs && knownMacs.has(mac)) continue;         // già scoperto altrove
      const addrs = Array.isArray(ndTable[macRaw]) ? ndTable[macRaw] : [];
      for (const ip6raw of addrs) {
        const ip6 = String(ip6raw || '').trim();
        if (!ip6) continue;
        const cls = classify ? classify(ip6) : 'global';
        if (cls !== 'global' && cls !== 'ula') continue;     // solo indirizzi-host routabili
        if (knownIp6 && knownIp6.has(ip6)) continue;
        out.push({ ip6, mac, viaFrom: fromIp });
      }
    }
    return out;
  }
  ...
  // ============================================================
  // suppressInferredDuplicates — un'adiacenza = un solo cavo (pure)
  // ============================================================
  /**
   * Fix "doppio cavo + porta fantasma": quando il vicino annuncia il port-id
   * come MAC, la vista di A produce (A-p1 ↔ B-guess) INFERRED 0.82 e la vista
   * di B (stesso Sync o successivo) produce (B-p3 ↔ A-p1) LLDP 0.97. Il dedup
   * per coppia porta-porta non li vede — stessa ADIACENZA, pair diversi →
   * due cavi su A-p1, uno verso una porta inventata.
   *
   * Regola fisica: una porta porta UN cavo. Un link AUTO non-trusted (porta
   * remota dedotta: INFERRED/MAC/…) è un duplicato se una delle sue porte ha
   * GIÀ un link trusted (manuale, o auto LLDP/CDP) verso lo STESSO nodo remoto.
   * I link manuali non vengono mai toccati (manual-first).
   *
   * @param {Array} links - state.links
   * @param {function(string):string} portNodeId - portId → nodeId
   * @returns {{links: Array, removed: Array}}
   */
  function suppressInferredDuplicates(links, portNodeId) {
    // "trusted" = manuale, oppure protocollo di vicinato dichiarato. `_isTrustedProto`
    // e' l'UNICA definizione (regge le label fuse 'LLDP+MAC'/'CDP+MAC'): riscriverla qui
    // come { LLDP, CDP } la faceva divergere — un link corroborato 'LLDP+MAC' non copriva
    // la sua porta e il doppione INFERRED sopravviveva proprio nel caso confermato.
    const isTrusted = (l) => !l.autoLinked || _isTrustedProto(l.protocol);
    const cover = Object.create(null);   // "pid\u0000nodoRemoto" → true
    for (const l of (links || [])) {
      if (!isTrusted(l)) continue;
      const sn = portNodeId(l.src), dn = portNodeId(l.dst);
      if (!sn || !dn) continue;
      cover[l.src + '\u0000' + dn] = true;
      cover[l.dst + '\u0000' + sn] = true;
    }
    const kept = [], removed = [];
    for (const l of (links || [])) {
      if (isTrusted(l)) { kept.push(l); continue; }
      const sn = portNodeId(l.src), dn = portNodeId(l.dst);
      const dup = !!(sn && dn && (cover[l.src + '\u0000' + dn] || cover[l.dst + '\u0000' + sn]));
      if (dup) removed.push(l); else kept.push(l);
    }
