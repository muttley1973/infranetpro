...
(function (root, factory) {
  ...
  function buildIpIndex(nodes) {
    ...
    for (const n of (nodes || [])) {
      ...
      if (ip6 && !idx[ip6]) idx[ip6] = n;   // host scoperto via ND aggancia il nodo per IPv6
    }
    return idx;
  }
  ...
  function suppressInferredDuplicates(links, portNodeId) {
    const TRUSTED = { LLDP: 1, CDP: 1 };
    const cover = Object.create(null);   // "pid\u0000nodoRemoto" → true
    for (const l of (links || [])) {
      const trusted = !l.autoLinked || TRUSTED[String(l.protocol || '').toUpperCase()];
      if (!trusted) continue;
    ...
    const kept = [], removed = [];
    for (const l of (links || [])) {
      if (!l.autoLinked || TRUSTED[String(l.protocol || '').toUpperCase()]) { kept.push(l); continue; }
      const sn = portNodeId(l.src), dn = portNodeId(l.dst);
