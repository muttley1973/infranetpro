...
(function (root, factory) {
  ...
  function validateCable(link, opts) {
    ...
    if (med === 'copper' && speed && COPPER_CAT_MAX[cat] && speed > COPPER_CAT_MAX[cat]) {
      ...
      const tail = rec
        ? `Per ${l.maxSpeed} a ${recReach}m serve ${rec}${cat === 'Cat6' && speed === 10000 ? ' (su Cat6 il 10G regge solo fino a ~55m)' : ''}.`
      ...
      out.push({ level: 'warn', code: 'speed-cat', title: 'Velocità oltre la categoria del cavo',
        why: `${l.maxSpeed} su ${cat}: la categoria garantisce fino a ${_mbpsLabel(COPPER_CAT_MAX[cat])} a ${catReach}m. ${tail}` });
    }
    ...
    if (med === 'copper' && hasLen && COPPER_CAT_MAXLEN[cat] && len > COPPER_CAT_MAXLEN[cat] && len <= 100) {
      const max = COPPER_CAT_MAXLEN[cat];
      out.push({ level: 'warn', code: 'cat-reach', title: `${cat} oltre ${max}m`,
        why: `${len}m su ${cat}: questa categoria è specificata solo per canali fino a ${max}m (25/40GBASE-T). Oltre, la classe non è garantita — per tratte lunghe usa Cat6A a 100m o passa alla fibra.` });
    }
