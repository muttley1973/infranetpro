...
(function (root, factory) {
  ...
  function validateCable(link, opts) {
    ...
    // 13) L'IP di un capo cade in una VLAN diversa da quella in cui risulta la
    //     porta. Il colore del cavo segue l'INDIRIZZO dell'endpoint (source
    //     'declared-ip'), ma la porta a cui e' cablato risulta in un'altra VLAN:
    //     o l'IP sta nella sottorete sbagliata, o la porta nella VLAN sbagliata.
    //     Come la regola 12, la DECISIONE arriva gia' fatta dalla glue
    //     (`opts.ipVlanMismatch`) — qui e' solo un reperto SOBRIO (warn): non si
    //     distingue una scelta voluta da una svista, e non si rifa' il calcolo
    //     della VLAN (una sola risposta, in lib/link-vlan-color.js).
    const _iv = opts.ipVlanMismatch;
    const _ivIp = _iv ? parseInt(_iv.ip, 10) : NaN;
    const _ivPort = _iv ? parseInt(_iv.port, 10) : NaN;
    if (Number.isFinite(_ivIp) && Number.isFinite(_ivPort) && _ivIp !== _ivPort) {
      out.push({ level: 'warn', code: 'ip-vlan-mismatch', params: { ip: _ivIp, port: _ivPort } });
    }
