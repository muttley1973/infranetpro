...
(function (root, factory) {
  ...
  // params }]: SOLO codici e dati, mai parole — le frasi (educative: il perché,
  // non solo il divieto) le compone il renderer via i18n, chiavi `wfi.<code>.t/.w`.
  function validateWifi(cfg) {
    ...
    if (band && channel != null) {
      ...
      if (valid.length && !valid.includes(channel)) {
        out.push({
          ...
          params: { channel, band, min: valid[0], max: valid[valid.length - 1] },
        });
      }
    }
    ...
    if (band === '6' && security && _securityIsWpa2OrOpen(security)) {
      out.push({ level: 'error', code: 'band6-security' });
    }

    if (security === 'open') {
      out.push({ level: 'warn', code: 'open-network' });
    }
    ...
    if (standard && band && !standardSupportsBand(standard, band)) {
      ...
      // `std`/`stdFull` sono NOMI DI STANDARD (Wi-Fi 6 · 802.11ax), non prosa:
      // restano dati, uguali in ogni lingua. Le bande arrivano come elenco già
      // formattato perché la lib conosce l'unità, non la frase che la circonda.
      out.push({
        level: 'warn', code: 'standard-band',
        params: {
          std: s ? s.label.replace(/ \(.*/, '') : standard,
          stdFull: s ? s.label : standard,
          band,
          bands: (s ? s.bands : []).map(b => b + ' GHz').join(' / '),
        },
      });
    }
