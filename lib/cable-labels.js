...
(function (root, factory) {
  ...
  // Il nome di una presa RADIO sta sul modello (`node.radios[i].label`, scritto a
  // mano o importato dal DCIM), non nel pid: la risoluzione e' UNA sola, in
  // lib/radio.js. Risolta alla chiamata perche' nel browser radio.js e' caricato
  // DOPO questo file. null = non e' una radio, o non ha nome.
  function _radioLabel(node, pid) {
    const api = (typeof module !== 'undefined' && module.exports) ? require('./radio.js') : root;
    const fn = api && api.radioLabelForPid;
    return (typeof fn === 'function') ? fn(node, pid) : null;
  }

  // Una radio si stampa col suo nome, senza la "P" delle porte numeriche.
  function _endpointName(node, pid) {
    const name = node && node.name ? node.name : '?';
    const radio = _radioLabel(node, pid);
    return radio ? `${name} ${radio}` : `${name} P${_portSuffix(pid)}`;
  }
