...
(function (root, factory) {
  ...
  function quoteDiUnaRiga(riga) {
    ...
    const nonConfermati = _n(c.macOrphan) + _n(c.unverified) + _n(c.ipChanged) + _n(c.identityDrift);
    const presenza = nodi > 0 ? _pct(nodi - nonConfermati, nodi) : null;

    // CECITÀ — di quella non-conferma, quanta è «non l'ho guardato» invece di
    // «non c'è». È il guardiano della regola ①: se sale, la presenza che sale
    // non vuol dire niente.
    const cecita = _pct(_n(c.unverified), nonConfermati);

    // CODA — elementi che aspettano una decisione umana (definizione del Drift).
    const coda = _actionable ? _actionable(c) : null;

    return { at: riga && riga.at, verify: riga && riga.verify, nodi, porte, presenza, cecita, coda, counts: c };
  }
  ...
  function trendVerifiche(righe, opts) {
    ...
    if (quote.length < minC) {
      perche.push('servono almeno ' + minC + ' Verifiche confrontabili: qui ce ne sono ' + quote.length
        + '. La quota di confermato matura con l\'uso, non al primo giro.');
      return out;
    }
    if (out.cecita.verdetto === 'peggiora') {
      perche.push('la parte di rete che NON si riesce a guardare è cresciuta: qualunque miglioramento '
        + 'delle altre curve potrebbe venire da lì, non dalla rete. Prima si recupera la copertura.');
      return out;
    }
    ...
    if (out.verdetto !== 'migliora') {
      ...
      if (dom) perche.push('la coda non cala: «' + dom.bucket + '» è il bucket che pesa di più ('
        + dom.totale + ' su ' + quote.length + ' Verifiche). È lì che sta il lavoro non deciso.');
      else perche.push('la coda è vuota: la stagnazione non è lavoro arretrato.');
    }
    if (rottura) perche.push('la serie è stata spezzata al ' + rottura.at + ': la rete è passata da '
      + rottura.da + ' a ' + rottura.a + ' nodi, e due reti diverse non si confrontano.');
    return out;
  }
