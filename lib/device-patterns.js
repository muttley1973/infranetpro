...
(function (root, factory) {
  ...
  // `junos` RIMOSSO (2026-07-22): e' un OS (identita'), non una funzione — un
  // vSRX/SRX che si dichiara "firewall" veniva tipizzato router. I router
  // Juniper veri restano coperti: il loro sysDescr dice "internet router"
  // (token `router`) e MX/PTX/ACX hanno CARRIER_ROUTER_RE dedicata (per modello).
  const ROUTER_WORDS_RE = /router|gateway|mikrotik|vyos|openwrt|edgerouter|zywall|usg|fritz|web-based configurator/;
  const NET_VENDOR_GW_RE = /zyxel|tp-link|netgear|d-link|mikrotik|ubiquiti|huawei|avm|fritz/;
  // `ethernet multi-environment` e' la stringa che le schede di rete JetDirect
  // scrivono da sole nel sysDescr: e' un PRODOTTO che si annuncia, non un
  // marchio. Mancava, e la regola chiedeva `hp` E `print` insieme — una
  // stampante il cui hostname non dice "OfficeJet" non veniva riconosciuta.
  const PRINTER_RE = /officejet|laserjet|deskjet|pagewide|designjet|colorlaserjet|printer|printserver|\bhp.*print|print.*hp|ethernet multi-?environment|epson|xerox|ricoh|kyocera|jetdirect|imagerunner|imageclass|ecosys|taskalfa|bizhub|lexmark|brother/;
  ...
  // `diskstation`/`rackstation` sono le LINEE Synology — cioe' quello che il NAS
  // scrive da solo nel sysDescr ("Linux DiskStation 4.4…"), mentre il nome del
  // vendor ci finisce solo se qualcuno ha chiamato cosi' l'host. Senza di loro il
  // riconoscimento dipendeva da quale delle due stringhe capitava nel testo.
  const NAS_RE = /\bnas\b|synology|sinology|diskstation|rackstation|qnap|lacie|truenas|freenas|netapp|readynas|buffalo|drobo|iomega|wd\s*my\s*cloud|seagate\s*nas|asustor|terramaster|openmediavault/;
  ...
  // PDU/UPS — LINEE DI PRODOTTO, non marche. Un marchio multi-prodotto nella
  // regola di un tipo fa sbagliare tutto il resto del suo catalogo:
  //  · `raritan` stava qui e faceva uscire PDU il Dominion KX, che e' un KVM
  //    over IP. Restano le sue linee vere (PX, Dominion PX);
  //  · `\bapc\b` stava in UPS_RE e faceva uscire UPS ogni rack PDU e ogni ATS
  //    APC. Le sue linee UPS dicono gia' "UPS" (Smart-UPS, Back-UPS); Symmetra
  //    e' l'unica che non lo dice, ed e' una linea, non la marca.
  // Chi resta e' mono-prodotto o e' una linea: servertech e geist fanno PDU da
  // rack, powerware e' la linea UPS Eaton, riello fa UPS.
  // Stessa cura per `cyberpower`, `liebert` e `vertiv`, che erano multi-prodotto
  // quanto APC: Liebert e' soprattutto raffreddamento di precisione, e un suo
  // CRAC usciva UPS. Al loro posto le LINEE: `gxt` (Liebert/Vertiv, la piu'
  // diffusa che non scrive "UPS" nel nome) e le sigle Liebert accoppiate al
  // marchio. CyberPower non ha una linea distintiva ma i suoi apparati dicono
  // "UPS": li prende il primo token. Cio' che nessuna di queste prende resta
  // coperto dal voto sysObjectID e dalla UPS-MIB (RFC 1628), che sono misure.
  const PDU_RE = /\bpdu\b|power.?distribution|\bpx[234]-?\d|dominion ?px|servertech|\bgeist\b/;
  const UPS_RE = /\bups\b|symmetra|powerware|riello|\bgxt[0-9]|liebert ?(?:ita|nx|exm|exs|exl|aps|psi|psa|trinergy)/;
  ...
  // SRX / MX-PTX-ACX — erano `JUNIPER_*_RE`, gli unici due rami della tabella
  // intestati a un vendor. Non perche' Juniper sia speciale: perche' qualcuno
  // le ha aggiunte per lui e per gli altri no. La regola vera e' il MODELLO —
  // "SRX345" dice firewall con o senza "Juniper Networks" davanti, ed e' cosi'
  // che funzionano tutte le altre righe di questo file. Via il ramo `juniper.*`,
  // che oltretutto era largo: `juniper.*mx` prendeva qualsiasi "mx" nel testo.
  // ⚠️ `vsrx` va nominato a parte: in "vsrx" NON c'e' confine di parola prima di
  // `srx` (`v` e `s` sono entrambi word char), quindi ne' `\bsrx\b` ne' `\bsrx[0-9]`
  // lo prendono — e il sysDescr Junos standard e' «vsrx internet router», dove la
  // parola `router` porta 80 punti e vince. Il vecchio ramo `juniper.*srx` lo
  // copriva per caso, come sotto-stringa. `vsrx` e' un nome di prodotto univoco
  // (il firewall virtuale Juniper), quindi sta da solo; `srx` nudo no — chiede una
  // parola-funzione accanto, perche' tre lettere sole capitano ovunque.
  const SRX_FIREWALL_RE = /\bv?srx[0-9]{3,4}\b|\bvsrx\b|\bsrx\b.{0,24}(?:firewall|services gateway|cluster)/;
  const CARRIER_ROUTER_RE = /\bmx[0-9]{3,4}\b|\bptx[0-9]{3,4}\b|\bacx[0-9]{3,4}\b/;
  ...
  // (vendor != tipo): raritan e' uscito da PDU_RE (fa anche KVM), epson resta in
  // PRINTER_RE perche' e' mono-prodotto in questo dominio, avocent/
  ...
  // Piattaforme di EMULAZIONE di rete: una classe, non un inventario. La lista
  // era `pnetlab|eve-ng|unetlab` — cioe' esattamente il banco su cui e' stata
  // scritta. GNS3, containerlab e Cisco Modeling Labs sono la stessa cosa e
  // uscivano «switch» dal ripiego per i responder SNMP non riconosciuti.
  const NET_EMULATOR_RE = /pnetlab|eve-ng|unetlab|gns3|containerlab|cisco modeling lab|\bvirl\b/;
  const SERVER_VIRT_RE = new RegExp('windows server|truenas scale|openstack|kubernetes|k8s|docker|' + NET_EMULATOR_RE.source);
  ...
  return {
    ...
    HYPERVISOR_RE, NET_EMULATOR_RE, SERVER_VIRT_RE, SERVER_LINUX_RE, APPLIANCE_RE, SMART_HOME_RE,
