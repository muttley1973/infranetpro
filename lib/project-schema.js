...
//
// ── Un contenitore `secret` non è un contenitore TUTTO segreto ────────────
// `node.integration` porta la credenziale SNMP, ma anche l'host, il driver e le
// misure dell'ultimo poll. Dire «secret» sul contenitore serve a dire «qui dentro
// c'è da svuotare qualcosa»; CHE COSA lo dice lo scope `integration`, in fondo a
// questo file. Prima non lo diceva nessuno: i due consumatori enumeravano tre nomi
// per conto loro, e un quarto campo segreto sarebbe uscito in chiaro lasciando le
// prove verdi, perché anche le prove enumeravano quei tre.
//
 ...
 *  @typedef {'state'|'node'|'spec'|'port'|'link'|'integration'} FieldScope */
...
(function () {
  ...
  const LINK = {
    ...
    // ⚠️ La lunghezza di un cavo ha DUE nomi, e finora solo uno aveva una classe.
    // `lengthM` è quello dei documenti e delle etichette (`lib/cable-labels.js`,
    // che lo preferisce e ripiega su `length`); `length` è quello che il pannello
    // cavo SCRIVE davvero (`data-lprop="length"`), quindi è quello che sta nei
    // progetti veri — ed è il nome che il guard sui progetti reali ha trovato
    // senza classe. Stesso fatto, stessa classe: la digita una persona.
    color: 'document', lengthM: 'document', length: 'document',
  };

  // ── integration: il sacchetto delle credenziali SNMP ─────────────────────
  // Lo STESSO contenitore, con gli STESSI nomi, vive in tre posti: `node.integration`,
  // `vm.integration` e il legacy `vm.snmp`. Non è una somiglianza da verificare: lo
  // dichiara `src/app-hypervisor.js` in testa a `updateVmIntegration` («STESSO
  // contenitore e STESSI nomi campo dei device»), ed è il motivo per cui qui c'è una
  // tabella sola invece di tre.
  //
  // ── Perché questo scope esiste ─────────────────────────────────────────
  // `node.integration` e `node.snmp` erano classificati `secret` PER INTERO, e i due
  // consumatori di quella classe svuotavano tre nomi scritti a mano, in due copie
  // (`lib/project-format.js` e `server/routes/projects.js`). Ma il contenitore non è
  // tutto un segreto: dentro ci stanno anche la configurazione che digita una persona
  // e le misure che scrive il poll. Dire «tutto segreto» e poi svuotarne tre è la
  // variante peggiore del bug-classe di questo progetto — un lato DERIVA, l'altro
  // ELENCA — e non si guasta rumorosamente: **un quarto campo segreto lascerebbe la
  // suite verde**, perché anche le prove enumeravano gli stessi tre nomi. Qui i nomi
  // si dichiarano una volta; i consumatori (e le prove) li CHIEDONO.
  //
  // ⚠️ **Di questo scope oggi si consuma solo la classe `secret`.** Le misure qui
  // dentro escono ancora nell'export, perché nessuno chiama `_applyFieldClasses` su
  // questo scope. Farlo cambierebbe cosa esce da un export, che è una decisione da
  // prendere apposta — non l'effetto collaterale di una tabella. Sono classificate
  // lo stesso, perché il giorno che si decide la risposta è già scritta.
  //
  // ⚠️ Il censimento di questa tabella è una MISURA, presa il 2026-08-31 su 17
  // progetti reali (191 sacchetti) — non una deduzione dal codice che ci scrive. La
  // differenza l'ha pagata subito: `lastError` sta in tre sacchetti veri e NESSUNA
  // riga del prodotto lo scrive o lo legge (lo lasciano i generatori di demo in
  // `_local/`). Dedotto dal codice, sarebbe rimasto senza classe.
  /** @type {Record<string, FieldClass>} */
  const INTEGRATION = {
    // ── i segreti ──
    // Quali siano non è un'opinione: il pannello Integrazione li marca
    // `type="password"` (`src/app-properties-node.js`), e sono esattamente questi
    // tre. `v3user` e `v3context` sono campi di testo normali — identificano, non
    // autenticano — e restano documento.
    community: 'secret', v3authPass: 'secret', v3privPass: 'secret',
    // ── la configurazione: la digita una persona, quindi è documento ──
    driver: 'document', host: 'document', port: 'document', timeout: 'document',
    v3user: 'document', v3authProto: 'document', v3privProto: 'document',
    v3secLevel: 'document', v3context: 'document',
    // gemello di `nameManual`/`ipManual`: dice che l'host l'ha deciso una persona,
    // e il Drift lo legge per non riscriverglielo sotto (src/app-drift.js).
    hostManual: 'document',
    // ── le misure: le scrive il poll SNMP (src/app-snmp.js) ──
    lags: 'measure', vlans: 'measure', vlanNames: 'measure',
    inventory: 'measure', system: 'measure', printer: 'measure',
    hostResources: 'measure', lastPoll: 'measure',
    // letto da `lib/correlate.js` come uno dei MAC misurati del nodo; nel pannello
    // non c'è un campo che lo scriva.
    mac: 'measure',
    // residuo senza scrittore né lettore nel prodotto (vedi il censimento qui
    // sopra): resta classificato per ciò che PRETENDE di essere, un errore di poll.
    lastError: 'measure',
  };
  ...
  /** @type {FieldScope[]} */
  const FIELD_SCOPES = ['state', 'node', 'spec', 'port', 'link', 'integration'];
  /** @type {FieldClass[]} */
