...
(function (root, factory) {
    ...
    // Nome porta qualificato per un device in stack. La forma NON e' piu' cablata
    // sulla convenzione di un vendore: si usa quella MISURATA sulle interfacce
    // (detectStackFromInterfaces la riconosce gia' per quattro famiglie e la
    // registra in spec.stackPortFormat/stackPortSlot quando accetti lo hint).
    //   Standalone, port 24                    -> "24"
    //   Formato misurato M/S/P, membro 2       -> "2/0/24"   (S = slot MISURATO)
    //   Formato SCONOSCIUTO (stack a mano)     -> "2/24"
    // L'ultimo caso e' il punto: senza una misura non si sa quale slot scriva quel
    // vendore, e il vecchio `<m>/0/<p>` fisso — lo schema fixed-config Cisco —
    // inventava un pezzo di nome per TUTTI gli apparati. Membro e porta li sappiamo
    // davvero; lo slot no, e quindi non si scrive. (ADR ③: niente convenzioni di
    // naming di un apparato specifico nel codice condiviso.)
    function getQualifiedPortName(node, portNum) {
        ...
        const fmt = String(spec.stackPortFormat || '').trim();
        if (!fmt) return `${mid}/${n}`;
        const slot = Number.isFinite(spec.stackPortSlot) ? spec.stackPortSlot : 0;
        return `${mid}/${slot}/${n}`;
    }
    ...
    function detectStackFromInterfaces(interfaceNames) {
        const empty = { stackDetected: false, memberIds: [], suggestedFormat: null, suggestedSlot: null, sampleNames: [] };
        ...
        // Lo SLOT misurato, non assunto: `getQualifiedPortName` lo usa al posto
        // dello «0» che prima era cablato (convenzione fixed-config Cisco).
        ...
        for (const rawName of interfaceNames) {
            ...
            if (member !== null && Number.isFinite(member) && member >= 1) {
                ...
                if (!detectedFormat) { detectedFormat = format; detectedSlot = Number.isFinite(slot) ? slot : null; }
            }
        }
        ...
        return { stackDetected, memberIds, suggestedFormat: detectedFormat, suggestedSlot: detectedSlot, sampleNames };
    }

    return {
        isInStack,
        // ⚠️ PUBBLICI apposta. `isInStack` accetta due forme — `spec.stackId` (quella
        // che l'app scrive) e `node.stackId` piatto (che un progetto importato o
        // scritto a mano può avere) — e chi la interroga poi deve LEGGERE quel valore.
        // Finché questi due restavano privati, chi chiamava se lo rileggeva da solo,
        // e chi lo rileggeva guardando solo dentro `spec` andava in errore proprio
        // sui nodi che la guardia aveva appena dichiarato in stack. Una guardia e una
        // lettura che non concordano sono peggio di nessuna guardia: fanno passare
        // esattamente i casi che dovrebbero fermare.
        stackIdOf: _stackId,
        stackMemberIdOf: _stackMemberId,
        getStackMembers,
        getStackMaster,
        getNextMemberId,
        getAllStackIds,
