...
(function (root, factory) {
  ...
  //   · phy0-ap0 / phy0-sta0 / phy0-mesh0 (OpenWrt attuale) · wlx<mac> (chiavetta USB).
  ...
  //
  // ⚠️ I DUE aggiunti il 01/09 erano falsi negativi VERI, misurati su nomi di fabbrica:
  //   • `phy0-ap0` è il naming di serie di OpenWrt recente — e OpenWrt è dichiarato
  //     supportato in COMPATIBILITY.md. ⚠️ Su Linux una radio in modalità AP si presenta
  //     al kernel come ETHERNET (net-snmp riporta ifType=6, non 71), quindi lì il nome è
  //     l'unico canale: non riconoscerlo vuol dire non vedere NIENTE, in silenzio.
  //   • `wlx<mac>` è il nome predicibile di systemd per una chiavetta USB — cioè proprio
  //     lo scenario SoftAP economico. La controparte cablata è `enx`, quindi niente collisioni.
  // ⛔ E TRE nomi restano fuori DI PROPOSITO, perché non trasportano mai un client:
  //   • `run0`/`iwm0`/`rtwn0` sono nomi di DRIVER FreeBSD: lì la rete non ci gira sopra, si
  //     clona un `wlan0` (`ifconfig wlan0 create wlandev ath0`) ed è quello a portare il
  //     traffico — già coperto. Aggiungerli allargherebbe la regex a interfacce vuote.
  //   • `mon0` è modalità MONITOR: cattura pacchetti, non ha associati. Riconoscerla
  //     sarebbe semanticamente falso.
  const WIFI_NAME_RE = /(?:^|[^a-z0-9])(?:wlan|wifi|wi-fi|wlp|ath|rai?|apcli|wds|dot11|802\.?11|wl)\d|(?:^|[^a-z])(?:wireless|wlan|wifi|wi-fi|radio)(?:\d|[^a-z0-9]|$)|(?:^|[^a-z0-9])phy\d+-(?:ap|sta|mesh)\d|(?:^|[^a-z0-9])wlx[0-9a-f]{6}/i;
