...
(function (root, factory) {
  ...
  // Lo SCOPO di un indirizzo: se puo' vivere sull'Internet pubblica, e quando
  // non puo', PERCHE'. Serve dove un campo PROMETTE un indirizzo pubblico e chi
  // lo compila puo' incollarci quello che ha sott'occhio — la faccia interna del
  // router, il 100.64 che gli ha dato il provider — senza che niente a valle
  // possa accorgersene: il documento non prova a raggiungere quell'indirizzo,
  // quindi o lo dice il documento stesso o non lo dice nessuno.
  //
  // La fonte e' il registro IANA degli indirizzi a scopo speciale (RFC 6890 e
  // successivi), non una lista di vendor: la stessa risposta per ogni apparato
  // del mondo.
  //
  // ⚠️ L'IPv6 NON si riclassifica qui. La classe di un IPv6 la dice `ipv6Class`
  // (lib/ipv6.js), che e' l'unica definizione nel progetto: questa funzione la
  // TRADUCE nel vocabolario comune alle due famiglie. Riscrivere qui `fe80::/10`
  // vorrebbe dire tenere due definizioni di link-local, e la seconda diverge dalla
  // prima al primo ritocco. Restano da aggiungere i due soli prefissi che
  // `ipv6Class` non esprime, perche' per lui sono `global`: la documentazione e
  // gli IPv4-mapped.
  // I prefissi IPv4 a scopo speciale. L'ordine conta per l'unica coppia che si
  // contiene: `0.0.0.0/32` (l'indirizzo "non specificato") prima di `0.0.0.0/8`.
  const _V4_SPECIAL = [
    ['0.0.0.0/32', 'unspecified'],
    ['0.0.0.0/8', 'reserved'],
    ['10.0.0.0/8', 'private'],
    ['100.64.0.0/10', 'cgnat'],           // RFC 6598 — il NAT del provider
    ...
    ['240.0.0.0/4', 'reserved'],          // include 255.255.255.255
  ];
  const _V4_SPECIAL_INFO = _V4_SPECIAL.map(function (e) {
    return { info: _parseCidrInfo(e[0]), scope: e[1] };
  });

  // Le classi di `ipv6Class` nel vocabolario comune. Una classe che non sta qui
  // non si traduce a indovinare: `addrScope` risponde `null`, cioe' "non lo so",
  // e chi chiama non accusa nessuno su una supposizione.
  const _V6_CLASS_SCOPE = {
    unspecified: 'unspecified', loopback: 'loopback', multicast: 'multicast',
    'link-local': 'linkLocal', ula: 'private',
  };

  // Il vocabolario degli scopi, DERIVATO dalle due tabelle sopra piu' `global`.
  // Non e' un elenco parallelo: chi disegna (il pannello ne chiede la parola in
  // due lingue) e chi classifica devono avere lo stesso insieme, e un elenco
  // scritto a mano accanto a una tabella smette di seguirla senza far rumore.
  const ADDR_SCOPES = ['global'].concat(
    Array.from(new Set(_V4_SPECIAL.map(function (e) { return e[1]; })
      .concat(Object.keys(_V6_CLASS_SCOPE).map(function (k) { return _V6_CLASS_SCOPE[k]; }))))
      .filter(function (s) { return s !== 'global'; }).sort());

  // Accetta un indirizzo o un BLOCCO ("203.0.113.8/29"): un blocco vale per la
  // rete che dichiara, e i suoi indirizzi hanno tutti lo stesso scopo.
  // `null` quando non e' un indirizzo — che non e' un giudizio sull'indirizzo,
  // e chi chiama deve distinguerlo da "non e' pubblico".
  function addrScope(addr) {
    const s = String(addr == null ? '' : addr).trim();
    if (!s) return null;
    const base = s.split('/')[0].trim();
    if (base.indexOf(':') >= 0) {
      const f = v6 && v6.ipv6Class;
      const cls = typeof f === 'function' ? f(base) : null;
      if (!cls) return null;                      // ipv6.js assente, o non e' un IPv6
      if (_V6_CLASS_SCOPE[cls]) return _V6_CLASS_SCOPE[cls];
      if (cls !== 'global') return null;          // una classe che non conosco
      if (_ipInCidr(base, _parseCidrInfo('2001:db8::/32'))) return 'documentation';
      if (_ipInCidr(base, _parseCidrInfo('::ffff:0:0/96'))) return 'reserved';
      return 'global';
    }
    if (_parseIpv4Int(base) == null) return null;
    for (const e of _V4_SPECIAL_INFO) if (_ipInCidr(base, e.info)) return e.scope;
    return 'global';
  }
  ...
  function subnetInputToCidr(input) {
    ...
    // Un prefisso NUMERICO che _parseCidrInfo ha rifiutato è fuori range ("/33",
    // "/-1", "/129"): è un refuso, non un IP nudo, e non va trasformato in /24 in
    // silenzio (smoke 06/09 — il dichiarato è legge). Una netmask ("/255.255.255.0")
    // o altre forme non-numeriche cadono invece nel ramo IP-nudo qui sotto, come prima.
    const _sl = s.lastIndexOf('/');
    if (_sl >= 0 && /^-?\d+$/.test(s.slice(_sl + 1).trim())) return '';
    // range "a.b.c.X-Y", IP singolo, o "ip/netmask": prendo la parte-IP prima di '-' o '/'.
  ...
  return { _parseIpv4Int, _parseCidrInfo, _ipInCidr, _cidrsOverlap, _intToIpv4, subnetInputToCidr,
           addrFamily, addrKey, addrIsLinkLocalV6, addrScope, ADDR_SCOPES, segmentKey };
});
