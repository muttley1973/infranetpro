...
//     ⛔ **Ciò che NON si può fare è appoggiare l'operatore SULL'ARCO**, come se
//     quella linea portasse quel tunnel: il modello non lo dichiara da nessuna
//     parte (l'unico posto dove esiste è `underlayUplinkIds` dell'SD-WAN). Con
//     due linee in una sede, «quale delle due porta l'IPsec» non lo sa nessuno, e
//     disegnarlo sarebbe inventare con la faccia di un fatto.
...
(function (root, factory) {
  ...
  const INTER_SITE_LAYOUT_DEFAULTS = {
    ...
    bowStep: 34,        // scostamento fra archi della stessa coppia (⑤)
   ...
   */
  /** @param {number} n @param {number} maxW @param {{nodeGap:number, minRingR:number}} g */
  function _ringRadius(n, maxW, g) {
    if (n <= 1) return 0;
    const corda = maxW + g.nodeGap;
  ...
  function buildInterSiteLayout(rawOrg, opts) {
    ...
    const ringR = _ringRadius(ring.length, maxW, g);
    out.ringR = _r2(ringR);
