...
//      snmpDown,                                      // device SNMP in errore all'ultimo Sync
//      portConflicts,                                 // porte documentate ≠ realtà SNMP (access↔trunk/LAG)
//      drift: { absent, undocumented, ipChanged,
//               unverified },                         // conteggi dal Drift vivo
...
(function (root, factory) {
  ...
  function nextStep(summary) {
    ...
    // 3) Device SNMP in errore all'ultimo Sync → PRIMA la realtà operativa:
    //    finché i device monitorati non rispondono, «tutto in ordine» mentirebbe
    //    (il caso 0/N: rete sana ma sessione/alimentazione/credenziali da sistemare).
    //    Solo informativo (niente bottone: il testo dice già cosa fare).
    if (_num(s.snmpDown) > 0) return _step('snmpDown', null, null, { n: _num(s.snmpDown) });

    // 4) Subnet non raggiunte dalla Verifica → presenza NON confermabile là:
    //    dirlo esplicitamente (mai «tutto in ordine» su ciò che non si è visto).
    if (_num(d.unverified) > 0) return _step('unverified', null, 'onboard.askUnverified', { n: _num(d.unverified) });

    // 5) VLAN senza subnet → l'IPAM è cieco lì: dichiara la subnet.
    if (_num(g.noSubnet) > 0) return _step('vlanSubnet', null, 'onboard.askVlanSubnet', { n: _num(g.noSubnet) });

    // 6) VLAN senza gateway → manca il default gateway.
    if (_num(g.noGateway) > 0) return _step('vlanGateway', null, 'onboard.askVlanGateway', { n: _num(g.noGateway) });

    // 7) Non-documentati visti dalla Verifica → proponi l'adozione.
    if (_num(d.undocumented) > 0) return _step('adopt', null, 'onboard.askAdopt', { n: _num(d.undocumented) });

    // 8) Device documentati ma ASSENTI dall'ultima Verifica.
    if (_num(d.absent) > 0) return _step('absent', null, 'onboard.askAbsent', { n: _num(d.absent) });

    // 9) Cambi IP da riconciliare.
    if (_num(d.ipChanged) > 0) return _step('ipChanged', null, 'onboard.askIpChanged', { n: _num(d.ipChanged) });

    // 10) Porte documentate ≠ realtà SNMP (access ↔ trunk/LAG) → riconciliare
    //     dal pannello del device (banner portReconcileConflicts).
    if (_num(s.portConflicts) > 0) return _step('portConflicts', null, 'onboard.askPortConflicts', { n: _num(s.portConflicts) });

    // 11) Tutto in ordine: documentata e verificata, nessun buco aperto.
