...
// Rimuove eventuali credenziali (`user:pass@` / `user@`) dall'AUTORITÀ di un URL.
...
//
// ⚠️ Vale per le TRE forme in cui il campo arriva davvero, perché `lib/mgmt-url.js`
// dichiara legittimo anche il valore SENZA schema:
//     `https://user:pw@host`   ·   `//user:pw@host`   ·   `user:pw@host`
// Prima si vedeva solo la prima: `admin:pw@10.0.0.1` usciva INTATTO nel DTO REST,
// nell'inventory Ansible e nel contesto spedito all'LLM (smoke 07/09). Il puntatore
// backup, poche righe sotto, aveva già la regola giusta — era la stessa regola
// applicata a metà. Ora ce l'hanno tutt'e due.
//
// La userinfo è TUTTO ciò che precede l'ULTIMA `@` dell'autorità (una `@` non è
// valida in un host): così cade anche `https://u@evil.com@10.0.0.1`, che prima
// lasciava `evil.com@…` e si leggeva come se l'host fosse quello.
// Si tocca SOLO l'autorità: una `@` dentro path o query resta dov'è.
function _stripUrlCreds(u) {
  const s = String(u == null ? '' : u).trim();
  if (!s) return '';
  const m = /^([a-z][a-z0-9+.-]*:\/\/|\/\/)?([^/?#]*)([\s\S]*)$/i.exec(s);
  if (!m) return s;
  const pre = m[1] || '', auth = m[2] || '', rest = m[3] || '';
  const at = auth.lastIndexOf('@');
  if (at < 0) return s;
  // Una userinfo non contiene spazi: se ce ne sono non è un URL con credenziali ma
  // testo libero → non si tocca. Meglio un valore intatto che uno mutilato.
  if (/\s/.test(auth.slice(0, at))) return s;
  return pre + auth.slice(at + 1) + rest;
}
