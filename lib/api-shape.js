...
function _backupDto(backup) {
  ...
  if (!b) return null;
  const ref = _stripUrlCreds(b.ref) || null;
...
function nodeToDevice(node, ctx) {
  ...
  return {
    ...
    networkOs: vendorToNetworkOs({ brand: node.brand, model: node.model, sysDescr: node.integration && node.integration.system && node.integration.system.sysDescr }),
