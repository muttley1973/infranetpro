...
//     reserved?: ['10.90.20.240', …],  // indirizzi che il piano IMPEGNA senza un apparato
//     source?                   // 'manual' | 'dcim' — provenienza, come nel resto dell'app
//   }
//
// `reserved` sta sul PREFISSO e non altrove perche' e' quello che e': un indirizzo
// prenotato non appartiene a un apparato — se ci fosse un apparato non sarebbe una
// prenotazione — appartiene alla RETE. Lo scrive l'import DCIM (gli indirizzi che
// l'IPAM dichiara e non aggancia a niente) e lo legge l'occupazione (lib/ipam.js):
// non sono liberi, quindi «prossimo IP libero» non li propone.
//
...
(function (root, factory) {
  ...
  function migrateIpam(state) {
    ...
    // ⚠️ **Due righe con lo STESSO CIDR sono uno stato impossibile, non un caso
    // d'uso.** Qui una rete È il suo CIDR: `findPrefix` trova solo la prima,
    // `upsertPrefix` scrive solo sulla prima e `removePrefix` le toglie TUTTE.
    // La seconda riga è quindi un fantasma che solo il disegno distingue — e a
    // schermo si vedevano due reti che, premendo «togli» su una, sparivano
    // insieme. Le scriveva l'import DCIM da un NetBox con lo stesso prefisso in
    // due VRF (là sono reti diverse, qui no); ora l'import le fonde, e questa
    // riga ripara i documenti già salvati. Vince la prima, e la VLAN si tiene da
    // qualunque riga ce l'abbia: è l'informazione che il resto dell'app usa.
    // A senso unico e idempotente come tutto il resto della migrazione.
    const visti = new Map();
    const tenuti = [];
    for (const p of ipam.prefixes) {
      if (!p || typeof p !== 'object') { tenuti.push(p); continue; }
      const k = prefixKey(p.cidr);
      if (!k) { tenuti.push(p); continue; }          // una riga senza CIDR non si fonde con niente
      const gia = visti.get(k);
      if (!gia) { visti.set(k, p); tenuti.push(p); continue; }
      if (gia.vlan == null && p.vlan != null) gia.vlan = p.vlan;
      for (const campo of ['gateway', 'dns', 'description']) {
        if (!gia[campo] && p[campo]) gia[campo] = p[campo];
      }
    }
    if (tenuti.length !== ipam.prefixes.length) ipam.prefixes = tenuti;
