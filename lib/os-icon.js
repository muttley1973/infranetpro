...
(function (root, factory) {
  ...
  const OS_ICON_COLORS = {
    ...
    qemu: '#FF6600', hypervisor: '#4EB1A6', container: '#2496ED', netdev: '#00D4FF',
  };
  ...
  // logo dedicato: Hyper-V e Azure Local (Stack HCI, Windows-based) usano il logo
  // Windows (tecnologia Microsoft); kvm usa il logo QEMU dello stack QEMU/KVM.
  // XCP-ng non ha logo su Simple Icons → null: niente logo inventato. TrueNAS
  // Scale = Debian-based. (I loghi colorati proprietari — vSphere, Azure "A" — non
  // sono CC0 e non si incorporano: si resta ai loghi Simple Icons + glifi propri.)
  const HV_PLATFORM_KEY = { esxi: 'vmware', proxmox: 'proxmox', docker: 'container', truenas: 'linux', nutanix: 'nutanix', unraid: 'unraid', kvm: 'qemu', hyperv: 'windows', azurelocal: 'windows' };
  ...
  function osIconFromHvPlatform(code) {
    ...
    if (!k) return null;
    const fam = k === 'windows' ? 'windows' : k === 'container' ? 'container' : (['vmware', 'proxmox', 'qemu', 'nutanix'].indexOf(k) >= 0) ? 'hypervisor' : 'unix';
    return _mk(k, fam);
  }
