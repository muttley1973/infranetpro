...
(function (root, factory) {
  ...
  // `openAdopt(x);close()` → `openAdopt`. Null se non c'è una chiamata. (I bottoni
  // migrati a event delegation non hanno onclick: l'azione viene dal loro data-act.)
  ...
  function extractCatalog(html) {
    ...
    while ((m = reBtn.exec(src)) !== null) {
      ...
      // ASSE B (event delegation): i bottoni migrati non hanno più `onclick` ma
      // `data-act="chiave"` → la chiave è l'azione citabile (stabile, univoca per
      // comando). Fallback sull'onclick per i bottoni ancora classici.
      const action = _handler(_attr(openTag, 'onclick')) || _attr(openTag, 'data-act');
