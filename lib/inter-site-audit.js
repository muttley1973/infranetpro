 ...
 *   uplinksWithNonPublicIp: {uplinkId:string, siteId:string, addr:string, scope:string}[],
...
(function (root, factory) {
  ...
  const INTER_SITE_AUDIT_PROBLEMS = [
    ...
    'crossedPeerIps', 'uplinksImplausible', 'uplinksWithNonPublicIp',
  ];
  ...
  function buildInterSiteAudit(rawOrg) {
    ...
    const audit = {
      ...
      subnetsNotCarried: [], linksWithoutReach: [], sitesWithoutLink: [],
      sitesWithoutUplink: [], uplinksWithoutPublicIp: [], uplinksWithNonPublicIp: [],
      staticUplinksWithoutNextHop: [],
      linksWithoutUnderlay: [], underlaysAtOneEndOnly: [],
      notChecked,
    };
    ...
    if (!anyUplink) {
      ...
      notChecked.push({ check: 'uplinksWithNonPublicIp', reason: 'no-uplinks' });
      ...
      // Quante linee dichiarano ALMENO un indirizzo: senza nessuna, il controllo
      // sugli indirizzi non pubblici non ha guardato niente, e va detto.
      let conIndirizzi = 0;
      for (const u of uplinks) {
        ...
        const ips = prov.isFact(u.publicIps) ? prov.factValue(u.publicIps) : null;
        if (!ips || !ips.length) { audit.uplinksWithoutPublicIp.push({ uplinkId: u.id, siteId: u.siteId }); continue; }
        conIndirizzi++;
        // ㉗ **Un indirizzo che PROMETTE di essere pubblico e non lo è.**
        // Il campo si chiama «indirizzi pubblici» e accettava qualunque cosa
        // fosse un indirizzo: la faccia interna del router, il 100.64 che ha dato
        // il provider, uno 0.0.0.0. È una contraddizione fra il campo e il suo
        // valore — due cose DICHIARATE — e per questo sta fra i problemi e non
        // fra le lacune: non manca una riga, ce n'è una che dice il falso.
        // Nessun altro può accorgersene: il documento non prova a raggiungere
        // quell'indirizzo, quindi o lo dice il documento o non lo dice nessuno.
        // ⚠️ La classe la stabilisce `addrScope` (lib/cidr.js), che la legge dal
        // registro IANA (RFC 6890) e non da una lista di vendor.
        // ⚠️ Chi non è un indirizzo NON finisce qui: `normalizeWanUplink` non lo
        // salva nemmeno, e `addrScope` risponde `null` — che vuol dire «non lo
        // so», non «non è pubblico». Su un dubbio non si accusa.
        for (const raw of ips) {
          const scope = cidr.addrScope(raw);
          if (!scope || scope === 'global' || scope === 'documentation') continue;
          // L'indirizzo si riporta COME È SCRITTO: è il campo che qualcuno va a
          // correggere, e una forma canonica gli farebbe cercare una riga che
          // nel pannello non c'è.
          audit.uplinksWithNonPublicIp.push({
            uplinkId: u.id, siteId: u.siteId, addr: String(raw).trim(), scope,
          });
        }
      }
      // ⚠️ Gli indirizzi di DOCUMENTAZIONE (RFC 5737 e 2001:db8::/32) restano
      // fuori, ed è una decisione misurata, non una dimenticanza: sono gli
      // indirizzi che la IANA riserva proprio a chi scrive un esempio, il
      // suggerimento sotto il campo ne mostra due (`org.publicIpsHint`) e
      // l'archivio del progetto ne è pieno. Accusarli vorrebbe dire accusare la
      // pratica che il prodotto stesso consiglia.
      if (!conIndirizzi) notChecked.push({ check: 'uplinksWithNonPublicIp', reason: 'no-public-ip' });
