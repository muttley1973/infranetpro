...
// A model core is specific enough to carry an EXACT only if it keeps a separator
// or has >=2 letters; a bare/short number ('1024d', '60e', '9000') has lost its
// vendor prefix and would collide across vendors, so it may only feed the family
// step (which needs >=2 variants). Vendor-neutral: a rule on the core's shape.
function _specificForExact(core) {
  return core.indexOf('-') !== -1 || core.replace(/[^a-z]/g, '').length >= 2;
}
...
function _resolveModelCore(model, idx) {
  ...
  //    A core may assert an EXACT only when it is SPECIFIC (see _specificForExact):
  //    it keeps a separator ('2960-24tc-l') or has >=2 letters ('srx300'). A core
  //    collapsed to a short number collides across vendors ('DES-1024D' and
  //    'FortiSwitch 1024D' both reduce to '1024d'), so it is left to the family
  //    step, which needs >=2 variants and can never assert a single wrong model.
  for (const key of [core, alt]) {
    if (!key || !_specificForExact(key)) continue;
