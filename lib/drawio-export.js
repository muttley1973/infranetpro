...
//     normalizePortStatus(s) -> 'active'|'fault'|'inactive',
...
(function (root, factory) {
  ...
  const api = factory(isNode ? require('./xml-escape.js') : root);
  ...
  // Le due porte «off» MISURATE dallo switch (lib/port-state.js). Vincono su
  // qualunque stato dichiarato: e' la sola parte del disegno dove parla l'apparato.
  // ⚠️ Gemelli in 01-tokens.css e export.js — guardia in test/porta-senza-link-colore.
  const SHUT_COLOR   = '#0a0d11';   // spenta a mano (`shutdown`) — una DECISIONE, quieta
  const NOLINK_COLOR = '#d29922';   // nessun link da >=N verifiche — un SINTOMO, ambra
  ...
  const H_STAGGER_MIN = 4;         // passo MIN garantito fra cavi di uno stesso device (puo' sforare l'altezza del device sui device molto densi: meglio che collassarli in un blocco)
  const H_STAGGER_MAX = 14;        // passo MAX di sfalsamento verticale (i device con pochi cavi non ventagliano troppo)
  // Tabella cavi per VLAN (a destra del rack, sul layer della VLAN): click su una riga
  // -> highlight+scroll del cavo (custom action link draw.io, attivo in modalita' Vista).
  const TABLE_W      = 240;        // larghezza tabella
  const TABLE_HDR_H  = 20;         // altezza intestazione (nome VLAN)
  const TABLE_ROW_H  = 16;         // altezza riga cavo
  const TABLE_GAP_X  = 40;         // stacco della tabella dai nomi device
  const TABLE_HDR_BG = '#161b22';  // sfondo intestazione
  const TABLE_ROW_BG = '#0d1117';  // sfondo riga
  const TABLE_STROKE = '#30363d';  // bordo celle tabella
  const HL_COLOR     = '#ffd000';  // colore del lampeggio al click (ambra, alto contrasto)
  const CABLE_W      = 1.5;        // spessore base cavo (= stato "non evidenziato")
  const HL_W         = 5;          // spessore cavo EVIDENZIATO (persistente via Set Style)
  // Formato pagina: A4 verticale di default; se il contenuto non entra -> A3 verticale.
  const A4_W = 827, A4_H = 1169;
  const A3_W = 1169, A3_H = 1654;
  ...
  // --- Utility -------------------------------------------------------------
  // L'escape XML sta in `lib/xml-escape.js`: da quando l'SVG della mappa
  // inter-sede ne ha bisogno pure lui, tenerne una copia qui sarebbe la solita
  // definizione doppia — due funzioni identiche che divergono al primo ritocco.
  const _escXml = X.escapeXML;
  ...
  function _mkHelpers(h) {
    ...
    return {
      ...
      normalizePortStatus: h.normalizePortStatus || function (s) {
      ...
      // (pi)->'shut'|'no-link'|null — lib/port-state.js, la STESSA regola del rack a
      // schermo. Iniettata e non ri-scritta qui: un disegno esportato che colora le
      // porte con criteri suoi e' la classe di bug piu' cara del progetto. Se manca,
      // il diagramma perde i due grigi ma non ne inventa altri.
      portShade: h.portShade || null,
  ...
  // Colore di una porta. Ordine: la MISURA dello switch batte tutto (una porta in
  // `shutdown` non e' verde solo perche' il documento la dice attiva), poi il LAG
  // col ciano, poi lo stato dichiarato.
  function _portColor(pi, H) {
    const shade = H.portShade ? H.portShade(pi) : null;
    if (shade === 'shut') return SHUT_COLOR;
    if (shade === 'no-link') return NOLINK_COLOR;
    const lag = !!(pi.lagGroup) || (parseInt(pi.lagId || 0, 10) > 0);
    if (lag) return LAG_COLOR;
    return STATUS_COLOR[H.normalizePortStatus(pi.statusOvr != null ? pi.statusOvr : pi.status)] || STATUS_COLOR.inactive;
  }
  ...
  // Id STABILE di un cavo (cbl_<idlink>, fallback indice): la riga-tabella lo richiama
  // nel custom action link per evidenziarlo. Sanifico i caratteri non-parola.
  function _cblId(l, idx) {
    const raw = (l && l.id != null && l.id !== '') ? l.id : idx;
    return 'cbl_' + String(raw).replace(/[^\w.-]/g, '_');
  }

  // Un cavo come ARCO nativo: source/target = le celle-porta (id = pid). Nessuna freccia
  // (un cavo non ha verso). ANTI-SOVRAPPOSIZIONE: ogni cavo scende in una CORSIA
  // verticale dedicata a sinistra del cabinet (2 waypoint a `laneX`, tratto verticale
  // unico per corsia); esce/entra dal lato SINISTRO delle porte (exitX/entryX=0).
  // Colore = VLAN/override -> distinguibile. id STABILE -> bersaglio dell'highlight.
  function _cableEdge(l, idx, layerId, esc, color, laneX, ySrc, yDst, exitDy, entryDy) {
    const style = `endArrow=none;startArrow=none;html=0;edgeStyle=orthogonalEdgeStyle;rounded=0;`
      ...
      + `strokeColor=${color};strokeWidth=${CABLE_W};`;
    const pts = `<Array as="points"><mxPoint x="${_n(laneX)}" y="${_n(ySrc)}"/><mxPoint x="${_n(laneX)}" y="${_n(yDst)}"/></Array>`;
    return `<mxCell id="${_cblId(l, idx)}" style="${style}" edge="1" parent="${_escXml(layerId)}" `
      + `source="${esc(l.src)}" target="${esc(l.dst)}"><mxGeometry relative="1" as="geometry">${pts}</mxGeometry></mxCell>`;
  }

  // Tabella cavi di UNA VLAN, sul LAYER della VLAN (compare/scompare con essa). Header =
  // nome VLAN; una riga per cavo "DevA/portA -> DevB/portB". Ogni riga porta un CUSTOM
  // ACTION LINK draw.io (`data:action/json`): al click (in modalita' Vista/lightbox) il
  // cavo diventa SPESSO e RESTA tale (Set Style su strokeWidth = una delle poche azioni
  // PERSISTENTI di draw.io; l'`highlight` invece e' solo un lampeggio transiente). E'
  // "a radio": prima azzera lo spessore di TUTTI i cavi di questa VLAN, poi ingrossa
  // SOLO quello scelto -> ne resta evidenziato uno alla volta. Click sull'INTESTAZIONE
  // = azzera (deseleziona). NB: draw.io non ha un auto-spegnimento nativo "quando
  // modifichi il cavo" (l'azione `select`, che lo farebbe, non scatta dai link); la
  // deselezione e' manuale (altra riga o header). html=0 -> ZERO artefatti.
  function _vlanTable(layerId, title, rows, x) {
    const y0 = CABINET_Y;
    const idsJson = rows.map(function (r) { return `"${r.edgeId}"`; }).join(',');
    const reset = `{"style":{"cells":[${idsJson}],"key":"strokeWidth","value":"${CABLE_W}"}}`;  // tutti i cavi -> spessore base
    const hdrStyle = `rounded=0;html=0;fillColor=${TABLE_HDR_BG};strokeColor=${TABLE_STROKE};`
      + `fontColor=${NAME_COLOR};fontSize=11;fontStyle=1;align=left;verticalAlign=middle;spacingLeft=6;`;
    // Header = azzera evidenziazioni (click sull'intestazione).
    const hdrAction = `data:action/json,{"actions":[${reset}]}`;
    const hdrCell = `<mxCell style="${hdrStyle}" vertex="1" parent="${_escXml(layerId)}">`
      + `<mxGeometry x="${_n(x)}" y="${_n(y0)}" width="${_n(TABLE_W)}" height="${_n(TABLE_HDR_H)}" as="geometry"/></mxCell>`;
    let out = `<UserObject label="${_escXml(_fit(title, 34))}" link="${_escXml(hdrAction)}" id="cbltbl_${layerId}">${hdrCell}</UserObject>`;
    const rowStyle = `rounded=0;html=0;fillColor=${TABLE_ROW_BG};strokeColor=${TABLE_STROKE};`
      + `fontColor=${NAME_COLOR};fontSize=10;align=left;verticalAlign=middle;spacingLeft=6;`;
    rows.forEach(function (r, k) {
      const y = y0 + TABLE_HDR_H + k * TABLE_ROW_H;
      const action = `data:action/json,{"actions":[`
        + reset + `,`                                                                          // 1) azzera tutti
        + `{"style":{"cells":["${r.edgeId}"],"key":"strokeWidth","value":"${HL_W}"}},`          // 2) ingrossa QUESTO (persistente)
        + `{"highlight":{"cells":["${r.edgeId}"],"color":"${HL_COLOR}","opacity":100}},`        // 3) lampeggio di conferma
        + `{"scroll":{"cells":["${r.edgeId}"],"smooth":true}}]}`;                               // 4) porta in vista
      const cell = `<mxCell style="${rowStyle}" vertex="1" parent="${_escXml(layerId)}">`
        + `<mxGeometry x="${_n(x)}" y="${_n(y)}" width="${_n(TABLE_W)}" height="${_n(TABLE_ROW_H)}" as="geometry"/></mxCell>`;
      out += `<UserObject label="${_escXml(_fit(r.label, 40))}" link="${_escXml(action)}" id="cblrow_${r.edgeId}">${cell}</UserObject>`;
    });
    return out;
  }
  ...
  function _rackDiagram(rack, nodes, ports, U, H, skins, links, cablesName) {
    ...
    let layerDecl = '', edges = '', tables = '', tblRows = 0;
    if (rackLinks.length) {
      ...
      const vlanKeyOf = function (l) {
        let vid = hasVlan ? H.linkVlan(l) : null;
        if (vid == null || vid === '') vid = hasVlan ? 1 : null;   // default/untagged -> VLAN 1
        return (vid == null) ? '_all' : String(vid);
      };
      const portLabel = function (pid, devId) {
        const s = String(pid == null ? '' : pid);
        return (devId && s.indexOf(devId + '-') === 0) ? s.slice(devId.length + 1) : s;
      };


      const devName = function (n) { return (n && n.name) || (n && n.id) || '?'; };

      // UN ARCO PER CAVO (niente bundle): la lettura "quale cavo" vive nella TABELLA per
      // VLAN (sotto); la linea mostra il percorso quando la evidenzi dalla tabella.
      const cab = rackLinks.map(function (l, i) {
        const sn = nodeOf(l.src), dn = nodeOf(l.dst);
        const yA = yCenter(sn), yB = yCenter(dn);
        const label = devName(sn) + '/' + portLabel(l.src, sn && sn.id) + ' -> ' + devName(dn) + '/' + portLabel(l.dst, dn && dn.id);
        return { l: l, i: i, sn: sn, dn: dn, yA: yA, yB: yB, top: Math.min(yA, yB), bot: Math.max(yA, yB),
          color: _cableColor(l, H), layerKey: vlanKeyOf(l), edgeId: _cblId(l, i), label: label,
          lane: 0, exitDy: 0, entryDy: 0, ySrc: yA, yDst: yB };
      });

      // SFALSAMENTO verticale: i cavi che toccano lo STESSO device escono a Y distinte
      // (passo fra H_STAGGER_MIN e H_STAGGER_MAX; sui device molto connessi il ventaglio
      // sfora l'altezza invece di collassare). exitDy/entryDy spostano uscita/entrata.
      ...
      perDev.forEach(function (list, id) {
        ...
        const fit  = n > 1 ? band / (n - 1) : 0;                              // passo che sta DENTRO l'altezza del device
        const step = n > 1 ? Math.min(H_STAGGER_MAX, Math.max(H_STAGGER_MIN, fit)) : 0;
      ...
      // Dopo lo sfalsamento il tratto verticale va da ySrc a yDst: uso QUESTI estremi
      // (non i centri) per il partizionamento in corsie -> resta fedele anche con lo sforo.
      cab.forEach(function (c) { c.top = Math.min(c.ySrc, c.yDst); c.bot = Math.max(c.ySrc, c.yDst); });
      ...
      // UN LAYER PER VLAN, nominato col nome VLAN (la glue inietta linkVlan/vlanName).
      // Ogni cavo va sul layer della sua VLAN -> in draw.io ogni VLAN si attiva/disattiva
      // da sola. Senza VLAN iniettata (uso base/test) resta un unico 'Cavi'.
      const layerKeys = Array.from(new Set(cab.map(function (c) { return c.layerKey; })))
        .sort(function (a, b) { return (Number(a) || 0) - (Number(b) || 0); });
      const layerById = new Map(), layerName = new Map();
      layerKeys.forEach(function (k) {
        ...
        layerById.set(k, id); layerName.set(k, name);
        layerDecl += _layerCell(id, name);
      });

      // Emissione cavi: un arco per cavo. laneX = a SINISTRA del cabinet.
      ...
      // TABELLE per VLAN (una per layer), TUTTE allo STESSO ancoraggio a destra dei nomi
      // device: cosi' il contenuto resta entro la larghezza A4 (tableX0+TABLE_W < 827).
      // I layer VLAN sono visibili UNO ALLA VOLTA -> le tabelle non si sovrappongono nel
      // flusso normale (se ne mostri piu' d'una si sovrappongono: e' il prezzo dell'A4).
      // Sul layer della VLAN -> compaiono con essa. Righe nell'ordine originale dei link.
      const tableX0 = CABINET_X + MARGIN_LEFT + DEVICE_W + MARGIN_RIGHT + LABEL_GAP + LABEL_W + TABLE_GAP_X;
      layerKeys.forEach(function (k) {
        const rows = cab.filter(function (c) { return c.layerKey === k; })
          .map(function (c) { return { edgeId: c.edgeId, label: c.label }; });
        if (rows.length > tblRows) tblRows = rows.length;   // tabella VLAN piu' lunga (per l'altezza pagina)
        tables += _vlanTable(layerById.get(k), layerName.get(k), rows, tableX0);
      });
    }

    // Ordine: base(0,1) + [layer cavi] + cabinet/device + archi cavi + tabelle.
    const body = `<mxCell id="0"/><mxCell id="1" parent="0"/>` + layerDecl + devBody + edges + tables;

    // Formato pagina PER RACK: A4 verticale; se il contenuto (rack alto o tabella VLAN
    // lunga) non entra in A4, passa ad A3 verticale. Ingombri: destra = nomi/tabella,
    // basso = max(cabinet, tabella VLAN piu' lunga).
    const nameRight = CABINET_X + MARGIN_LEFT + DEVICE_W + MARGIN_RIGHT + LABEL_GAP + LABEL_W;
    const contentRight = Math.max(CABINET_X + cabW, nameRight, tables ? nameRight + TABLE_GAP_X + TABLE_W : 0);
    const tableBottom = tblRows ? CABINET_Y + TABLE_HDR_H + tblRows * TABLE_ROW_H : 0;
    const contentBottom = Math.max(CABINET_Y + cabH, tableBottom);
    const fitsA4 = contentRight <= A4_W && contentBottom <= A4_H;
    const pageW = fitsA4 ? A4_W : A3_W;
    const pageH = fitsA4 ? A4_H : A3_H;
    const model = `<mxGraphModel dx="800" dy="600" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="${pageW}" pageHeight="${pageH}" math="0" shadow="0"><root>${body}</root></mxGraphModel>`;
