...
(function (root, factory) {
  ...
  // scadenza; separarle vorrebbe dire regole che col tempo divergono. Sono QUATTRO:
  // `adminDown` (spenta a mano), `operUp` (c'e' link), `downStreak` (da quante
  // verifiche non c'e') e `operWait` (la parola dell'agente quando non e' ne' su ne'
  // giu': dormant, testing). L'ultima e' quella che sostituisce il vecchio stato
  // «idle»: era una TINTA che non scadeva mai, adesso e' una lettura che scade con la
  // prova che la reggeva.
  ...
  function forgetPortMeasure(pi) {
    ...
    if (Object.prototype.hasOwnProperty.call(pi, 'operWait')) { delete pi.operWait; forgotten = true; }
    return forgotten;
  }
