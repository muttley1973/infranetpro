...
//     identityCount,                     // identityDrift (serial/model = apparato sostituito)
//     absentCount, ghostCount,           // SECONDARI (macOrphan + ghostCable)
//     primaryCount,                      // headline = newCount + changedCount + identityCount
//     secondaryCount,                    // absentCount + ghostCount
//     hasChanges, allClear
//   }
// Un apparato SOSTITUITO (serial/model diverso) è un evento importante e azionabile
// → PRIMARY (headline). Il solo-firmware (identityFirmware) è informativo → NON contato
// (si aggiorna a ogni patch, come nel banner del Drift).
//
...
(function (root, factory) {
  ...
  function summarizeDriftReport(report) {
    ...
    const identityCount = _n(c.identityDrift);       // serial/model = apparato sostituito (azionabile)
    const absentCount = _n(c.macOrphan);             // SECONDARIO (rumore infra)
    const ghostCount = _n(c.ghostCable);             // SECONDARIO
    const primaryCount = newCount + changedCount + identityCount;
