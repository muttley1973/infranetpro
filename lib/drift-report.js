...
(function (root, factory) {
  ...
  function buildDriftReport(snmp, doc, ignores, opts) {
    ...
    // Osservabilità PER-DEVICE (fix falso "assente" cross-subnet E multi-fabric): un
    // device senza alcun segnale è "assente" solo se avevamo davvero modo di vederlo.
    // Due limiti di visibilità reali:
    //  • la sweep ARP/ping raggiunge solo i segmenti che il server tocca → un device
    //    su una subnet non spazzata resta "non-verificabile", non assente;
    //  • la FDB bridge vede i MAC su tutte le VLAN DEL SUO DOMINIO L2, ma NON quelli di
    //    un altro fabric raggiungibile solo a L3 (es. una LAN reale dietro un router,
    //    separata dal lab): gli switch del lab non imparano i MAC della LAN reale. Con
    //    `fdbSubnets` la copertura FDB è quindi limitata alle subnet realmente viste a
    //    L2; senza quel dettaglio si ricade sul comportamento storico (FDB copre tutto).
    ...
    const fdbNets = (snmp.fdbObserved === true && Array.isArray(snmp.fdbSubnets)) ? new Set(snmp.fdbSubnets) : null;
    // Back-compat: FDB osservata ma SENZA dettaglio per-subnet → si assume copra tutte
    // le VLAN (comportamento storico, per i chiamanti che non passano fdbSubnets).
    const fdbCoversAll = (snmp.fdbObserved === true) && !Array.isArray(snmp.fdbSubnets);
    const _net24 = ip => { const m = /^(\d{1,3}\.\d{1,3}\.\d{1,3})\./.exec(String(ip || '')); return m ? m[1] : null; };
    function _observable(dm) {
      if (fdbCoversAll) return true;                            // FDB senza dettaglio subnet → storico
      const net = _net24(dm.ip);
      if (!net) return true;                                    // IP del device ignoto → non possiamo affermare la cecità
      if (observedNets && observedNets.has(net)) return true;   // la sweep ha raggiunto questa subnet
      if (fdbNets && fdbNets.has(net)) return true;             // la FDB bridge copre la L2 di questa subnet
      if (!observedNets && !fdbNets) return true;               // nessuna info per-subnet → comportamento storico
      return false;                                             // copertura nota ma non include questa subnet → unverified
    }
    if (haveObservability) {
      ...
      // l'audit per-MAC sopra non li vede. Presenza per-nodeId: SNMP risposto / sweep.
      //   • segnale di presenza (responded/present)        → presente (salta);
      //   • nessun segnale, subnet NON osservabile          → non verificabile (grigio);
      //   • nessun segnale, subnet osservabile, HA SNMP     → assente (l'ho interrogato, muto);
      //   • nessun segnale, subnet osservabile, NO SNMP     → non lo si è sondato attivamente
      //                                                        → non si inventa (resta com'è).
