...
(function (root, factory) {
  ...
  function buildDriftReport(snmp, doc, ignores, opts) {
    ...
    const out = { consistent: [], stateDrift: [], macOrphan: [], undocumented: [], ghostCable: [], ipChanged: [], unverified: [], identityDrift: [] };
    ...
    if (haveObservability) {
      ...
      for (const dm of (doc.macs || [])) {
        ...
        if (dm.nodeId && trustAbsent[dm.nodeId]) {
          ...
          out.unverified.push({ key: `unver:${mac}`, mac: dm.mac, label: dm.label || '', nodeId: dm.nodeId || '', ip: dm.ip || (rel && rel.ip) || '', reason: rel ? 'leaseReleased' : undefined, hostname: rel ? rel.hostname : '' });
        }
      }
