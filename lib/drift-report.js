...
//
// ── OUTPUT ───────────────────────────────────────────────────────────
// { counts:{consistent,stateDrift,macOrphan,undocumented,undocumentedEndpoint,shutCable,ghostCable,ipChanged,unverified,identityDrift,identityFirmware},
...
(function (root, factory) {
  ...
  // La CODA azionabile: quanti elementi aspettano una decisione umana. Estratta
  // dal banner e resa pubblica perche' ora la legge anche la tendenza nel tempo
  // (lib/verify-trend.js): due posti che contano "la coda" con due somme diverse
  // sarebbero due verita' sullo stesso numero, e la seconda si guasterebbe in
  // silenzio al primo bucket nuovo.
  // ⚠️ Chi NON e' in coda, e il perche':
  //  · shutCable            — una porta spenta a mano e' uno STATO DICHIARATO, non
  //                           un'anomalia: non c'e' niente da decidere.
  //  · undocumentedEndpoint — rumore endpoint/guest, contato a parte per disegno.
  //  · unverified           — non e' lavoro da fare, e' una cosa che NON abbIAMO
  //                           guardato: sta nella copertura, non nella coda.
  //  · identityFirmware     — informativo (versione cambiata), non azionabile.
  function driftActionable(counts) {
    const c = counts || {};
    return (c.stateDrift || 0) + (c.macOrphan || 0) + (c.undocumented || 0)
      + (c.ghostCable || 0) + (c.ipChanged || 0) + (c.identityDrift || 0);
  }

  function driftBannerKind(counts, report) {
    const c = counts || {};
    const r = report || {};
    const actionable = driftActionable(c);
  ...
  return { buildDriftReport, driftBannerKind, driftActionable };
});
