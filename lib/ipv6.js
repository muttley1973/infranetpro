...
(function (root, factory) {
  ...
  // viene rimosso: non fa parte dell'indirizzo. Esportata come `_parseIpv6Words`
  // (non come "parseIpv6": un nome cosi' generico su window sarebbe un invito a
  // riscriverlo altrove) perche' lib/cidr.js deve poter mascherare un prefisso, e
  // l'espansione di un IPv6 in 8 word deve avere UNA definizione sola.
  ...
  // Da una lista di indirizzi IPv6 (es. gli indirizzi PROPRI di un device dalla
  // ipAddressTable SNMP) sceglie il MIGLIORE come "indirizzo di gestione" per un
  // singolo campo node.ip6. Regole: SCARTA link-local/loopback/multicast/unspecified
  // (tiene solo global/ULA routabili); preferisce STABILE (EUI-64/manuale) su privacy
  // (temporaneo), poi GLOBAL su ULA. Un indirizzo documentato deve essere PERSISTENTE,
  // quindi lo stabile vince sul temporaneo. Ritorna la forma canonica, o null.
  function pickBestIp6(list) {
    const cands = [];
    for (const raw of (Array.isArray(list) ? list : [])) {
      const ip = canonicalizeIpv6(raw);
      if (!ip) continue;
      const cls = ipv6Class(ip);
      if (cls !== 'global' && cls !== 'ula') continue;   // no link-local/loopback/multicast/unspecified
      cands.push({ ip, global: cls === 'global', privacy: isPrivacyIid(ip) });
    }
    if (!cands.length) return null;
    cands.sort((a, b) =>
      (Number(a.privacy) - Number(b.privacy)) ||   // stabile (privacy=false) prima
      (Number(b.global) - Number(a.global)));      // poi global prima di ULA
    return cands[0].ip;
  }

  return { isValidIpv6, canonicalizeIpv6, bytesToIpv6, macFromEui64, ipv6Class, isPrivacyIid, pickBestIp6,
           _parseIpv6Words: parseIpv6 };
});
