// ============================================================
// DRIFT SNAPSHOT — costruzione PURA degli snapshot doc / realtà
// ============================================================
// Estrae le REGOLE di costruzione dei due snapshot che alimentano il diff
// engine (lib/drift-report.js), separandole dalla raccolta-dati da stato/DOM
// (che resta nel glue src/app-drift.js). Funzioni PURE: input espliciti →
// output JSON, nessun accesso a DOM/state/global. Tutto ciò che è impuro
// (etichette UI i18n, classificazione MAC virtuale/randomizzato, conteggio MAC
// per porta, staleness dei lease) è INIETTATO come callback — stesso pattern di
// computeIpamUsage (lib/ipam.js) che inietta parseCidr/ipInCidr. Condiviso
// browser + test (UMD-lite).
//
// ── buildDocSnapshot(model) → { ports, macs, ipOnly, deviceSigs, cables } ────
//    ipOnly = documentati CON IP ma SENZA MAC ({nodeId,label,ip,hasSnmp}):
//    audit di presenza per-nodeId (non per-MAC) in buildDriftReport.
//   model = {
//     nodes, links, ports,                 // stato grezzo (array/mappa)
//     portLabel(pid)   -> string,           // etichetta porta (UI)
//     nodeLabel(node)  -> string,           // nome visualizzato del device
//     cableLabel(link) -> string,           // etichetta cavo (auto se assente)
//     normMac(mac)     -> string,           // normalizzazione MAC (lower)
//     isPassiveNoIp(node) -> bool           // passivo senza IP proprio → fuori audit presenza
//   }
//
// ── buildSnmpSnapshot(model) → snapshot REALTÀ per buildDriftReport ─────────
//   model = {
//     nodes, docPorts, ports,               // stato grezzo + le porte documentate da rispecchiare
//     fdb, vlanCache,                       // _topoFdbCache / _topoFdbVlanCache
//     reachable, arpTable,                  // sweep raggiungibilità + tabella ARP
//     snmpArp,                              // { macLower: ip4 } ARP dei router/switch L3 (Fase 2): presenza VIVA cross-subnet → macAtIp/observedMacs (verde) + observedSubnets
//     snmpNd,                               // { macLower: ip6 } Neighbor Discovery IPv6 dei router/switch L3 (ND discovery): presenza VIVA cross-subnet → SOLO observedMacs (verde per-MAC). NON alimenta macAtIp/observedSubnets (vedi blocco consumo)
//     leases, knownSigs, rejectedAutoLinks, // lease DHCP, firme note, link rifiutati
//     leaseReleasedHint,                    // bool opt-in: se true, un lease `state==='released'` → releasedMacs (sfumatura debole "probabilmente scollegato"; MAI da expiry, MAI rosso)
//     normMac, isVirtualMac, isRandomizedMac, isLeaseStale, countMacsPerPort,
//     fdbSeenLabel(sw, ifName) -> string,   // "visto su <switch> · <ifName>"
//     leaseSeenLabel(ip, hostname) -> string
//   }
(function (root, factory) {
  const api = factory(
    typeof module !== 'undefined' && module.exports ? require('./vm-nics.js') : root,
    typeof module !== 'undefined' && module.exports ? require('./port-id.js') : root,
    typeof module !== 'undefined' && module.exports ? require('./cidr.js') : root,
    typeof module !== 'undefined' && module.exports ? require('./mac-class.js') : root,
    typeof module !== 'undefined' && module.exports ? require('./identity-reconcile.js') : root,
    typeof module !== 'undefined' && module.exports ? require('./vlan-trunk.js') : root
  );
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  if (typeof window !== 'undefined') Object.assign(window, api);
})(typeof self !== 'undefined' ? self : this, function (vn, pidApi, cidrApi, mc, idApi, vt) {
  'use strict';

  // Lista di VLAN normalizzata e CONFRONTABILE: «10,20,100-102» e [20,10] devono
  // dare la stessa cosa, o due scritture diverse dello stesso elenco sembrerebbero
  // una discrepanza. Una sola definizione (lib/vlan-trunk.js), come per i MAC e i
  // segmenti: mai un confronto per stringa. Vuoto → null = «non dichiarato».
  const _parseVlans = (vt && vt.parseVlanList) || (raw => (Array.isArray(raw) ? raw.slice() : []));
  function _vlanList(raw) {
    if (raw === undefined || raw === null || raw === '') return null;
    const a = _parseVlans(raw);
    return (a && a.length) ? a : null;
  }

  // MAC di TUTTE le vNIC di una VM (lib/vm-nics.js). Fallback difensivo al
  // vecchio campo piatto se la lib non è ancora caricata: meglio un MAC solo
  // che nessuno — un MAC mancante rimette la VM fra i "non documentati".
  const _vmMacs = (vn && vn.vmMacs) || (vm => (vm && vm.mac) ? [vm.mac] : []);

  // primo valore non-null tra a, b, null (≡ a ?? b ?? null: mantiene '' e 0)
  function _pref(a, b) { return a != null ? a : (b != null ? b : null); }
  // Segmento di un indirizzo — la definizione sta in lib/cidr (`segmentKey`), qui
  // si passa solo l'elenco dei prefissi DICHIARATI: se uno contiene l'indirizzo
  // vince la sua maschera vera, altrimenti resta l'assunzione /24. Non è un
  // dettaglio estetico: questa chiave alimenta `observedSubnets`, cioè «la sweep
  // ha coperto questo segmento?», da cui dipende se un device assente diventa
  // ROSSO (assenza provata) o GRIGIO (non verificabile). Con due /25 dentro una
  // /24, l'assunzione dichiarava coperta anche la metà mai sondata → rosso su un
  // device che nessuno aveva cercato (audit D2).
  // ⚠️ La stessa lista di prefissi deve arrivare a lib/project-networks, che
  // CONFRONTA le sue righe con queste chiavi: due liste diverse = due mondi.
  const _net24 = (ip, prefixes) => (cidrApi.segmentKey(ip, prefixes) || null);

  // ── 1) Snapshot DOC ──────────────────────────────────────────────────
  function buildDocSnapshot(model) {
    const m = model || {};
    const nodes = m.nodes || [];
    const links = m.links || [];
    const portsState = m.ports || {};
    const portLabel = typeof m.portLabel === 'function' ? m.portLabel : (pid => String(pid));
    const nodeLabel = typeof m.nodeLabel === 'function' ? m.nodeLabel : (n => (n && (n.name || n.id)) || '');
    const cableLabel = typeof m.cableLabel === 'function' ? m.cableLabel : (l => (l && (l.label || l.id)) || '');
    const normMac = typeof m.normMac === 'function' ? m.normMac : mc.macKey;   // mai un lowercase nudo: la chiave MAC è una sola (lib/mac-class)
    const isPassiveNoIp = typeof m.isPassiveNoIp === 'function' ? m.isPassiveNoIp : (() => false);

    const ports = {};
    const linkPids = new Set();
    // Il cavo che tocca la porta: la lista «VLAN trasportate» DICHIARATA vive lì
    // (`link.trunkVlans`, manual-first), non sulla porta — la porta ha solo quella
    // MISURATA. Sono le due metà del confronto, e stanno in due posti diversi.
    const linkOfPid = new Map();
    for (const l of links) {
      if (l && l.src) { linkPids.add(l.src); if (!linkOfPid.has(l.src)) linkOfPid.set(l.src, l); }
      if (l && l.dst) { linkPids.add(l.dst); if (!linkOfPid.has(l.dst)) linkOfPid.set(l.dst, l); }
    }
    for (const pid of linkPids) {
      const pi = portsState[pid] || {};
      const lk = linkOfPid.get(pid) || null;
      // Solo un trunk DICHIARATO a mano entra nel confronto: la derivazione
      // automatica (voce/SSID/VM) non è una dichiarazione dell'utente, e metterla
      // qui vorrebbe dire confrontare la realtà con una deduzione della realtà.
      const trunkVlans = (lk && lk.mode !== 'access') ? _vlanList(lk.trunkVlans) : null;
      ports[pid] = {
        label: portLabel(pid),
        status: _pref(pi.statusOvr, pi.status),
        speed: _pref(pi.speedOvr, pi.speed),
        duplex: pi.duplex != null ? pi.duplex : null,
        vlan: _pref(pi.vlanOvr, pi.vlan),
        trunkVlans,
        linkId: (trunkVlans && lk) ? (lk.id || null) : null,   // dove si scrive, se si adotta la realtà
      };
    }
    const macs = [];
    // Documentati CON IP ma SENZA MAC (infra/endpoint mai sincronizzati con
    // successo): audit di presenza SEPARATO, per-nodeId (SNMP risposto / sweep),
    // non per-MAC. Senza questo restavano a colori pieni anche se irraggiungibili.
    const ipOnly = [];
    const deviceSigs = [];
    // Identità hardware DICHIARATA (serial/model/firmware), confrontata in
    // buildDriftReport col misurato ENTITY-MIB per scoprire un apparato sostituito.
    const identities = [];
    for (const n of nodes) {
      // VM ospitate: MAC "noti" → fuori dai non-documentati. SOLO deviceSigs,
      // NON macs: una VM (magari spenta) non entra nell'audit di presenza.
      // Dalla 78ª si prendono i MAC di TUTTE le sue vNIC: un firewall virtuale
      // a tre gambe si presentava in rete con tre MAC e due su tre finivano fra
      // i non-documentati della Verifica.
      if (Array.isArray(n.vms)) for (const vm of n.vms) { for (const m of _vmMacs(vm)) deviceSigs.push(normMac(m)); }
      // Identità hardware dichiarata: solo se almeno un campo è valorizzato; il
      // confronto per-campo e il gate su responded vivono nel report (drift-report.js).
      // Indipendente dal ramo MAC: uno switch non ha n.mac ma ha serial/inventory.
      {
        const sn = String(n.serialNumber == null ? '' : n.serialNumber).trim();
        const md = String(n.model == null ? '' : n.model).trim();
        const fw = String(n.firmwareVer == null ? '' : n.firmwareVer).trim();
        if (sn || md || fw) identities.push({ nodeId: n.id, label: nodeLabel(n), serialNumber: sn, model: md, firmwareVer: fw });
      }
      const ip = ((n.integration && n.integration.host) || n.ip || '').trim();
      if (!n.mac) {
        // Senza MAC ma con IP e non-passivo → nell'audit per-nodeId.
        if (ip && !isPassiveNoIp(n)) {
          const drv = String((n.integration && n.integration.driver) || '');
          ipOnly.push({ nodeId: n.id, label: nodeLabel(n), ip, hasSnmp: drv.indexOf('snmp') === 0 });
        }
        continue;
      }
      deviceSigs.push(normMac(n.mac));
      // I passivi SENZA IP proprio (prese a muro/patch panel/passacavi/quadri) non
      // hanno identità di rete verificabile → fuori dall'audit di presenza.
      if (isPassiveNoIp(n)) continue;
      macs.push({
        mac: n.mac,
        label: nodeLabel(n),
        nodeId: n.id,
        ip,
        ipManual: !!n.ipManual,
      });
    }
    for (const pid of Object.keys(portsState)) { const mm = portsState[pid] && portsState[pid].mac; if (mm) deviceSigs.push(normMac(mm)); }
    // ⭐ Gli indirizzi che il DOCUMENTO conosce e per cui NON ha un MAC. Servono a non
    // accusare di essere sconosciuto un MAC che la rete colloca a uno di essi: è il MAC
    // di qualcosa che documentiamo, e che non abbiamo mai scritto perché non c'è modo di
    // leggerlo (un apparato SNMP non espone un «MAC di device»).
    // Sono DUE forme della stessa dichiarazione, e il banco insegna che fermarsi alla
    // prima cura un esemplare invece del caso: l'indirizzo di gestione di uno switch, e
    // i GATEWAY dichiarati nel piano — che di quello stesso switch sono le altre
    // interfacce (SW-CORE risponde su .99.1 come su .10.1, .20.1 e .30.1).
    // Un indirizzo per cui il documento HA un MAC resta fuori: lì un MAC diverso non è
    // rumore da zittire, è un'identità cambiata, e merita un segnale suo.
    const ipsWithMac = new Set();
    for (const dm of macs) { const ip = String((dm && dm.ip) || '').trim(); if (ip) ipsWithMac.add(ip); }
    const knownIps = [];
    const _addKnownIp = (raw) => {
      const ip = String(raw == null ? '' : raw).trim();
      if (!ip || ipsWithMac.has(ip) || knownIps.indexOf(ip) >= 0) return;
      knownIps.push(ip);
    };
    for (const dn of ipOnly) _addKnownIp(dn && dn.ip);
    for (const p of (m.prefixes || [])) _addKnownIp(p && p.gateway);
    // autoLinked distingue il cavo DEDOTTO dal MANUALE: solo il dedotto può diventare
    // «cavo fantasma» su porta-down (il manuale su porta-down è il NODO che è down —
    // cablaggio != liveness, coerente col Proof-State).
    const cables = links.map(l => ({ id: l.id, label: cableLabel(l), src: l.src, dst: l.dst, autoLinked: !!l.autoLinked }));
    return { ports, macs, ipOnly, knownIps, deviceSigs, cables, identities };
  }

  // ── 2/3) Snapshot REALTÀ ─────────────────────────────────────────────
  function buildSnmpSnapshot(model) {
    const m = model || {};
    const nodes = m.nodes || [];
    const docPorts = m.docPorts || {};
    const portsState = m.ports || {};
    const fdb = (m.fdb && typeof m.fdb === 'object') ? m.fdb : {};
    const vlanCache = (m.vlanCache && typeof m.vlanCache === 'object') ? m.vlanCache : {};
    const reach = (m.reachable && typeof m.reachable === 'object') ? m.reachable : null;
    const arp = (m.arpTable && typeof m.arpTable === 'object') ? m.arpTable : null;
    const snmpArp = (m.snmpArp && typeof m.snmpArp === 'object') ? m.snmpArp : null;   // Fase 2: ARP router/switch L3 (mac→ip4)
    const snmpNd  = (m.snmpNd  && typeof m.snmpNd  === 'object') ? m.snmpNd  : null;   // ND discovery: Neighbor Discovery IPv6 router/switch L3 (mac→ip6)
    const leases = Array.isArray(m.leases) ? m.leases : [];
    // Reti DICHIARATE: danno a `_net24` la maschera vera (v. sopra). Se mancano,
    // resta l'assunzione /24 — stesso risultato di prima, ma dichiarata in un
    // posto solo (lib/cidr `segmentKey`).
    const prefixes = Array.isArray(m.prefixes) ? m.prefixes : null;
    const leaseReleasedHint = m.leaseReleasedHint === true;   // opt-in: lease RILASCIATO = sfumatura debole "probabilmente scollegato" (mai da expiry, mai rosso)
    const knownSigs = m.knownSigs || [];
    const rejectedAutoLinks = m.rejectedAutoLinks || [];
    const normMac = typeof m.normMac === 'function' ? m.normMac : mc.macKey;   // mai un lowercase nudo: la chiave MAC è una sola (lib/mac-class)
    const isVirtualMac = typeof m.isVirtualMac === 'function' ? m.isVirtualMac : (() => false);
    const isRandomizedMac = typeof m.isRandomizedMac === 'function' ? m.isRandomizedMac : (() => false);
    const leaseStale = typeof m.isLeaseStale === 'function' ? m.isLeaseStale : (() => false);
    const countMacsPerPort = typeof m.countMacsPerPort === 'function' ? m.countMacsPerPort : (() => ({}));
    const fdbSeenLabel = typeof m.fdbSeenLabel === 'function' ? m.fdbSeenLabel : ((sw, ifName) => `${sw}${ifName ? ' · ' + ifName : ''}`);
    const leaseSeenLabel = typeof m.leaseSeenLabel === 'function' ? m.leaseSeenLabel : ((ip, hn) => `${ip}${hn ? ' · ' + hn : ''}`);

    const responded = {};
    for (const n of nodes) { if (n.snmpStatus === 'ok') responded[n.id] = true; }
    // Identità hardware MISURATA (ENTITY-MIB → integration.inventory) SOLO per i
    // device che hanno RISPOSTO in questo ciclo: l'inventory è persistita nel JSON,
    // un device muto la porterebbe stantìa → falso "apparato sostituito". Il report
    // rigatea comunque su responded (come le porte), belt-and-suspenders.
    //
    // «Ha risposto» non basta però: un apparato può rispondere benissimo e NON
    // riconfermare la propria identità (walk ENTITY-MIB troncata, MIB assente). In
    // quel caso l'inventory che sopravvive è l'ULTIMO NOTO, marcato `stale` da
    // lib/identity-reconcile.js — e da una misura vecchia non si tira fuori
    // un'accusa di apparato sostituito. Chi non riconferma, qui non parla.
    const _confirmed = (idApi && idApi.isConfirmedMeasure) || (inv => !!inv && typeof inv === 'object' && inv.stale !== true);
    const measuredIds = {};
    for (const n of nodes) {
      if (!responded[n.id]) continue;
      const inv = n.integration && n.integration.inventory;
      if (!_confirmed(inv)) continue;
      measuredIds[n.id] = {
        serialNumber: String(inv.serialNumber == null ? '' : inv.serialNumber).trim(),
        model: String(inv.model == null ? '' : inv.model).trim(),
        firmwareVer: String(inv.firmwareVer == null ? '' : inv.firmwareVer).trim(),
      };
    }
    // Osservabilità: la sweep ha girato se ha trovato vivo almeno un host OPPURE
    // se c'è una tabella ARP non vuota (anch'essa una vista della rete).
    const reachabilityChecked = (!!reach && Object.values(reach).some(r => r && r.alive))
      || (!!arp && Object.keys(arp).length > 0);
    // Presenza multi-segnale: SNMP risposto OPPURE IP raggiunto dalla sweep.
    const presentNodeIds = {};
    for (const n of nodes) {
      if (n.snmpStatus === 'ok') { presentNodeIds[n.id] = true; continue; }
      if (reach) {
        const ip = ((n.integration || {}).host || n.ip || '').trim();
        if (ip && reach[ip] && reach[ip].alive) presentNodeIds[n.id] = true;
      }
    }
    // Assenza AFFIDABILE per-nodeId (presenza "onesta"): SOLO se la sweep ha provato
    // che l'IP documentato è sul FILO locale e non risponde più all'ARP dopo il ping
    // (`reach[ip].absent === true`, calcolato lato server). È l'unico segnale che
    // autorizza il "rosso" a valle (drift-report); qualunque altro silenzio (SNMP
    // muto, FDB invecchiata, ICMP filtrato, IP remoto) resta "non-verificato" (grigio).
    // Nel Sync NON c'è sweep → reach assente → QUESTO campo resta vuoto (la via ARP-miss
    // è off). Il rosso resta comunque possibile via la porta-down autoritativa (Fase 3,
    // portDownStreak, aggiunta a valle in drift-report), che non richiede lo sweep.
    const trustAbsentNodeIds = {};
    if (reach) {
      for (const n of nodes) {
        const ip = ((n.integration || {}).host || n.ip || '').trim();
        if (ip && reach[ip] && reach[ip].absent === true) trustAbsentNodeIds[n.id] = true;
      }
    }
    // ── Le TABELLE sono RICORDI, la sweep è una MISURA ────────────────────────
    // FDB dello switch, ARP/ND del router, lease DHCP: nessuna di queste tabelle
    // ri-verifica alcunché quando la si legge — dice solo «l'ho visto passare».
    // Una MAC-table tiene un client appena scollegato per tutto l'aging time (300s
    // di default, di più su alcuni vendor); un lease dura ore. Se la sweep ha appena
    // PROVATO che l'IP documentato non c'è più sul NOSTRO filo (ARP-miss dopo il
    // ping), quel ricordo NON deve rimettere in piedi il device: la misura fresca
    // batte la tabella. Senza questa guardia il device resta verde per minuti dopo
    // lo spegnimento — o non finisce in NESSUN bucket di presenza.
    // Due chiavi perché le tabelle sono indicizzate diversamente: per IP (ARP dei
    // router, lease) e per MAC (FDB, ND).
    const _provenAbsentIp = (ip) => !!(reach && ip && reach[ip] && reach[ip].absent === true);
    const absentMacs = new Set();
    for (const n of nodes) {
      if (trustAbsentNodeIds[n.id] && n.mac) absentMacs.add(normMac(n.mac));
    }
    // MAC→IP dalla tabella ARP completa del segmento (un MAC visto vivo a un IP è
    // presente; IP diverso dal documentato = cambio IP). Fallback ai soli IP
    // documentati raggiunti se non c'è ARP.
    // Un MAC può essere vivo su PIÙ IP contemporaneamente (alias eth0:1, dual-IP
    // di NAS/stampanti, multihoming): macAtIps li conserva TUTTI — il cambio-IP va
    // dichiarato solo se il documentato non è in nessuno. macAtIp (primo visto)
    // resta per compatibilità col contratto storico.
    const macAtIp = {};
    const macAtIps = {};
    const releasedMacs = {};   // { macLower: {ip, hostname} } — solo se leaseReleasedHint: lease RILASCIATO (sfumatura debole per drift-report)
    const _addLive = (mac, ip) => {
      // F7: la chiave è quella CANONICA (normMac), non un lowercase nudo. Il
      // giorno in cui una fonte emette 'AA-BB-CC-…' o la forma Cisco, un indice
      // costruito a mano smette di incontrare le altre chiavi e la presenza
      // muore in silenzio (device grigio per sempre).
      const k = normMac(mac);
      if (!k || !ip) return;
      if (!macAtIp[k]) macAtIp[k] = ip;
      if (!macAtIps[k]) macAtIps[k] = [];
      if (macAtIps[k].indexOf(ip) < 0) macAtIps[k].push(ip);
    };
    if (arp) {
      for (const ip of Object.keys(arp)) _addLive(arp[ip], ip);
    } else if (reach) {
      for (const ip of Object.keys(reach)) {
        const info = reach[ip];
        if (info && info.alive && info.mac) _addLive(info.mac, ip);
      }
    }
    // /24 OSSERVATI dalla sweep (fix falso "assente" cross-subnet).
    const observedSubnets = new Set();
    if (arp) { for (const ip of Object.keys(arp)) { const n = _net24(ip, prefixes); if (n) observedSubnets.add(n); } }
    // IP direttamente VIVI dalla sweep (ping/ARP/TCP), a PRESCINDERE dal MAC risolto.
    // DRIFT-M6: un IP documentato che risponde alla sweep NON è "cambiato" anche se
    // l'ARP non ne ha risolto il MAC (→ non entra in macAtIps). Serve a drift-report
    // per non dichiarare un falso "cambio IP" (e non farlo auto-applicare da autoIpRenew).
    const aliveIps = new Set();
    if (reach) { for (const ip of Object.keys(reach)) { if (reach[ip] && reach[ip].alive) { const n = _net24(ip, prefixes); if (n) observedSubnets.add(n); aliveIps.add(String(ip)); } } }
    const ports = {};
    for (const pid of Object.keys(docPorts)) {
      const pi = portsState[pid] || {};
      // `trunkVlans` della porta = la lista MISURATA (scritta da applyPollResult),
      // e vale solo se l'apparato dichiara la porta come trunk: su una access la
      // colonna resta popolata su certi vendor e non descrive niente.
      ports[pid] = { status: _pref(pi.status, null), speed: _pref(pi.speed, null), duplex: _pref(pi.duplex, null), vlan: _pref(pi.vlan, null),
                     trunkVlans: pi.isTrunk ? _vlanList(pi.trunkVlans) : null };
    }
    // Osservabilità L2: almeno una MAC-table (FDB) popolata? Senza, il motore non
    // dichiara nessuno "assente" (guardia macOrphan in drift-report.js).
    const fdbObserved = Object.values(fdb).some(tbl => tbl && Object.keys(tbl).length > 0);
    const observedMacs = [];
    // Guardia `absentMacs`: la MAC-table dello switch è un RICORDO con un tempo di
    // invecchiamento (vedi sopra) — non sopprime una prova di assenza appena misurata.
    for (const sw of Object.keys(fdb)) {
      for (const mac of Object.keys(fdb[sw] || {})) {
        if (absentMacs.has(normMac(mac))) continue;
        observedMacs.push(mac);
      }
    }
    // MAC di porta persistiti (state.ports[pid].mac = ifPhysAddress del device, salvato
    // nel JSON dall'ultimo poll riuscito) contano come "visto in rete" SOLO se il device
    // proprietario ha RISPOSTO in questo ciclo. Altrimenti sono stantii: un device spento
    // resterebbe "osservato" (verde) per sempre — DRIFT-A2, audit 2026-07-21. La presenza
    // reale resta comunque coperta dai segnali vivi (FDB/ARP/ND/sweep/lease).
    for (const pid of Object.keys(portsState)) {
      const mm = portsState[pid] && portsState[pid].mac;
      if (!mm) continue;
      const nodeId = pidApi && typeof pidApi.nodeIdOfPort === 'function'
        ? pidApi.nodeIdOfPort(pid, new Set(nodes.map(n => n && n.id).filter(Boolean)))
        : pid;
      if (responded[nodeId]) observedMacs.push(mm);
    }
    const known = new Set(knownSigs);
    const observedDevices = [];
    const seen = new Set();
    for (const sw of Object.keys(fdb)) {
      const swFdb = fdb[sw] || {};
      const swVlan = vlanCache[sw] || {};
      // MAC per porta: un uplink affollato (AP/hub/guest) raccoglie tanti MAC su
      // una sola ifName → tutti endpoint, non infrastruttura.
      const macsPerIf = countMacsPerPort(swFdb) || {};
      for (const mac of Object.keys(swFdb)) {
        const ifName = swFdb[mac];
        const sig = normMac(mac);
        if (!sig || seen.has(sig) || known.has(sig) || isVirtualMac(mac)) continue;
        seen.add(sig);
        observedDevices.push({
          sig, mac,
          label: fdbSeenLabel(sw, ifName),
          vlan: (swVlan[mac] != null) ? swVlan[mac] : null,
          portMacCount: macsPerIf[ifName] || 0,
          consumer: isRandomizedMac(mac),
        });
      }
    }
    // rejectedAutoLinks → MAC delle porte coinvolte (un device rifiutato non riappare)
    const rejectedSigs = [];
    for (const psig of rejectedAutoLinks) {
      for (const pid of String(psig).split('||')) { const mm = portsState[pid] && portsState[pid].mac; if (mm) rejectedSigs.push(normMac(mm)); }
    }
    const portDownStreak = {};
    const portAdminDown = {};
    for (const pid of Object.keys(docPorts)) {
      portDownStreak[pid] = (portsState[pid] && portsState[pid].downStreak) || 0;
      // `=== true` e non verita'/falsita': il campo ASSENTE vuol dire «non risulta»
      // (agente che non espone ifAdminStatus), non «accesa». Vedi lib/port-state.js.
      if (portsState[pid] && portsState[pid].adminDown === true) portAdminDown[pid] = true;
    }
    // ── Lease DHCP come FONTE (cross-VLAN) ───────────────────────────────
    // MAC→IP autorevole su TUTTE le VLAN (ciò che l'ARP locale non vede dietro un
    // firewall L3). G2: un lease scaduto/rilasciato non prova né presenza né IP.
    // G1: il lease prova la presenza del SUO MAC e dà l'IP corrente, ma NON marca
    // la subnet come "osservata" → un documentato senza lease resta non-verificabile.
    let leaseChecked = false;
    for (const l of leases) {
      const macLc = normMac((l && l.mac) || '');   // F7: chiave canonica, come ogni altro indice MAC
      const ip = String((l && l.ip) || '').trim();
      if (!macLc || !ip) continue;
      // Segnale DEBOLE opt-in: un lease RILASCIATO (il device ha mandato DHCPRELEASE, gesto
      // deliberato "me ne vado") è un INDIZIO, mai una prova. SOLO `state==='released'`, MAI da
      // `expiry` (un file lease importato vecchio marcherebbe di massa). NON tocca la presenza
      // (resta grigio, mai rosso): drift-report lo usa solo per annotare la voce unverified.
      // Il device è già stato dedup-ato per-MAC a monte → `released` = il suo lease più corrente.
      if (leaseReleasedHint && l && l.state === 'released') releasedMacs[macLc] = { ip, hostname: (l.hostname || '') };
      if (leaseStale(l)) continue;   // scaduto/rilasciato/declinato/free → nessun segnale di presenza
      // Stessa guardia delle altre tabelle: un lease è un RICORDO che dura ore. Se la
      // sweep ha PROVATO assente quell'IP sul nostro filo, il lease non lo rianima.
      // `leaseChecked` resta true (il file è stato letto: è osservabilità, non presenza).
      if (_provenAbsentIp(ip)) { leaseChecked = true; continue; }
      // ARP vivo ha priorità: il lease riempie solo i MAC senza segnale vivo.
      if (!macAtIp[macLc]) macAtIp[macLc] = ip;
      if (!macAtIps[macLc]) macAtIps[macLc] = [ip];
      observedMacs.push(l.mac);                    // il MAC col lease è "visto"
      const sig = normMac(l.mac);
      if (sig && !known.has(sig) && !seen.has(sig)) {
        seen.add(sig);
        observedDevices.push({ sig, mac: l.mac, label: leaseSeenLabel(ip, l.hostname), vlan: (l && l.vlan != null) ? l.vlan : null, portMacCount: 0, consumer: false });
      }
      leaseChecked = true;
    }

    // ── ARP dei router/switch L3 come FONTE cross-subnet (Fase 2, presenza onesta) ──
    // La ipNetToMedia/ipNetToPhysicalTable di un router prova che gli host sulle sue
    // VLAN sono VIVI (adiacenza L3) → verde SENZA che il server li pinghi, anche dietro
    // un firewall. Alimenta: presenza per-MAC (macAtIp/macAtIps → verde, e cambio-IP se
    // l'IP del router ≠ documentato), "visto in rete" (observedMacs) e marca la /24 come
    // OSSERVATA (a differenza del lease G1: qui il router RAGGIUNGE davvero quel segmento
    // → in Verifica la sua VLAN risulta "coperta"). NON marca reachabilityChecked (non è
    // una probe attiva; in Sync observedSubnets resta inerte, gate su sweepRan) e NON crea
    // non-documentati (resta questione di discovery). L'ARP vivo dello sweep ha priorità.
    if (snmpArp) {
      for (const mac of Object.keys(snmpArp)) {
        const ip = String(snmpArp[mac] || '').trim();
        if (!ip) continue;
        const nt = _net24(ip, prefixes);
        if (nt) observedSubnets.add(nt);                    // la VLAN dietro il router è osservata
        // LA MISURA BATTE IL RICORDO. Questa tabella è l'ARP che un router/switch ci ha
        // dato al SYNC: dice "quel MAC era a quell'IP", non "c'è adesso". Se la sweep di
        // ADESSO ha la prova dura che quell'IP non è più sul filo locale (ARP-miss dopo
        // il ping → reach[ip].absent), il ricordo NON deve resuscitare l'host: il
        // "segnale positivo vince sempre" nasce per i segnali VIVI, e qui faceva vincere
        // una cache stantia sopra una prova fresca. Effetto misurato prima del fix: un
        // device spento restava fuori da OGNI bucket di presenza (drift-report esce dal
        // ramo `liveIps` con un `continue`) → né rosso né grigio, colori pieni.
        // La /24 resta osservata: il router quel segmento lo raggiunge davvero.
        if (_provenAbsentIp(ip)) continue;
        _addLive(mac, ip);                                  // presenza per-MAC (gap-fill: lo sweep vivo vince)
        observedMacs.push(mac);                             // il MAC è "visto in rete" → presente
      }
    }

    // ── Neighbor Discovery IPv6 dei router/switch L3 (ND discovery, presenza onesta) ──
    // La ipNetToPhysicalTable IPv6 (vicini ND) di un router prova che un MAC è un vicino
    // VIVO dietro di lui → verde cross-subnet SENZA pingarlo, anche per un host IPv6-only
    // o il cui ARP IPv4 è invecchiato (li coglie dove la Fase 2 no). Gemello di snmpArp
    // ma PRESENCE-ONLY, di proposito: alimenta SOLO observedMacs (verde per-MAC). NON
    // chiama _addLive/macAtIps — un IPv6 di ND NON deve entrare nel rilevamento cambio-IP:
    // (1) cross-family, marcherebbe un nodo documentato con IPv4 come "cambio IP" verso un
    //     ip6 (drift-report confronta il doc-IP con la lista degli IP vivi del MAC);
    // (2) il cambio dell'ip6 PROPRIO è già gestito, in modo autorevole e col lucchetto,
    //     dalla ipAddressTable del device stesso (66ª) — l'ND del router è un segnale
    //     debole. E NON tocca observedSubnets: un /64 IPv6 non è una /24 (_net24 è IPv4).
    if (snmpNd) {
      // Stessa guardia del gemello ARP qui sopra: anche questa tabella è un RICORDO del
      // Sync. Qui la prova non si può appaiare per IP (l'ND porta ip6, la sweep misura
      // ip4), quindi si appaia per MAC del nodo: se la sweep ha provato assente l'IP
      // DOCUMENTATO di quel nodo (trustAbsentNodeIds), un suo vicino ND memorizzato non
      // lo rimette in piedi. Nessun dato ND nell'ambiente di prova (IPv4-only): guardia
      // simmetrica per costruzione, non verificata dal vivo.
      for (const mac of Object.keys(snmpNd)) {
        if (absentMacs.has(normMac(mac))) continue;
        observedMacs.push(mac);   // vicino ND vivo → "visto in rete" → presente (verde)
      }
    }

    return {
      responded, ports, observedMacs, observedDevices, rejectedSigs, portDownStreak, portAdminDown,
      fdbObserved, presentNodeIds, trustAbsentNodeIds, reachabilityChecked: reachabilityChecked || leaseChecked,
      macAtIp, macAtIps, observedSubnets: [...observedSubnets], aliveIps: [...aliveIps], releasedMacs,
      measuredIds,
    };
  }

  return { buildDocSnapshot, buildSnmpSnapshot };
});
