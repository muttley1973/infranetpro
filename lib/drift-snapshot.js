...
(function (root, factory) {
  ...
  function buildSnmpSnapshot(model) {
    ...
    // /24 la cui L2 è realmente COPERTA dalla FDB bridge raccolta. Fix del falso
    // "assente" MULTI-FABRIC: la FDB degli switch di un fabric NON vede i MAC di un
    // altro dominio L2 raggiungibile solo a L3 (es. una LAN reale 192.168.x separata
    // dal lab 10.10.x da un router). Senza questo, `fdbObserved` faceva credere al
    // motore che gli switch vedessero i MAC di TUTTE le subnet → ogni device della
    // subnet non-osservata veniva dichiarato assente (grigio) invece di
    // "non-verificabile". Una subnet è coperta se (a) è quella di uno switch di cui
    // abbiamo la FDB bridge (siamo nel suo dominio L2) o (b) contiene un device
    // documentato il cui MAC compare in una FDB bridge (l'abbiamo visto a L2). Il
    // motore ricade sul comportamento storico (FDB copre tutto) se questo campo manca.
    const fdbSubnets = new Set();
    if (fdbObserved) {
      const nById = {};
      for (const n of nodes) nById[n.id] = n;
      const _nodeNet = n => _net24(((n.integration || {}).host || n.ip || '').trim());
      for (const swId of Object.keys(fdb)) {
        if (!fdb[swId] || !Object.keys(fdb[swId]).length) continue;   // (a) switch con FDB bridge
        const sn = nById[swId];
        const net = sn ? _nodeNet(sn) : null;
        if (net) fdbSubnets.add(net);
      }
      const fdbMacSet = new Set();                                    // MAC APPRESI da un bridge (non i MAC-porta propri)
      for (const sw of Object.keys(fdb)) for (const mac of Object.keys(fdb[sw] || {})) fdbMacSet.add(normMac(mac));
      for (const n of nodes) {                                        // (b) device documentato visto a L2
        const macN = normMac(n.mac);
        if (!macN || !fdbMacSet.has(macN)) continue;
        const net = _nodeNet(n);
        if (net) fdbSubnets.add(net);
      }
    }
    ...
    return {
      ...
      macAtIp, macAtIps, observedSubnets: [...observedSubnets], fdbSubnets: [...fdbSubnets],
    };
  }
