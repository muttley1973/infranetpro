...
(function (root, factory) {
  ...
  // nodi lungo il percorso (estremi inclusi). Ritorna { ok, warnings:[{code,params}] }.
  // Nessuna PAROLA qui dentro: il codice dice COSA non torna, la frase la mette
  // il renderer via i18n (chiavi `chn.*`) — una lib pura non parla una lingua.
  /** @param {string[]} types @returns {{ok:boolean, warnings:{code:string,params?:object}[]}} */
  function validateCablingChain(types) {
    ...
    if (n > 6) warnings.push({ code: 'too-long', params: { n } });
    ...
    if (a === 0 && b === 0) warnings.push({ code: 'both-endpoints' });
    ...
    if ((_workArea(a) && _transit(b)) || (_transit(a) && _workArea(b))) {
      warnings.push({ code: 'incomplete-chain' });
    }
    ...
    if (s !== n) warnings.push({ code: 'non-monotone' });
