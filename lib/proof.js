...
(function (root, factory) {
  ...
  function cableProof(l, srcProof, dstProof, now) {
    ...
    // 1) DICHIARATO — legge, salvo CONTRADDIZIONE. Un estremo muto non lo tocca (il
    //    silenzio di un device non dice niente sul cavo), ma due cose sì:
    //      · 'declared-review' — il vicino osservato contraddice QUESTO cavo, o
    //        l'estremo è 'diverged': il documento è sbagliato;
    //      · 'declared-shut'   — la porta sotto è SPENTA A MANO (`l.portShut`, da
    //        ifAdminStatus). Non è un sintomo: è una decisione scritta sull'apparato,
    //        e vale quanto la tua — due dichiarazioni che si contraddicono. Lì a
    //        misurare è lo switch, autorevole sulla propria porta.
    //    Nessuno dei due è 'ghost': quella parola vuol dire «l'ho dedotto io e
    //    l'evidenza è evaporata», e su un cavo che hai visto tu è falsa.
    if (!l || !l.autoLinked) {
      if (worst === 'diverged' || miscabled) return 'declared-review';
      return l && l.portShut ? 'declared-shut' : 'declared';
    }

    // 2) DEDOTTO — dipende dall'evidenza. Contraddizioni/perdita d'evidenza → fantasma:
    //    miscablaggio · porta di accesso DOWN da >=N sync (l.portDown, dal bucket Drift
    //    ghostCable — copre il multi-homed: la porta è giù anche se il device risponde
    //    per altra via) · estremo non provato · confidence decaduta.
    if (miscabled || l.portDown || worst === 'absent' || worst === 'unverified' || worst === 'diverged') return 'ghost';
  ...
  //
  // 'declared-shut' cade in 'diverged' e non in 'toVerify': una porta spenta a mano
  // sotto un cavo dichiarato non e' un dubbio, sono due DICHIARAZIONI che si
  // contraddicono — la tua e quella scritta sull'apparato. Stesso genere di fatto di
  // un miscablaggio.
  ...
  function truthScore(elements, _now) {
