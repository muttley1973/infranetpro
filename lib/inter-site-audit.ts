...
interface InterSiteNotChecked {
  check: string;
  reason: string;
}

/** L'esito dell'audit. Liste per nome, come in `ipam-audit`; la glue racconta. */
interface InterSiteAudit {
  // ── incoerenze: qualcosa NON torna ──
  ...
  // ── lacune: niente è rotto, ma non sai rispondere ──
  ...
  // ── ciò che non è stato esaminato, e perché (①) ──
  notChecked: InterSiteNotChecked[];
}

(function (root, factory) {
  const isNode = typeof module !== 'undefined' && !!module.exports;
  const api = factory(isNode ? require('./inter-site.ts') : root);
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  if (typeof window !== 'undefined') Object.assign(window, api);
})(typeof self !== 'undefined' ? self : globalThis, function (IS: any) {
  'use strict';

  const _sortedKeys = (o: Record<string, unknown>): string[] => Object.keys(o).sort();
   ...
   */
  function buildInterSiteAudit(rawOrg: unknown): InterSiteAudit {
    const org = IS.normalizeOrganization(rawOrg);
    const sites = org.sites as { id: string; role: string; subnets: string[] }[];
    const links = org.links as { id: string; kind: string; aSiteId: string; bSiteId: string; reach: unknown }[];
    const uplinks = org.uplinks as { id: string; siteId: string; publicIp: unknown }[];

    const notChecked: InterSiteNotChecked[] = [];
    ...
    if (!anySiteSubnet) {
      ...
      const idx = IS.subnetSiteIndex(org) as Record<string, string[]>;
    ...
    if (!anyLink) notChecked.push({ check: 'subnetsNowhere', reason: 'no-links' });
      ...
      const hits: Record<string, { linkId: string; siteId: string }[]> = Object.create(null);
      for (const l of links) {
        const r = IS.linkReach(l);
        for (const [end, siteId] of [['a', l.aSiteId], ['b', l.bSiteId]] as [string, string][]) {
          for (const n of (r as Record<string, string[]>)[end]) {
    ...
    if (!anyHub) notChecked.push({ check: 'spokesWithoutHub', reason: 'no-hub' });
      ...
      for (const s of sites) {
        ...
        const toHub = IS.linksOfSite(org, s.id)
          .some((l: unknown) => hubs.has(IS.linkPeerSite(l, s.id) as string));
    ...
    if (!anyUplink) {
      ...
      for (const u of uplinks) {
        ...
        if (!u.publicIp) audit.uplinksWithoutPublicIp.push({ uplinkId: u.id, siteId: u.siteId });
      }
    }
   ...
   */
  function interSiteAuditCounts(audit: InterSiteAudit): { problems: number; gaps: number; notChecked: number } {
    ...
    const GAPS = ['subnetsNotCarried', 'linksWithoutReach', 'sitesWithoutLink',
      'sitesWithoutUplink', 'uplinksWithoutPublicIp'];
    const n = (keys: string[]) => keys.reduce(
      (acc, k) => acc + (((audit as unknown as Record<string, unknown[]>)[k] || []).length), 0);
    return {
      problems: n(PROBLEMS),
      gaps: n(GAPS),
