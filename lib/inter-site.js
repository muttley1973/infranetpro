...
//     ⚠️ `cirMbps` fa ECCEZIONE dal 31/08: porta la banda della PORTA, che un
//        apparato la sa dire (è `ifSpeed`) e che l'import DCIM legge da
//        `port_speed`. Resta senza envelope — aggiungerglielo è una modifica al
//        modello, non una nota — e finché non ce l'ha, chi la legge non sa se
//        l'ha scritta una persona o l'ha portata un import.
//
 ...
 *  ⚠️ `cirMbps` porta la **banda della PORTA**, ed è così che la chiamano il
 *  pannello e il dossier (decisione dell'utente, 2026-08-31). Il nome del campo
 *  è storico — nasceva come banda contrattuale — e non è stato cambiato per non
 *  migrare i valori già scritti. L'import DCIM è allineato: legge `port_speed`
 *  sulla terminazione di sede e **non** il `commit_rate` del circuito.
 *  ⚠️ Non c'è oggi nessun campo per la banda del CONTRATTO: chi la vuole
 *  documentare ha solo le note. Aggiungerlo è una decisione di prodotto — e chi
 *  la prende ricordi che qui i due numeri sono davvero diversi (misurato: 100
 *  Mbps di contratto su una porta da 1000).
 *
...
(function (root, factory) {
  ...
  const api = factory(
    ...
    isNode ? require('./user-text.js') : root
  );
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  if (typeof window !== 'undefined') Object.assign(window, api);
})(typeof self !== 'undefined' ? self : globalThis, function (cidr, prov, refs, testo) {
  'use strict';
  ...
  /**
   * Ogni stringa dell'organizzazione passa di qui — nomi di sede, etichette,
   * provider, note — quindi è QUI che vale la forma di una scritta d'utente:
   * senza caratteri di controllo, con un tetto (`lib/user-text.js`).
   * Misurato prima: un nome da 2 MB entrava intero, e `organization.json`
   * diventava un file da 4.096 KB — riletto e ri-normalizzato a ogni GET.
   * ⚠️ Vale anche in LETTURA, perché `normalizeOrganization` normalizza in tutt'e
   * due i versi: un file scritto a mano, o da una versione precedente, si
   * raddrizza da sé invece di restare una mina.
   * @param {unknown} v @returns {string|null}
   */
  function _str(v) {
    if (v == null) return null;
    const s = testo.cleanUserText(v);
    return s ? s : null;
  }
  ...
  /**
   * La chiave con cui due grafie sono lo STESSO operatore.
   *
   * Regola di casa: mai un confronto per stringa. «TIM», «tim» e « TIM  Italia »
   * scritti in tre righe diverse sono un operatore solo, e questo è ciò che li fa
   * combaciare — il gemello di `addrKey` per gli indirizzi e di `macKey` per i MAC.
   * Non normalizza *dentro* il documento: serve a riconoscere, non a riscrivere.
   */
  ...
  /**
   * Gli operatori già scritti in questo documento, una volta ciascuno.
   *
   * A che serve: «Operatore» è un campo di testo libero, e **resta** libero —
   * l'elenco degli operatori del mondo non ce l'ha nessuno, e chiuderlo renderebbe
   * indocumentabile il primo provider locale che si presenta. Ma riscrivere «TIM»
   * a mano su ogni linea è esattamente come nascono tre grafie dello stesso
   * operatore, e da lì in poi il dossier e l'audit ne contano tre. Questo dice
   * cosa c'è già, da suggerire: chi sceglie prende la grafia che esiste, chi ha un
   * nome nuovo lo scrive.
   *
   * ⚠️ La grafia che sopravvive è la PRIMA incontrata, non la chiave: come si
   * scrive il nome del proprio fornitore lo decide chi documenta, non la
   * normalizzazione. La chiave serve solo a non proporre due volte la stessa cosa.
   *
   * Le fonti sono le linee WAN **e** i collegamenti, perché `provider` è lo stesso
   * campo nei due pannelli: un elenco che ne guardasse solo metà ometterebbe nomi
   * che l'utente ha scritto lui, in questo stesso documento.
   *
   * Ordine alfabetico sulla chiave — deterministico, come le coordinate della
   * mappa: la stessa organizzazione dà lo stesso elenco a ogni apertura. Non per
   * frequenza, che rimescolerebbe la tendina a ogni riga aggiunta.
   */
  /** @param {Organization|null} org @returns {string[]} */
  function knownProviders(org) {
    /** @type {Map<string, string>} */
    const visti = new Map();
    /** @param {{provider?:unknown}[]|null|undefined} righe */
    const raccogli = (righe) => {
      for (const r of (righe || [])) {
        const nome = String((r && r.provider) == null ? '' : r.provider).trim();
        if (!nome) continue;
        const k = providerKey(nome);
        if (!visti.has(k)) visti.set(k, nome);
      }
    };
    raccogli(org && org.uplinks);
    raccogli(org && org.links);
    return Array.from(visti.keys()).sort().map(k => visti.get(k) || '');
  }
