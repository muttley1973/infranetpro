...
(function (root, factory) {
  ...
  // DISC-M4 (audit 2026-07-21): i valori TXT arrivano da un responder mDNS NON fidato e
  // finiscono nei nomi nodo → togli i caratteri di controllo C0 (0x00-0x1f) + DEL (0x7f)
  // che romperebbero UI/export. Il match dei control-char è VOLUTO (sanitizzazione).
  ...
  function _parseTxtInto(buf, start, rdlen, out) {
    ...
    while (o < end) {
      ...
      const eq = s.indexOf('=');
      if (eq > 0) {
        const k = s.slice(0, eq).toLowerCase().replace(_CTRL_RE, '');
        if (k) out[k] = s.slice(eq + 1).replace(_CTRL_RE, '');
      }
    }
  }
