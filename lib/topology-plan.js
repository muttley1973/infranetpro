...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('./netnames'), require('./port-id.js'), require('./mac-class.js'), require('./cidr.js'));
  } else {
    Object.assign(root, factory(root, root, root, root)); // browser: già su window
  }
})(typeof self !== 'undefined' ? self : this, function (nn, pidApi, mc, cidr) {
  'use strict';
  ...
  function _nodeIdOfPort(pid, meta) {
    if (meta && meta.nodeId) return meta.nodeId;
    return pidApi && typeof pidApi.nodeIdOfPort === 'function' ? pidApi.nodeIdOfPort(pid) : String(pid || '');
  }
  ...
  function _portMetaMap(portIndex) {
    ...
    for (const nodeId of Object.keys(ports)) {
      for (const e of (ports[nodeId] || [])) {
        m[e.portId] = { nodeId, lagId: e.lagId || 0, lagGroup: e.lagGroup || '' };
      }
    }
    return m;
  }
  ...
  function _portLagRef(portId, meta) {
    const nodeId = _nodeIdOfPort(portId, meta);
  ...
  function _logicalPairKey(src, dst, portMetaMap) {
    ...
    const sn = _nodeIdOfPort(src, portMetaMap[src]);
    const dn = _nodeIdOfPort(dst, portMetaMap[dst]);
  ...
  // MAC virtuale / randomizzato: la risposta la dà lib/mac-class.js, che è la
  // sola definizione nel progetto (audit D3). Qui c'era una COPIA — lista OUI
  // propria + un `_normMac6` che sostituiva solo i trattini: un MAC in forma
  // Cisco `aabb.ccdd.eeff` non veniva quindi MAI riconosciuto né virtuale né
  // randomizzato, in silenzio. Le due liste OUI sono state fuse in mac-class.
  const _isVirtualOui = (mac) => mc.isVirtualMac(mac);
  const _isRandomizedMac = (mac) => mc.isRandomizedMac(mac);
  // Stesso segmento? La definizione è in lib/cidr (`segmentKey`): col prefisso
  // DICHIARATO si usa la maschera vera, senza resta la /24 assunta. Qui il
  // confine decide se proporre «router» (audit D2): su una /22, due indirizzi
  // della stessa rete sembravano cross-subnet e nasceva un router inventato.
  function _subnet24(ip, prefixes) { return cidr.segmentKey(ip, prefixes) || ''; }
   ...
   * @param {Array} prefixes - reti DICHIARATE (opzionale): danno la maschera vera
   *        al confronto di segmento invece della /24 assunta.
   */
  function classifyIntermediary(macs, srcIp, prefixes) {
    const list = (macs || []).filter(Boolean);
    const n = list.length || 1;
    // ROUTER — un endpoint documentato in una subnet DIVERSA dallo switch = confine L3.
    const sSub = _subnet24(srcIp, prefixes);
    if (sSub) {
      for (const m of list) { const iSub = _subnet24(m.ip, prefixes); if (iSub && iSub !== sSub) return { type: 'router', signal: 'cross-subnet' }; }
    }
  ...
  function inferUnmanagedNodes(portEndpoints, opts) {
    ...
    for (const pe of (portEndpoints || [])) {
      ...
      const cls = classifyIntermediary(macs, pe.srcIp, o.prefixes);
      out.push({
  ...
  function buildTopologyPlan(inputs) {
    ...
    for (const g of groups.values()) {
      ...
      links.push({
        ...
        reason: { code: tier, vars: {
          protocol: g.protocol,
          srcNode: _nodeIdOfPort(g.src, portMetaMap[g.src]), dstNode: _nodeIdOfPort(g.dst, portMetaMap[g.dst]),
    ...
    if (inferredNodes.length) {
      ...
      for (let i = links.length - 1; i >= 0; i--) {
        ...
        if ((upSrc && upSrc.has(_nodeIdOfPort(l.dst, portMetaMap[l.dst]))) ||
            (upDst && upDst.has(_nodeIdOfPort(l.src, portMetaMap[l.src])))) {
          links.splice(i, 1);
        }
      }
    }
    ...
    if (tiers.inferred) {
      for (const inf of (p.inferredNodes || [])) {
        ...
        for (const l of existing) {
          ...
          if ((l.src === inf.onUplinkPid && behindSet.has(_nodeIdOfPort(l.dst, portMetaMap[l.dst]))) ||
              (l.dst === inf.onUplinkPid && behindSet.has(_nodeIdOfPort(l.src, portMetaMap[l.src])))) pruneIds.add(l.id);
        }
      }
    }
    ...
    if (resolveEp) {
      for (const l of existing) {
        ...
        for (const end of [l.src, l.dst]) {
          const nid = _nodeIdOfPort(end, portMetaMap[end]);
