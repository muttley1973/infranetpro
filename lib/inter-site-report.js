...
(function (root, factory) {
  ...
  function buildInterSiteWanReport(org, opts) {
    ...
    // ㉗ `scope` viaggia come CODICE, non come parola: il modello del dossier e'
    // indipendente dalla lingua, e chi stampa ha il dizionario. Vale per la sola
    // riga che ce l'ha; su tutte le altre resta `null`.
    /** @type {{check:string, group:string, subject:string|null, note:string|null, scope:string|null}[]} */
    const findings = [];
    for (const [group, chiavi] of [['problem', AUD.INTER_SITE_AUDIT_PROBLEMS],
      ...
      for (const check of chiavi) {
        for (const r of (rilievoDi[check] || [])) {
          findings.push({ check, group, subject: soggettoDi(r), note: notaDi(r), scope: r.scope || null });
        }
      }
    }
    const audit = {
