...
(function (root, factory) {
  ...
  const POWER_OIDS = {
    ...
    // ⚠️ PROFILO VENDOR-SPECIFICO (APC PowerNet). È l'unica eccezione della
    // codebase in cui OID proprietari NON stanno accanto a uno standard ma AL SUO
    // POSTO — perché per gli ATS uno standard equivalente alla RFC 1628 non esiste.
    // Chi non parla PowerNet non risponde: `parseAts` lo dichiara con `answered`,
    // invece di lasciare che il silenzio somigli a una lettura riuscita.
    ats: {
  ...
  // ⚠️ Il profilo ATS è VENDOR-SPECIFICO (APC PowerNet) ed è l'unico percorso che
  // abbiamo: per gli ATS non esiste un equivalente della RFC 1628 che il ramo UPS
  // qui sopra usa per tutti. Un ATS Eaton/Vertiv/Socomec quindi non risponde a
  // questi OID — e prima restava SILENZIOSAMENTE muto: tre campi a `null`,
  // indistinguibili da un apparato che risponde «non lo so» su tutti e tre.
  //
  // `answered` dice se ALMENO un OID del profilo ha risposto. Non aggiungiamo OID
  // di altri vendori a indovinare (sarebbe la violazione ② che stiamo chiudendo:
  // senza datasheet né hardware su cui provarli non sono una misura, sono una
  // speranza) — ma il silenzio smette di somigliare a una lettura riuscita, e
  // l'interfaccia può dire «non leggibile con questo profilo» invece di niente.
  function parseAts(raw) {
    ...
    return {
      ...
      profile:         'apc-powernet',
      answered:        sel != null || red != null || oc != null,
    };
  }
