...
(function (root, factory) {
    ...
    function sanitizeSvg(svgText) {
        ...
        // handler eventi inline: on*="..." / on*='...'
        if (/\son[a-z]+\s*=\s*("[^"]*"|'[^']*')/gi.test(s)) {
            removed.push('event-handlers');
            s = s.replace(/\son[a-z]+\s*=\s*("[^"]*"|'[^']*')/gi, '');
        }

        // javascript: URI ovunque in un attributo
        if (/javascript:/i.test(s)) {
            removed.push('javascript-uri');
            s = s.replace(/(=\s*["'])\s*javascript:[^"']*(["'])/gi, '$1#$2');
        }

        // riferimenti ESTERNI in href/xlink:href/src: tieni solo i locali `#...`.
        // Toglie l'attributo (e il suo valore) quando NON inizia per `#`.
