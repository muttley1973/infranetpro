...
(function (root, factory) {
    ...
    // Tetto sulla dimensione dell'artwork. MISURATO, non stimato: la skin piu'
    // grande dell'archivio pesa 110 KB, le altre 8 KB — mezzo mega e' quattro
    // volte la piu' pesante, quindi non dice di no a nessun disegno vero. Senza
    // tetto un SVG da 1,5 MB entrava (provato: 201), finiva su disco e poi in
    // OGNI `GET /api/skins`, che le restituisce TUTTE con l'artwork dentro: una
    // libreria di skin gonfie si paga a ogni apertura dell'app, per sempre.
    var MAX_SVG_LEN = 512 * 1024;
    ...
    function sanitizeSvg(svgText) {
        ...
        // vbscript: URI (SEC-M4): stesso vettore di javascript:, gestito PRIMA
        // dell'external-ref-strip così il valore viene neutralizzato a #.
        if (/vbscript:/i.test(s)) {
            removed.push('vbscript-uri');
            s = s.replace(/(=\s*["'])\s*vbscript:[^"']*(["'])/gi, '$1#$2');
        ...
        // SEC-M4 (audit 2026-07-21): CSS injection nei blocchi <style> e negli attributi
        // style="". Vettori che l'href/src-strip non copre:
        //  - url(...) verso risorse ESTERNE / data:/javascript: (in fill:url(...), background,
        //    @font-face…). url(#id) resta: è il riferimento LOCALE lecito di SVG (gradient/mask).
        //  - expression(...): funzione CSS legacy che esegue JS (IE) → neutralizzata.
        //  - @import: importa un foglio di stile ESTERNO → rimosso.
        // Chiude anche la forma entity-encoded di javascript:: se sopravvive dentro un url()
        // (non locale) viene tolta qui; in href/src era già rimossa dall'external-ref-strip.
        if (/expression\s*\(/i.test(s)) { removed.push('css-expression'); s = s.replace(/expression\s*\(/gi, 'expr('); }
        if (/@import/i.test(s))         { removed.push('css-import');     s = s.replace(/@import/gi, '/*import*/'); }
        var cssUrl = /url\(\s*(["']?)(?!\s*#)[^)"']*\1\s*\)/gi;   // url(...) NON locale (#id)
        if (cssUrl.test(s)) { removed.push('css-url'); cssUrl.lastIndex = 0; s = s.replace(cssUrl, 'url(#)'); }
    ...
    function parsePanelSkin(svgText, meta) {
        ...
        var raw = String(svgText == null ? '' : svgText);
        // Il tetto si controlla PRIMA di sanitizzare: sanitizzare mezzo mega di
        // stringa con otto regex e' gia' il lavoro che il tetto esiste per evitare.
        if (raw.length > MAX_SVG_LEN) {
            return {
                ok: false,
                error: 'SVG troppo grande (' + Math.round(raw.length / 1024) + ' KB, massimo ' + (MAX_SVG_LEN / 1024) + ' KB).',
                errorCode: 'too-large', face: face,
                viewBox: null, width: 0, height: 0, svg: '',
                ports: [], counts: { data: 0, mgmt: 0 }, warnings: [], removed: []
            };
        }
        var sane = sanitizeSvg(raw);
        var s = sane.svg;
        var warnings = [];
        ...
        // Un blocco <style> NON viene tolto — e va detto perche' e' una scelta, non
        // una dimenticanza. Illustrator e Inkscape esportano le campiture proprio
        // come classi in un <style>: buttarlo renderebbe grigia meta' delle skin
        // vere, per chiudere un vettore che richiede comunque un account admin (il
        // POST /api/skins e' admin-only). Ma quel CSS, una volta che l'SVG e'
        // inline nella pagina, vale per l'INTERA pagina, non solo per il disegno:
        // un selettore `body`/`*` la ridipinge tutta. Quindi manual-first: si DICE
        // a chi carica, e decide lui. I vettori eseguibili dentro al blocco
        // (url() esterni, @import, expression) restano neutralizzati da sanitizeSvg.
        if (/<style\b/i.test(s)) {
            warnings.push('la skin contiene un blocco <style>: le sue regole CSS valgono per l\'intera pagina, non solo per il disegno');
        }
    ...
    return {
        ...
        PANEL_SKIN_MAX_LEN: MAX_SVG_LEN
    };
});
