...
(function (root, factory) {
  ...
  // Il NOME di una presa radio, risolto DAL PID. `radios[idx].label` è ciò che
  // l'utente scrive nel pannello (o che l'import DCIM copia dal nome
  // dell'interfaccia): è l'UNICA definizione di «come si chiama questa radio».
  // Chi compone un'etichetta la chiama invece di tagliare il pid a fette — il
  // suffisso `radio2` è un dettaglio interno, non un nome.
  // null = non è un pid radio, non è di questo nodo, o la radio non ha nome.
  /** @param {NetNode|null|undefined} node @param {string} pid @returns {string|null} */
  function radioLabelForPid(node, pid) {
    const p = parseRadioPid(pid);
    if (!p || !node || node.id !== p.nodeId || !Array.isArray(node.radios)) return null;
    const r = node.radios[p.idx];
    const lbl = (r && r.label != null) ? String(r.label).trim() : '';
    return lbl || null;
  }
  ...
  return {
    ...
    radioPid, parseRadioPid, radioLabelForPid, linkKind, radioCount,
