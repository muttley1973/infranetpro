(function (root, factory) {
  ...
  function pduManagementPortCount(node, fallback = 1) {
    ...
    // ONE Ethernet management port. Fall back to the declared port count only when
    // it is a POSITIVE number: a PDU imported with no data ports (n.ports === 0, a
    // finite value) must not collapse the management count to zero and hide the
    // cable-able port. `n.ports` counts data/front ports, not management — default
    // to 1 whenever it isn't a usable positive count.
