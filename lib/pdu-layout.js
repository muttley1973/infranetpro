(function (root, factory) {
  ...
  // ⚠️ `spec` PRIMA del livello nodo. È la precedenza di lib/hw-capabilities.js
  // (`_val`), di lib/stack.js e di lib/ha-pair.js, ed è l'unica coerente con
  // `_compactNodeSpec` (src/app-types.js), che al caricamento tiene `spec` e
  // CANCELLA la copia sul nodo. Finché qui il nodo veniva prima, un documento con
  // le due copie divergenti — «Applica modello» scriveva sul nodo, il pannello in
  // `spec` — mostrava un numero di prese nella griglia del rack e un altro in
  // Panoramica, e al ricaricamento la griglia cambiava da sola (audit
  // 2026-08-20, difetto C2). Il livello nodo resta come RIPIEGO per i documenti
  // che non sono ancora passati da una compattazione.
  function nodeField(node, key) {
    if (!node || typeof node !== 'object') return undefined;
    const spec = node.spec;
    if (spec && typeof spec === 'object' && spec[key] !== undefined) return spec[key];
    return node[key];
  }
