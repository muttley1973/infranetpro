...
(function (root, factory) {
  ...
  const CATALOG = {
    ...
    // Il tenant ENTRA (come provenienza sul nodo) ma nessuna schermata lo mostra:
    // non e' una perdita, e' un limite — e chiamarlo perdita sarebbe altrettanto
    // falso che tacerlo.
    'device.tenantSkipped': { severity: 'info' },
  ...
  function buildDecisions(report, chosen) {
    ...
    for (const issue of issues) {
      ...
      if (!g) {
        g = { code, count: 0, devices: [], models: [], roles: [], kinds: [], cables: [], data: {}, _models: new Set(), _roles: new Set(), _kinds: new Set(), _seen: new Set() };
        byCode.set(code, g);
      }
      g.count++;
    ...
    for (const g of byCode.values()) {
      ...
      delete g._models; delete g._roles; delete g._kinds; delete g._seen;
      const row = {
        code: g.code,
