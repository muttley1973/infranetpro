...
(function (root, factory) {
  ...
  const CATALOG = {
    ...
    // Lo stesso CIDR dichiarato più volte (VRF diverse, o con e senza VLAN):
    // qui lo spazio dei prefissi è piatto e le righe si fondono. 'info' e non
    // 'loss' perché non si perde una rete — si perde la distinzione fra due
    // dichiarazioni che InfraNet non saprebbe comunque tenere separate.
    'prefix.collapsed': { severity: 'info' },
    ...
    // Il campo «indirizzo» della porta ne tiene UNO, IPv4. Un secondo indirizzo
    // sulla stessa interfaccia, un IPv6, o un indirizzo su un'interfaccia che nel
    // documento non ha una porta restano fuori DAVVERO: 'loss', come le altre
    // righe che dichiarano qualcosa di NetBox che il modello non sa tenere.
    'ip.portExtra': { severity: 'loss' },
    // Questa dice cosa ENTRA, non cosa resta fuori: le prenotazioni dell'IPAM
    // diventano occupazione della rete, così «prossimo IP libero» smette di
    // proporre un indirizzo già impegnato. Nessuna scelta da fare → 'info'.
    'ip.reserved': { severity: 'info' },
    // I ruoli IPAM. La SCELTA (quale ruolo è la VLAN di gestione, quale la voce…)
    // non passa da qui: non è un'alternativa fra due comportamenti del motore, è
    // un abbinamento fra due vocabolari, e vive come riga a tendina nell'anteprima
    // — stessa forma dei «tipi da confermare». Qui stanno solo i suoi esiti.
    'vlanRole.applied': { severity: 'info' },
    // Non è una perdita — il documento tiene entrambe — ma è quasi sempre un
    // abbinamento sbagliato, e va guardato prima di premere Importa.
    'vlanRole.conflict': { severity: 'choice' },
    'vlanRole.nativeMany': { severity: 'choice' },
    // Un ruolo che sta solo sulle reti non ha una VLAN a cui appendersi: confine
    // dichiarato, non perdita — nel modello non c'è dove metterlo.
    'vlanRole.prefixOnly': { severity: 'info' },
    // Il wireless che ENTRA: radio, banda, canale, SSID, cifratura. 'info'.
    'wifi.imported': { severity: 'info' },
    // Questa invece è una LETTURA, non un dato: NetBox dice «personal» e non se
    // è WPA2 o WPA3. Si scrive WPA2 e si dichiara — 'choice', perché c'è
    // qualcosa da controllare e semmai correggere.
    'wifi.securityAssumed': { severity: 'choice' },
    'wifi.radiosCapped': { severity: 'loss' },
    // Il canale largo: la banda entra, il numero no perché non è confrontabile.
    // 'choice' e non 'loss': c'è qualcosa da fare — scegliere il primario.
    'wifi.wideChannel': { severity: 'choice' },
    // Le macchine virtuali che ENTRANO, con la conversione delle unità detta a
    // voce alta nel testo: 'info'.
    'vm.imported': { severity: 'info' },
    // Le due righe che sono LETTURE e non dati — stessa famiglia di
    // `wifi.securityAssumed`: 'choice' senza alternative da spuntare, cioè
    // «questo l'ho dedotto io, controllalo».
    'vm.viaCluster': { severity: 'choice' },
    'vm.hostRetyped': { severity: 'choice' },
    // VM dichiarate in NetBox che nessun host importato rivendica: restano
    // fuori davvero → 'loss', in cima.
    'vm.noHost': { severity: 'loss' },
    // Il confine, non una perdita: le VM di apparati e cluster che questo
    // import non tocca. Senza questa riga «nessuna VM» si legge come un guasto.
    'vm.outOfScope': { severity: 'info' },
    'vm.addrExtra': { severity: 'loss' },
    'pdu.outletsCapped': { severity: 'info' },
    'pdu.mgmtCapped': { severity: 'info' },
    'rack.missing': { severity: 'info' },
    // Le stanze nate dalle ubicazioni. 'choice' perche' la geometria e' una
    // NOSTRA scelta — NetBox non ha ne' posizione ne' dimensione — e l'utente
    // puo' rifarla a mano: e' esattamente il tipo di riga che chiede uno sguardo.
    'location.rooms': { severity: 'choice' },
    // Chi non ha un'ubicazione resta fuori dalle stanze: non e' una perdita
    // (l'apparato c'e'), e' un confine da sapere.
    'location.none': { severity: 'info' },
  };
  ...
  // Chiavi che hanno GIÀ una loro strada nella riga e non devono ricadere fra i
  // numeri di contesto. `n` soprattutto: nel pannello `{n}` è il numero di casi
  // RAGGRUPPATI (`row.count`), e il `n` del singolo avviso lo sovrascriverebbe —
  // «1 VLAN portano più di una rete» al posto del conteggio vero.
  const _STRUCTURAL = new Set([
    'code', 'n', 'deviceId', 'deviceName', 'model', 'role', 'kind', 'cableId', 'sample', 'mixed',
  ]);
  ...
  function buildDecisions(report, chosen) {
    ...
    for (const issue of issues) {
      ...
      // Un avviso può valere per SÉ — uno per apparato, e `ports.overTemplate` ne
      // emette tredici, uno a testa — oppure DICHIARARE quanti casi rappresenta in
      // `n`, quando il mapper ne emette uno solo per tutta la lettura (i tre avvisi
      // sull'IPAM: reti senza VLAN, VLAN con più reti, IP non agganciati).
      // ⚠️ Contare le occorrenze in entrambi i casi faceva dire «Un indirizzo IP
      // resta fuori» quando erano 180, e «Una VLAN porta più di una rete» sotto un
      // elenco che ne mostrava tre: il singolare/plurale sceglie su questo numero.
      g.count += (typeof issue.n === 'number' && Number.isFinite(issue.n) && issue.n > 0) ? issue.n : 1;
      ...
      // ⚠️ Qui c'era un ELENCO CHIUSO di chiavi ammesse, e un emettitore più nuovo
      // dell'elenco perdeva le sue: `prefix.multiPerVlan` manda `total`, il titolo lo
      // usa («…{total} in tutto»), e a schermo compariva la GRAFFA GREZZA. Nessun
      // test poteva vederlo — il motore era corretto, il testo era corretto, si
      // perdevano nel mezzo. Ora la regola è rovesciata: passa ogni chiave NUMERICA
      // che non sia già strutturale, così chi aggiunge un contatore non deve sapere
      // dell'esistenza di questa riga. Solo numeri: una stringa in un titolo sarebbe
      // contenuto, e il contenuto ha già le sue vie (`models`, `roles`, `kinds`).
      for (const k of Object.keys(issue)) {
        if (_STRUCTURAL.has(k)) continue;
        const v = issue[k];
        if (typeof v !== 'number' || !Number.isFinite(v)) continue;
        if (g.data[k] === undefined) g.data[k] = _n(v);
        else if (g.data[k] !== _n(v)) g.data.mixed = true;
      }
    }
    ...
    return {
      outcome: {
        ...
        // Le VM stanno nel preventivo per lo stesso motivo degli stack: sono
        // un'entita' che il documento acquista tutta insieme, e chi importa
        // deve vedere PRIMA se sono zero — e' successo, e sembrava un difetto
        // del pannello invece che un archivio mai interrogato.
        vms: _n(counts.vms),
        costs,
      },
      decisions,
