...
// - clamp e default sui contatori SFP (0..48 per blocco) e MGMT (0..4)
...
(function (root, factory) {
    ...
    function frontPanelState(node, portCount, mgmtEligible) {
        ...
        const sharedMediaSlots = normalizeSharedMediaSlots(fp.sharedMediaSlots, portCount || 0);
        // sfpCount: 0..48. Default 4 quando `separateSfp` e' attivo ma sfpCount
    ...
    function normalizeSharedMediaSlots(raw, portCount) {
        const max = Math.max(0, parseInt(portCount, 10) || 0);
        const input = Array.isArray(raw) ? raw : [];
        const output = [];
        const seen = new Set();
        const add = (value, media) => {
            const slot = parseInt(value, 10);
            if (!Number.isFinite(slot) || slot < 1 || slot > max || seen.has(slot)) return;
            seen.add(slot);
            output.push({ slot, media: Array.isArray(media) ? media.map(item => String(item).trim().toLowerCase()).filter(Boolean) : [] });
        };
    ...
    function frontPanelSharedSlots(node, portCount) {
        const state = frontPanelState(node, portCount, true);
        return state.sharedMediaSlots.map(item => item.slot);
    }
    ...
    return {
        ...
        frontPanelPortLabel,
        frontPanelSfpGroups,
        frontPanelSharedSlots,
        normalizeSharedMediaSlots,
        panelNumberOffset,
        patchPanelPortLabel,
        panelChainReaches,
    };
