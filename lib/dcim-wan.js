    ...
    for (const c of _list(o.circuits)) {
      ...
      if (capiSede.length === 2) {
        ...
        links.push({
          ...
          // ⑤ il nome del tipo, non una natura dedotta dal suo nome. `kind` è
          // esplicito e non sottinteso: i candidati di `lib/dcim-vpn.js` ne
          // portano uno vero (là il vocabolario di NetBox è chiuso), e chi li
          // innesta deve poter leggere lo stesso campo in tutti e due i casi.
          kind: 'other',
          kindLabel: tipo,
          provider,
          circuitId: cid,
