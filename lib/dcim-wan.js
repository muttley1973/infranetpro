...
(function (root, factory) {
  ...
  function circuitsToWan(nb, opts) {
    ...
    /** @type {Map<string,{slug:string|null, name:string|null, n:number, nLinks:number, kind:string}>} */
    const tipi = new Map();
    const uplinks = [];
    const links = [];
    const notes = [];
    ...
    for (const c of _list(o.circuits)) {
      ...
      if (tipoSlug || tipo) {
        ...
        voce = tipi.get(k);
        if (voce) voce.n++;
        else { voce = { slug: tipoSlug, name: tipo, n: 1, nLinks: 0, kind }; tipi.set(k, voce); }
      }
  ...
  return { circuitsToWan, wanPortLabel, _termTarget, _wanPort, _cirMbps, _kindDaSlug, KIND_TOKEN };
});
