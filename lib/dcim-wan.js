...
//  ── Le scelte, e perché ────────────────────────────────────────────────────
//
//   ① **La banda che entra è quella della PORTA, e va divisa.** Il campo del
//      modello si chiama `cirMbps` per ragioni storiche, ma quello che il
//      pannello e il dossier mostrano è la **banda della porta** — decisione
//      esplicita dell'utente (2026-08-31). Si prende quindi da `port_speed`
//      sulla terminazione che sta **sulla sede**, non dal `commit_rate` del
//      circuito, che è la banda del contratto ed è un'altra cosa. NetBox tiene
//      tutt'e due in **kbps**; il modello le vuole in Mbps.
//      ⚠️ **Il `commit_rate` non è un ripiego**, ed è la stessa trappola di
//      prima girata dall'altra parte. Il caso non è teorico: sul NetBox vero
//      `FW-VR-100M` dichiara 100 Mbps di contratto su una porta da **1000**.
//      Scrivere il contratto in un campo che dice «della porta» direbbe una cosa
//      falsa esattamente come faceva il contrario. Se `port_speed` manca, il
//      campo resta `null` e lo si DICE.
//      ⚠️ **Misurato** sul NetBox vero il 31/08 (46 circuiti, 71 terminazioni):
//      `port_speed` è valorizzato su **6 terminazioni di sede su 53**,
//      `commit_rate` su **13 circuiti su 46**. Il campo nascerà vuoto più spesso
//      di prima — è una conseguenza della scelta, non un difetto, e il pannello
//      la conta invece di nasconderla.
//      ⚠️ `upstream_speed` non si guarda: è l'altra direzione di una linea
//      asimmetrica, cioè una terza grandezza, e sul NetBox vero è vuota su tutte
//      e 71 le terminazioni.
//
...
(function (root, factory) {
  ...
  /** kbps → Mbps (①). `null` resta `null`, e zero non è una banda bassa: non è
   *  una banda. Converte e basta — QUALE campo convertire lo decide chi chiama. */
  function _kbpsToMbps(kbps) {
    const n = Number(kbps);
  ...
  function circuitsToWan(nb, opts) {
    ...
    for (const c of _list(o.circuits)) {
      ...
      if (capiSede.length === 1) {
        ...
        const tSede = (A === sede) ? tA : tZ;
        const porta = _wanPort(tSede);
        // ① La banda è quella della PORTA, e sta sulla terminazione di SEDE:
        // quella dall'altro capo è la porta di qualcun altro.
        const cir = _kbpsToMbps(tSede && (tSede.port_speed !== undefined ? tSede.port_speed : tSede.portSpeed));
        if (altro.what === 'cloud' && altro.name && nuvole.indexOf(altro.name) < 0) nuvole.push(altro.name);
        // ⚠️ Si conta SOLO qui: la banda è un campo dell'uplink, e un collegamento
        // fra sedi non ce l'ha. Contarlo anche là direbbe «resta vuoto» di un campo
        // che non esiste — un avviso che non si può togliere.
  ...
  return { circuitsToWan, wanPortLabel, _termTarget, _wanPort, _kbpsToMbps, _transportDaSlug, TRANSPORT_TOKEN };
});
