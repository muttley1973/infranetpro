...
(function (root, factory) {
  ...
  const KIND_TOKEN = (() => {
    ...
    const nature = (IS && IS.INTER_SITE_KINDS) || [];
  ...
  function _kindDaSlug(slug) {
    if (!slug) return 'other';
    return KIND_TOKEN[_token(slug)] || 'other';
  }
  ...
  function circuitsToWan(nb, opts) {
    ...
    // Il CENSIMENTO dei tipi di circuito incontrati: quanti circuiti per tipo e
    // in che natura sono entrati. È il verbale di ciò che ⑱ decide da sé, e
    // senza quello la decisione sarebbe invisibile a chi legge l'esito.
    /** @type {Map<string,{slug:string, name:string|null, n:number, kind:string}>} */
    const tipi = new Map();
    const uplinks = [];
    const links = [];
    const notes = [];
    ...
    for (const c of _list(o.circuits)) {
      ...
      const kind = _kindDaSlug(tipoSlug);
      if (tipoSlug || tipo) {
        const k = tipoSlug || ('~' + tipo);
        const gia = tipi.get(k);
        if (gia) gia.n++;
        else tipi.set(k, { slug: tipoSlug, name: tipo, n: 1, kind });
      }
      ...
      if (capiSede.length === 2) {
        ...
        links.push({
          ...
          kind,
          kindLabel: kind === 'other' ? tipo : null,
          provider,
          circuitId: cid,
    ...
    return {
      ...
      links,
      notes,
      // ⑱ I tipi di circuito incontrati, con quanti circuiti ciascuno e in che
      // natura sono entrati. Ordinati per nome: chi lo legge deve trovare la
      // riga dov'era ieri, e due letture uguali devono dare lo stesso elenco.
  ...
  return { circuitsToWan, _termTarget, _wanPort, _cirMbps, _kindDaSlug, KIND_TOKEN };
});
