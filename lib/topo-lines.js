...
(function (root, factory) {
  ...
  function buildTopoLines(model) {
    ...
    const nodeById = id => { for (const n of nodes) if (n && n.id === id) return n; return null; };
    const rackById = id => { for (const r of racks) if (r && r.id === id) return r; return null; };
    const linkById = id => { for (const l of links) if (l && l.id === id) return l; return null; };
    ...
    if (m.topoData) {
      ...
      for (const e of tEdges) {
        const srcTopo = tNodes.find(x => x.id === e.src);
        const dstTopo = tNodes.find(x => x.id === e.dst);
      ...
      for (const [, p] of pairMap) {
        ...
        const rANids = nodes.filter(n => n.rackId === p.rackAId).map(n => n.id);
        const rBNids = nodes.filter(n => n.rackId === p.rackBId).map(n => n.id);
        p.confirmed = links.some(l => {
          const ls = portNodeId(l.src), ld = portNodeId(l.dst);
          return (rANids.includes(ls) && rBNids.includes(ld)) || (rBNids.includes(ls) && rANids.includes(ld));
        });
      }
    }
    ...
    for (const r of racks) {
      ...
      const rackNids = new Set(nodes.filter(n => n.rackId === r.id && types[n.type]?.isRack).map(n => n.id));
      for (const link of links) {
