...
//    - documentazione: quota di device INDIRIZZABILI che hanno gia' un IP
//      documentato -> completamento della documentazione d'indirizzamento;
...
(function (root, factory) {
  ...
  // Oltre questa eta' una risposta SNMP non e' piu' «lo stato di adesso»: e' la
  // memoria di una risposta. Stessa soglia del livello 'old' del chip di freschezza
  // (src/app-snmp.js `_snmpFreshness`) e della lente ⑥ Salute — un solo numero per
  // «questa misura non e' piu' attuale» in tutto il prodotto.
  const SNMP_STALE_HOURS = 6;
  const _ts = (v) => { const n = (v instanceof Date) ? v.getTime() : Date.parse(_str(v)); return Number.isFinite(n) ? n : 0; };
  ...
  // Un tipo e' INDIRIZZABILE se puo' avere un indirizzo IP. Nel catalogo
  // (src/app-types.js) il flag `hasIP` marca solo i NON attivi che possono
  // averlo (UPS, PDU, ATS, media converter, e tutti gli endpoint di pavimento):
  // sugli attivi — switch, router, firewall, hypervisor, WLC… — l'indirizzo e'
  // implicito e il flag NON c'e'. Contare il solo `hasIP` escludeva quindi tutta
  // l'infrastruttura dal denominatore: su un progetto reale la barra dichiarava
  // «19/19 · 100%» ignorando 12 apparati. Stesso predicato usato ovunque nell'app
  // (app-discovery, app-drift-adopt, app-topology-crawl, app-properties…).
  function _isAddressable(def) {
    const d = def || {};
    return !!(d.isActive || d.hasIP);
  }
  ...
  // `now` = ms; passato dal chiamante per determinismo nei test. Serve a datare le
  // risposte SNMP: senza, un progetto riaperto dopo sei mesi mostrava la spia verde.
  function computeSubbarStats(nodes, types, now) {
    const list = Array.isArray(nodes) ? nodes : [];
    const T = (types && typeof types === 'object') ? types : {};
    const nowMs = Number.isFinite(now) ? now : Date.now();
    const staleBefore = nowMs - SNMP_STALE_HOURS * 3600000;
    let devices = 0, passive = 0, addressable = 0, withIp = 0, snmpTotal = 0, snmpOk = 0, snmpDown = 0;
    let snmpStale = 0, newestOk = 0;
    for (const n of list) {
      if (!n || typeof n !== 'object') continue;
      const def = T[n.type] || {};
      if (def.isStructural) continue;           // stanze/aree: non sono device
      devices++;
      // Passivi PURI di cablaggio (prese a muro, patch panel, passacavi, pannelli):
      // `isPassive` SENZA un IP proprio. Sono ELEMENTI documentati ma non «device».
      // Esposti a parte così il tooltip dice «di cui N passivi» — evita che «215
      // device» suoni come 215 apparati quando 73 sono prese a muro (schema ⑤). NB:
      // UPS/PDU sono isPassive ma HANNO un IP (indirizzabili) → NON sono qui dentro.
      if (def.isPassive && !def.hasIP) passive++;
      if (_isAddressable(def)) {
        addressable++;
        if (_str(n.ip)) withIp++;
      }
      if (_hasSnmp(n)) {
        ...
        if (n.snmpStatus === 'ok') {
          snmpOk++;
          // Il CONTEGGIO resta (quei device hanno risposto davvero), ma una
          // risposta ha una data: senza timestamp, o piu' vecchia della soglia,
          // non e' piu' «sta rispondendo adesso». Un progetto vecchio di mesi
          // mostrava la spia verde perche' `snmpStatus` sopravvive al salvataggio.
          const at = _ts(n.snmpLastOk);
          if (at > newestOk) newestOk = at;
          if (!at || at < staleBefore) snmpStale++;
    ...
    // Il verde pretende che rispondano tutti E che la risposta sia RECENTE: una
    // spia verde su misure di quattro giorni fa e' la stessa bugia gia' corretta
    // sul verdetto «Vero» e sulla lente ⑥.
    let snmpHealth;
    if (snmpTotal === 0) snmpHealth = 'none';
    else if (snmpOk === 0 && snmpDown > 0) snmpHealth = 'err';
    else if (snmpOk === snmpTotal && !snmpStale) snmpHealth = 'ok';
    else snmpHealth = 'warn';
    return { devices, passive, addressable, withIp, docPct, snmpTotal, snmpOk, snmpDown,
      snmpStale, snmpNewestOk: newestOk || null, snmpHealth };
  }
  ...
  return { computeSubbarStats, computeTopoHiddenCables, _hasSnmp, _isAddressable, SNMP_STALE_HOURS };
});
