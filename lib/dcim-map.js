...
(function (root, factory) {
  ...
  function netboxToState(nb, opts) {
    ...
    for (const dev of (Array.isArray(nb.devices) ? nb.devices : [])) {
      ...
      // Restano come METADATO DI PROVENIENZA: non si perdono, ma nessuna schermata
      // li legge. Vanno DICHIARATI nell'anteprima, altrimenti chi importa da' per
      // scontato che il tenant sia diventato il responsabile dell'apparato.
      const tenantName = _str(dev.tenant && (dev.tenant.name || dev.tenant.display));
      if (tenantName) {
        node.source.tenant = tenantName;
        _issue('device.tenantSkipped', { deviceId: dev.id, deviceName: name, kind: tenantName },
          'device ' + dev.id + ': tenant NetBox "' + tenantName + '" conservato come provenienza, non mostrato');
      }
    ...
    for (const p of (Array.isArray(nb.prefixes) ? nb.prefixes : [])) {
      ...
      const prefix = { id: p.id != null ? p.id : null, prefix: _str(p.prefix), vlan: vid };
