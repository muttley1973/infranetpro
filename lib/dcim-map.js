...
(function (root, factory) {
  ...
  // ── Virtualizzazione ──────────────────────────────────────────────────────
  // I tipi che sanno ospitare VM. ⚠️ È uno SPECCHIO del flag `hostsVms` in
  // src/app-types.js: quel file è un modulo ESM del frontend e questa è una lib
  // pura, quindi non si può importare. Due elenchi della stessa cosa divergono
  // al primo ritocco — perciò test/dcim-map.test.js legge app-types.js e
  // fallisce se qualcuno aggiunge un tipo là e non qui.
  const _VM_HOST_TYPES = new Set(['hypervisor', 'homelab']);
  ...
  // Il MAC di una vNIC. NetBox 4.2 ha spostato i MAC in oggetti dedicati
  // (`primary_mac_address`) lasciando il campo piatto per compatibilità: si
  // guardano entrambi, nell'ordine in cui NetBox li considera autorevoli.
  function _vmIfaceMac(itf) {
  ...
  function netboxToState(nb, opts) {
    ...
    if (vmList.length) {
      ...
      for (const vm of vmList) {
        ...
        // Un apparato su cui NetBox mette delle VM È un host di virtualizzazione:
        // lo dice il suo stesso archivio. Il tipo si adegua — altrimenti il dato
        // arriverebbe in un nodo che non ha la sezione per mostrarlo, che è il
        // difetto da cui siamo partiti. Rack → hypervisor, pavimento → homelab.
        ...
        for (const itf of (ifacesByVm.get(String(vm.id)) || [])) {
          ...
          const mac = _vmIfaceMac(itf); if (mac) nic.mac = mac;
  ...
  return {
    ...
    _vmGuestOs, _vmIfaceMac, _VM_HOST_TYPES,
