...
(function (root, factory) {
  ...
  const _ROLE_MAP = {
    ...
    'l3-switch': 'switch', router: 'router', 'edge-router': 'router', firewall: 'firewall',
    'ap': 'ap', 'wireless-ap': 'ap', 'access-point': 'ap', 'wireless-lan-controller': 'wlanctrl', wlc: 'wlanctrl',
    server: 'server', storage: 'nas', nas: 'nas', pdu: 'pdu', ups: 'ups', ats: 'ats',
  ...
  function _roleToInfranetType(roleSlug, roleName, model) {
    ...
    if (/\bserver\b|poweredge|proliant|\besxi\b/.test(hay)) return { type: 'server', mapped: true };
  ...
  function netboxToState(nb, opts) {
    ...
    for (const dev of (Array.isArray(nb.devices) ? nb.devices : [])) {
      ...
      const node = { id: nodeId, name: _str(dev.name) || nodeId, type: rt.type };
