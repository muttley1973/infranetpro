...
(function (root, factory) {
  ...
  function _interfacePortData(iface, kind, slotDefinition) {
    ...
    if (kind === 'fiber') port.physicalKind = 'fiber';
    else if (kind === 'copper') port.physicalKind = 'copper';
  ...
  function netboxToState(nb, opts) {
    ...
    const report = {
      ...
      counts: {
        ...
        vlans: 0, prefixes: 0, ips: 0, racks: 0,
      },
      ...
      warnings: [], truncated: !!nb.truncated,
    };
    ...
    for (const dev of (Array.isArray(nb.devices) ? nb.devices : [])) {
      ...
      if (manualMapping.placement === 'floor') {
        ...
        report.warnings.push('device ' + dev.id + ' non puÃ² essere spostato nel rack: rack NetBox assente');
      ...
      if (node.type === 'pdu') {
        ...
        if (mgmtIndex > 2) report.warnings.push('device ' + dev.id + ' ha ' + mgmtIndex + ' interfacce Ethernet PDU; InfraNet ne visualizza al massimo 2');
      ...
      if (node.type === 'pdu') {
        ...
        if (sourceOutlets.length > 48) report.warnings.push('device ' + dev.id + ' ha ' + sourceOutlets.length + ' prese power; InfraNet ne importa al massimo 48');
      ...
      if (tmpl && Number.isFinite(+tmpl.ports) && physical.length > +tmpl.ports) {
        ...
        report.warnings.push('device ' + dev.id + ' ha più interfacce fisiche NetBox (' + physical.length + ') del template catalogo (' + tmpl.ports + ')');
        ...
        if (catalogMatch.strategy === 'ambiguous') report.warnings.push('device ' + dev.id + ' ha un modello catalogo ambiguo (' + dk + ')');
      }
    ...
    for (const c of (Array.isArray(nb.cables) ? nb.cables : [])) {
      ...
      if (src === dst) {
        ...
        report.warnings.push('cavo ' + c.id + ' saltato (anello sulla stessa porta)');
        continue;
      }
    ...
    for (const p of (Array.isArray(nb.prefixes) ? nb.prefixes : [])) {
      ...
      const prefix = { id: p.id != null ? p.id : null, prefix: _str(p.prefix), vlan: Number.isFinite(+vid) ? +vid : null };
