  ...
  function netboxToState(nb, opts) {
    ...
    const report = {
      ...
      counts: {
        ...
        vlans: 0, vlanRecords: 0, prefixes: 0, ips: 0, racks: 0, stacks: 0, radios: 0, ssids: 0,
    ...
    let wifiDevices = 0, wifiOverCap = 0, wifiAssumedSec = 0;
    ...
    for (const dev of (Array.isArray(nb.devices) ? nb.devices : [])) {
      ...
      if (radioIfaces.length) {
        ...
        for (const itf of radioIfaces) {
          ...
          if (ch) { radio.band = ch.band; radio.channel = ch.channel; }
