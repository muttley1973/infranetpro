...
(function (root, factory) {
  ...
  // ⚠️ QUI passa OGNI stringa che arriva da NetBox — 92 chiamate — ed è quindi il
  // confine in cui un dato di un sistema TERZO diventa il nostro documento. Un
  // NetBox può essere compromesso, o solo rotto: misurato con un mock ostile,
  // un nome di apparato da **2 MB** attraversava tutto e tornava al client (2.049
  // KB di risposta), e i caratteri di controllo con lui. Da lì il nome va nel
  // progetto, nei PDF, nelle etichette e negli inventari Ansible — dove un a-capo
  // non è un carattere, è una riga nuova.
  // Stessa forma delle scritte dell'utente (`lib/user-text.js`): il CONTENUTO non
  // si tocca, la forma sì. Una definizione sola, due sorgenti di testo.
  const _testo = typeof require === 'function' ? require('./user-text.js') : null;
  function _str(s) {
    if (_testo) return _testo.cleanUserText(s);
  ...
  // Il nome PIENO di un'ubicazione: la catena dei padri unita da ' · '.
  // ⚠️ Non è più il modo di rappresentare l'annidamento — quello adesso si
  // DISEGNA (vedi `_autoPlaceRooms`, ㉗). Resta il nome di ciò che sta in cima:
  // il piano si chiama «Edificio A · Piano 1» quando sopra di lui c'è ancora
  // qualcosa, perché i livelli disegnati sono due e il resto va detto a parole.
  ...
  /**
   * ㉗ **La gerarchia di NetBox smette di essere una stringa.**
   *
   * Le ubicazioni NetBox si annidano (Sito → Ubicazione → sotto-Ubicazione →
   * Rack → Device); le stanze InfraNet no, e per questo la catena dei padri
   * finiva **appiattita nel nome** — «Piano 1 · Sala server» — con il commento
   * che diceva la verità: «invece di fingere una gerarchia che il documento non
   * sa rappresentare». Adesso il documento la sa rappresentare: c'è il tipo
   * `storey`, che è un contenitore disegnato esattamente come la stanza, un
   * gradino sotto. Quindi **il padre immediato diventa un PIANO e la figlia una
   * STANZA**, e il separatore ' · ' smette di essere una gerarchia travestita da
   * etichetta.
   *
   * ⚠️ **Due livelli disegnati, non N.** Se la catena è più lunga (Edificio →
   * Piano → Sala), il piano è il padre IMMEDIATO della stanza e tutto ciò che
   * sta più su resta nel NOME del piano («Edificio A · Piano 1»). Fingere tre
   * livelli con due sarebbe la stessa bugia di prima, solo più in alto — e un
   * piano dentro un piano il modello non lo prevede.
   * ⚠️ **Un piano sta sempre in cima.** Se un'ubicazione è padre di qualcosa,
   * è un piano: non diventa mai la stanza di un altro piano, altrimenti
   * servirebbe il terzo livello che non c'è.
   * ⚠️ **Un'ubicazione può avere contenuto suo E dei figli** (un rack appeso
   * direttamente a «Piano 1», più la «Sala server» dentro). Allora è un piano, e
   * i suoi apparati stanno **dentro il piano, accanto alle stanze** — non in una
   * stanza inventata che NetBox non ha mai dichiarato.
   * ⚠️ Un'ubicazione padre **senza contenuto da nessuna parte** non diventa
   * niente, come prima: un rettangolo vuoto non documenta.
   */
  function _autoPlaceRooms(state, ctx) {
    ...
    if (!gruppi.size) return { rooms: 0, storeys: 0, placed: 0, senzaUbicazione: 0, nested: 0 };

    // Il padre di un'ubicazione, ma solo se lo CONOSCIAMO: NetBox può nominare un
    // padre che la lettura non ha portato (fuori scope, troncata). Un padre che
    // non c'è non è un piano — è un id, e un piano senza nome non si disegna.
    const padreDi = (locId) => {
      const l = byId.get(_str(locId));
      const p = l && l.parent && l.parent.id;
      const pid = (p != null) ? _str(p) : null;
      return (pid && byId.has(pid)) ? pid : null;
    };

    // ── Chi è piano e chi è stanza ────────────────────────────────────────
    const figliDi = new Map();
    for (const locId of gruppi.keys()) {
      const p = padreDi(locId);
      if (!p) continue;
      if (!figliDi.has(p)) figliDi.set(p, []);
      figliDi.get(p).push(locId);
    }
    // Un piano non sta dentro un piano: se un figlio è a sua volta padre, sale in
    // cima e resta un piano suo. Chi rimane senza stanze non è più un piano —
    // il suo eventuale contenuto proprio torna a essere una stanza normale.
    const eraPadre = new Set(figliDi.keys());
    for (const [p, figli] of [...figliDi]) {
      const stanze = figli.filter(id => !eraPadre.has(id));
      if (stanze.length) figliDi.set(p, stanze); else figliDi.delete(p);
    }
    const piani = new Set(figliDi.keys());

    const etichetta = (locId) => _locationLabel(locId, names, byId);
    // Il nome PROPRIO, senza la catena: dentro un piano che si chiama già
    // «Piano 1», una stanza chiamata «Piano 1 · Sala server» direbbe due volte
    // la stessa cosa — e la seconda in un posto dove non serve più.
    const nomeProprio = (locId) => {
      const l = byId.get(_str(locId));
      return _str(l && l.name) || _str(names.get(_str(locId))) || ('nb-loc-' + locId);
    };

    // La dimensione di una stanza viene dal suo CONTENUTO (vedi _ROOM_LAYOUT).
    const dimStanza = (n) => {
      const cols = Math.max(1, Math.min(L.innerColumns, n));
      const rows = Math.ceil(n / cols);
      return { cols, w: Math.max(L.minW, L.pad * 2 + cols * L.cellX),
        h: Math.max(L.minH, L.padTop + L.pad + rows * L.cellY) };
    };
    // Gli apparati appesi DIRETTAMENTE a un piano: stessa griglia, senza il
    // rettangolo intorno — non c'è una stanza da disegnare, e inventarla
    // direbbe che NetBox l'ha dichiarata.
    const dimSciolti = (n) => {
      const cols = Math.max(1, Math.min(L.innerColumns, n));
      return { cols, w: cols * L.cellX, h: Math.ceil(n / cols) * L.cellY };
    };

    // Impacchetta una lista di rettangoli in righe da `cols`, e dice l'ingombro.
    // UNA funzione per i due usi — le stanze dentro un piano e le scatole sul
    // foglio — invece di due cicli gemelli che divergono al primo ritocco.
    const impacchetta = (rect, cols, gap) => {
      let x = 0, y = 0, rowH = 0, col = 0, maxW = 0;
      const pos = [];
      for (const r of rect) {
      ...
      return { pos, w: maxW, h: y + rowH };
    };

    const itemsDi = (locId) => {
      const grp = gruppi.get(locId);
      return grp ? grp.racks.concat(grp.nodes) : [];
    };

    // ── Le scatole di primo livello, in ordine STABILE ────────────────────
    // Due import dello stesso sito devono dare la stessa planimetria: si ordina
    // per etichetta e, a parità, per id — mai per ordine di arrivo.
    const ordina = (ids) => ids.slice().sort((x, y) => {
      const c = _str(etichetta(x)).localeCompare(_str(etichetta(y)));
      return c || _natCmp(_str(x), _str(y));
    });

    const scatole = [];
    for (const p of piani) {
      const stanze = ordina(figliDi.get(p));
      const dimStanze = stanze.map(id => dimStanza(itemsDi(id).length));
      const dentro = impacchetta(dimStanze, L.roomColumns, L.gap);
      const sciolti = itemsDi(p);
      const ds = sciolti.length ? dimSciolti(sciolti.length) : null;
      scatole.push({
        kind: 'storey', locId: p, stanze, dimStanze, dentro, sciolti, ds,
        w: L.pad * 2 + Math.max(dentro.w, ds ? ds.w : 0),
        h: L.padTop + dentro.h + (ds ? L.gap + ds.h : 0) + L.pad,
      });
    }
    for (const locId of gruppi.keys()) {
      if (piani.has(locId)) continue;                     // è un piano
      const p = padreDi(locId);
      if (p && piani.has(p)) continue;                    // sta dentro un piano
      const d = dimStanza(itemsDi(locId).length);
      scatole.push({ kind: 'room', locId, w: d.w, h: d.h, cols: d.cols });
    }
    scatole.sort((x, y) => {
      const c = _str(etichetta(x.locId)).localeCompare(_str(etichetta(y.locId)));
      return c || _natCmp(_str(x.locId), _str(y.locId));
    });

    const fuori = impacchetta(scatole, L.roomColumns, L.gap);

    // ── Si disegna ────────────────────────────────────────────────────────
    let rooms = 0, storeys = 0, placed = 0, nested = 0, fondo = 0;
    const metti = (items, x0, y0, cols) => {
      items.forEach((it, i) => {
        it.x = x0 + (i % cols) * L.cellX;
        it.y = y0 + Math.floor(i / cols) * L.cellY;
        it.positionSource = 'infranet-import-grid';
        placed++;
      });
    };
    const stanzaEl = (locId, nome, x, y, w, h) => {
      const el = { id: 'nb-loc-' + locId, type: 'room', name: nome,
        x, y, w, h, positionSource: 'infranet-import-grid' };
      _setRef(el, _T.location, locId);
      state.nodes.push(el);
      rooms++;
      return el;
    };

    scatole.forEach((box, i) => {
      const X = L.originX + fuori.pos[i].x, Y = L.originY + fuori.pos[i].y;
      fondo = Math.max(fondo, Y + box.h);
      if (box.kind === 'room') {
        stanzaEl(box.locId, etichetta(box.locId), X, Y, box.w, box.h);
        metti(itemsDi(box.locId), X + L.pad, Y + L.padTop, box.cols);
        return;
      }
      // Il PIANO: il rettangolo che contiene, e dentro le sue stanze.
      const piano = { id: 'nb-loc-' + box.locId, type: 'storey', name: etichetta(box.locId),
        x: X, y: Y, w: box.w, h: box.h, positionSource: 'infranet-import-grid' };
      _setRef(piano, _T.location, box.locId);
      state.nodes.push(piano);
      storeys++;
      box.stanze.forEach((locId, k) => {
        const d = box.dimStanze[k], p = box.dentro.pos[k];
        const rx = X + L.pad + p.x, ry = Y + L.padTop + p.y;
        stanzaEl(locId, nomeProprio(locId), rx, ry, d.w, d.h);
        nested++;
        metti(itemsDi(locId), rx + L.pad, ry + L.padTop, d.cols);
      });
      // Gli apparati del piano stesso, sotto le sue stanze e dentro il suo bordo.
      if (box.ds) metti(box.sciolti, X + L.pad, Y + L.padTop + box.dentro.h + L.gap, box.ds.cols);
    });
    ...
    return { rooms, storeys, placed, senzaUbicazione, nested };
  }
  ...
  function netboxToState(nb, opts) {
    ...
    if (_rooms.rooms) {
      _issue('location.rooms', { n: _rooms.rooms, placed: _rooms.placed, nested: _rooms.nested, storeys: _rooms.storeys });
