...
//     ports,       // { total, documented, used, free, list:[{speed,status,lagGroup,poe}] } o undefined
//                  // ⚠️ `total` = porte DICHIARATE (null se il documento non le
//                  // dichiara) · `documented` = porte con un record. Non sono lo
//                  // stesso numero e non vanno scambiati: v. server/ai/context.js.
...
(function (root, factory) {
  ...
  function _portsCap(ports, lagNames, lagModes) {
    ...
    const lags = [];
    // Σ della banda-membri dei LAG DI QUESTO device. NON è «uplink»: un LAG può
    // essere un downlink verso uno switch di accesso, e la direzione non è nota
    // (schema ④, misnomer). Nome onesto: banda LAG totale sul device.
    let lagAggregateMbps = 0;
    for (const g of Object.keys(groups)) {
      const e = groups[g];
      const lag = { name: _str(names[g]) || String(g).slice(0, 48), members: e.members };
      if (e.hasSpeed) { lag.aggregateMbps = e.aggregateMbps; lagAggregateMbps += e.aggregateMbps; }
    ...
    if (hasLags) { out.lags = lags; if (lagAggregateMbps > 0) out.lagAggregateMbps = lagAggregateMbps; }
  ...
  function computeFleetCapabilities(list) {
    ...
    // Il LAG fra due switch appare su ENTRAMBI i capi: SOMMARE la banda LAG di
    // tutta la flotta la conta due volte (schema ④: ×2.00 esatto su progetto 9,
    // 664 000 invece di 332 000 Mbps). Il fatto-flotta ONESTO è il collegamento
    // aggregato PIÙ CAPIENTE — un MAX non raddoppia i due capi (max{20G,20G}=20G).
    let maxLagAggregateMbps = 0;
    let aps = 0, ssids = 0;
    for (const c of arr) {
      if (!c || typeof c !== 'object') continue;
      if (c.ports && c.ports.free != null) { freePorts += Number(c.ports.free) || 0; hasFree = true; }
      if (c.ports && Array.isArray(c.ports.lags)) {
        for (const lag of c.ports.lags) {
          const m = lag && Number(lag.aggregateMbps);
          if (Number.isFinite(m) && m > maxLagAggregateMbps) maxLagAggregateMbps = m;
        }
      }
    ...
    if (maxLagAggregateMbps > 0) out.maxLagAggregateMbps = maxLagAggregateMbps;
    if (aps) out.aps = aps;
