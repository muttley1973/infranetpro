  ...
  function _complete(m) {
    ...
    const rows = [
      ...
      _row('subnets', {
        ...
        items: nets.map((x) => { const n = _obj(x); return { id: n.cidr || n.net, meta: _num(n.deviceCount) }; }),
      }),
      _row('vlanNames', {
  ...
  function _margin(m) {
    ...
    const rows = [
      ...
      _row('ipFree', {
        ...
        items: nets.map((x) => { const n = _obj(x); return { id: n.cidr || n.net, meta: _freeOf(n), of: _usableHosts(n.cidr || n.net) }; }),
      }),
    ];
    return {
      rows,
