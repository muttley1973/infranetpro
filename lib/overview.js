...
(function (root, factory) {
  ...
  function _sectionHealth(key, rows) {
    ...
    if (key === 'complete') {
      ...
      const issues = ['addr', 'name', 'mac', 'vlanNames'].filter((k) => hasGap(by.get(k))).length
    ...
    if (key === 'truth') {
      ...
      if (suspect > 0 || unverifiable > 0 || drift > 0 || conflicts > 0) return { level: 'warn', issues: suspect + drift + conflicts };
  ...
  function _truth(m) {
    ...
    const rows = [
      ...
      _row('conflicts', {
        value: conflictCount, total: null, prov: 'derived',
        tone: conflictCount > 0 ? 'alert' : 'normal',
    ...
    const freshest = Math.max(lastAt, lvAt);
    const staleDays = (freshest > 0 && now > 0) ? (now - freshest) / 86400000 : 0;
    const stale = freshest > 0 && staleDays > STALE_TRUTH_DAYS;
    const health = _sectionHealth('truth', rows);
    if (stale && health.level === 'ok') { health.level = 'warn'; health.stale = true; health.staleDays = Math.round(staleDays); }
    return {
      rows,
      health,
      headline: _headline(rows, [
  ...
  function _recovery(m) {
    ...
    const noBackup = [], noIdentity = [], noLocation = [], stalePresence = [], haItems = [];
  ...
  function _security(m) {
    ...
    // Salute: ROSSO se c'è una community indovinabile (porta di gestione aperta);
    // GIALLO se c'è SNMP in chiaro o nessuna VLAN di gestione; VERDE altrimenti.
    const exposed = weakSnmp.length;
    let level = 'ok';
    if (defaultComm.length > 0) level = 'bad';
    else if (exposed > 0 || (snmpTotal > 0 && mgmtVlans.length === 0)) level = 'warn';
  ...
  const _BLIND_SPOTS = [
    ...
    { key: 'lifecycle', tier: 'unmodeled' },   // garanzia / EOL / RMA
    { key: 'restore',   tier: 'unmodeled' },   // il backup si RIPRISTINA davvero (non solo: esiste)
    { key: 'power',     tier: 'elsewhere' },   // UPS: autonomia/carico letti (UPS-MIB), non nel verdetto DR
    { key: 'health',    tier: 'elsewhere' },   // CPU / memoria / errori d'interfaccia / temperatura live
  ];
  ...
  function buildOverview(model) {
    ...
    return {
      ...
      recovery: _recovery(m), security: _security(m),
