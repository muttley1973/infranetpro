...
(function (root, factory) {
  // Dipende da lib/proof.js (stato di prova del cavo) e da lib/cidr.js (aritmetica
  // degli indirizzi): in Node li si require, nel browser sono già su window (root)
  // via <script> — stesso pattern di drift-snapshot, letti a CALL-TIME.
  const isNode = typeof module !== 'undefined' && module.exports;
  const api = factory(isNode ? require('./proof.js') : root, isNode ? require('./cidr.js') : root,
    isNode ? require('./identity-reconcile.js') : root, isNode ? require('./device-status.js') : root,
    isNode ? require('./linkstate.js') : root, isNode ? require('./certainty.js') : root);
  if (typeof module !== 'undefined' && module.exports) module.exports = api;   // Node (test)
  if (typeof window !== 'undefined') Object.assign(window, api);               // browser
})(typeof self !== 'undefined' ? self : this, function (proof, C, idApi, dsApi, lsApi, ctyApi) {
  'use strict';

  const _str = (v) => (v == null ? '' : String(v)).trim();

  // Come sappiamo un cavo, in UNA parola: quella della notazione unica
  // (lib/certainty.js). Dopo una Verifica decide lo STATO DI PROVA, che e' il
  // piu' informato — e' li' che vivono il fantasma (un dedotto che ha perso
  // l'evidenza) e il contraddetto (la rete smentisce il documento). Senza
  // Verifica decide la PROVENIENZA, letta da `linkState`: la catena della
  // certezza sta scritta in un posto solo e qui non se ne ricopia un pezzo.
  // ⚠️ Nessun elenco nuovo di parole: i due vocabolari `proof` e `linkstate`
  //    sono gia' mappati sui gradi, e chi aggiunge uno stato a un motore lo
  //    mappa li' — la guardia di `test/certainty.test.js` lo pretende.
  function _cableGrade(l, proofState) {
    if (!ctyApi || typeof ctyApi.certaintyOf !== 'function') return '';
    if (proofState) {
      const g = ctyApi.certaintyOf('proof', proofState).grade;
      if (g) return g;
    }
    if (!lsApi || typeof lsApi.linkState !== 'function') return '';
    return ctyApi.certaintyOf('linkstate', lsApi.linkState(l).certaintyKey).grade || '';
  }

  // L'ordine delle schede e' quello della notazione, dal segno piu' portante al
  // meno: le schede nascono nell'ordine in cui i gruppi compaiono nella lista.
  const _gradeRank = (g) => {
    const all = (ctyApi && ctyApi.CERTAINTY_GRADES) || [];
  ...
  // Aritmetica degli indirizzi: la fa lib/cidr.js, l'unica definizione del progetto.
  // Qui vivevano una copia di `_ipToInt` e un parser CIDR a regex, tenuti in casa
  // perché «overview.js è bundlato e cidr.js è uno <script>: mescolarli è fragile».
  // Non lo è più: src/app-wifi.js importa già lib/cidr.js come ESM, quindi cidr è
  // GIÀ nel bundle e esbuild lo risolve al medesimo modulo — nessuna copia in più.
  // Le due locali erano per giunta IPv4-only mentre il modello è dual-stack: è la
  // divergenza che aveva prodotto il report L3 cieco sull'IPv6 (audit D1).
  const _ipToInt = (ip) => (C && typeof C._parseIpv4Int === 'function' ? C._parseIpv4Int(ip) : null);
  const _intToIp = (v) => (C && typeof C._intToIpv4 === 'function' ? C._intToIpv4(v) : '');
  // "10.20.0.0/16" (o forma host "10.20.5.1/16") → info della rete, o null.
  // ⚠️ `_parseCidrInfo` è dual-stack; questa lente misura la capacità con
  // aritmetica a 32 bit e riceve indirizzi IPv4 (deriveProjectNetworks), quindi
  // le reti v6 si SCARTANO qui, esplicitamente — prima sparivano perché il regex
  // locale non le riconosceva, che è la stessa cosa detta per sbaglio.
  function _cidrInfo(cidr) {
    const info = (C && typeof C._parseCidrInfo === 'function') ? C._parseCidrInfo(_str(cidr)) : null;
    if (!info || info.family !== 4) return null;
    const mask = info.prefix === 0 ? 0 : ((0xffffffff << (32 - info.prefix)) >>> 0);
    return { network: info.network, mask, prefix: info.prefix, cidr: _intToIp(info.network) + '/' + info.prefix };
  }
  // Le reti DICHIARATE come lista [{cidr, gateway}]. PREFIX-FIRST: `ipam.prefixes[]` è
  // l'autorità e contiene ANCHE le reti senza VLAN e il secondo prefisso di una VLAN
  // dual-stack — che la vista per-VLAN (ipamVlans) non conteneva, così una rete legale
  // con `vlan:null` finiva fra le «non dichiarate». Fallback alla vecchia forma per-VLAN
  // quando `prefixes` è assente (retrocompatibilità dei chiamanti/test che passano solo
  // ipamVlans). → [[declare-first-workflow]].
  function _declaredPrefixList(m) {
    const mm = _obj(m);
    if (Array.isArray(mm.prefixes)) {
      return mm.prefixes.map((p) => { const o = _obj(p); return { cidr: _str(o.cidr), gateway: _str(o.gateway) }; })
        .filter((p) => p.cidr);
    }
    const v = _obj(mm.ipamVlans);
    return Object.keys(v).map((k) => { const o = _obj(v[k]); return { cidr: _str(o.subnet), gateway: _str(o.gateway) }; })
      .filter((p) => p.cidr);
  }

  // Subnet DICHIARATE, dedup per rete, PIU' SPECIFICA prima (prefix desc): cosi', con
  // subnet annidate, il primo match e' la piu' precisa. `_cidrInfo` è IPv4 e il lato
  // osservato (deriveProjectNetworks) è IPv4 → un prefisso v6 dà null ed è saltato
  // (nessun IP v4 vi ricadrebbe comunque).
  function _declaredSubnets(m) {
    const out = [], seen = new Set();
    for (const p of _declaredPrefixList(m)) {
      const info = _cidrInfo(p.cidr);
      if (!info) continue;
  ...
  function _subnetGroups(m) {
    ...
    const declared = _declaredSubnets(m);
  ...
  // Chiave canonica per CONFRONTARE due nomi di VLAN — famiglia di macKey/addrKey/
  // segmentKey: mai un confronto per stringa nuda. Piega SOLO le maiuscole, perche'
  // quella e' l'unica differenza che non appartiene alla rete ma a come ogni vendor
  // compita: misurato al banco, la VLAN 1 e' `default` per Cisco e Arista e `Default`
  // per Extreme, ed e' la stessa VLAN. Tutto il resto resta una differenza VERA e va
  // detta: `DATA` contro `VLAN0010` significa che su quello switch il nome non c'e'
  // (l'immagine lo genera dal numero), ed e' esattamente cio' che si vuole vedere.
  // toLowerCase e non toLocaleLowerCase: la stessa coppia di nomi deve dare lo stesso
  // verdetto su qualunque macchina.
  function vlanNameKey(name) { return _str(name).trim().toLowerCase(); }
  ...
  function _sectionHealth(key, rows) {
    ...
    if (key === 'truth') {
      ...
      // ⚠️ La riga qui sopra promette «configurati e muti»: sono DUE popolazioni —
      // chi ha risposto errore e chi non ha mai risposto — e se ne contava una
      // sola. Un apparato aggiunto al documento e mai interrogato lasciava la
      // lente verde: la prosa diceva il vero, il conto no.
      const _vExtra = _obj(by.get('verifiable') && by.get('verifiable').extra);
      const snmpErrors = _num(_vExtra.errors) + _num(_vExtra.silent);
  ...
  function _complete(m) {
    ...
    // Reti dichiarate e, fra queste, quelle SENZA gateway: un gateway mancante è un buco
    // di ripristino (non sai come instradare la rete). PREFIX-FIRST: ogni prefisso ha un
    // suo gateway, comprese le reti senza VLAN e il secondo prefisso di una dual-stack —
    // che la vista per-VLAN non contava.
    const declPrefixes = _declaredPrefixList(m);
    const declaredSubnets = declPrefixes.length;
    const gwMissingArr = declPrefixes.filter((p) => !p.gateway);
    // Ordine del drill-down gateway: prima le SENZA (azione), poi le complete.
    const gwOrdered = gwMissingArr.concat(declPrefixes.filter((p) => p.gateway));
    ...
    for (const v of vlansInUse) {
      ...
      if (decl) vlansNamed++;
      // `said` è il verbale grezzo di chi ha detto cosa; le VARIANTI si ricavano qui,
      // piegandolo con `vlanNameKey`: la stessa chiave che poco sotto decide se il
      // dichiarato combacia. Un modello che porta solo `name` descrive un misurato
      // solo, e un nome solo non può discordare con nessuno: non è un ripiego, è
      // esattamente ciò che quel modello dice.
      const variants = [];
      for (const s of _arr(meas.said)) {
        const nm = _str(_obj(s).name).trim();
        if (!nm) continue;
        const k = vlanNameKey(nm);
        let variant = null;
        for (const x of variants) if (vlanNameKey(x.name) === k) { variant = x; break; }
        if (!variant) variants.push(variant = { name: nm, nodes: [] });
        const who = _str(_obj(s).node);
        if (who) variant.nodes.push(who);
      }
      if (!variants.length && measName) variants.push({ name: measName, nodes: [] });
      // «La rete un nome ce l'ha»: il rappresentante che il modello dichiara, o il primo
      // che si è sentito. Non si guarda `meas.name` da solo, o un modello che porta il
      // verbale senza rappresentante uscirebbe come «la rete non sa niente».
      const netName = measName || (variants[0] ? variants[0].name : '');
      const disagree = variants.length > 1;
      const declKey = vlanNameKey(decl);
      const declMeasured = !!declKey && variants.some((x) => vlanNameKey(x.name) === declKey);
      // Conflitto: (a) il dichiarato non lo dice nessun apparato, o (b) gli apparati
      // discordano tra loro — anche su una VLAN non dichiarata: la rete stessa non
      // concorda, quindi non c'è un nome da «colmare» senza una decisione umana.
      if (netName && ((decl && !declMeasured) || disagree)) {
        vlanConflict++;
        // Il reperto NOMINA il dissenziente. Prima stampava «dichiarato ↔ PRIMO
        // misurato»: quando a discordare erano gli apparati fra loro usciva due volte
        // lo STESSO nome («DATA ↔ DATA»), e chi legge non poteva né capire l'accusa né
        // chiuderla — il nome divergente viveva solo dentro un commento nei test. I
        // testimoni si mostrano SOLO se le varianti sono più d'una: con una sola non
        // c'è nessuna ambiguità da sciogliere e il nome dell'apparato è rumore.
        const parts = variants.map((x) => x.name + (disagree && x.nodes.length ? ' (' + x.nodes.join(', ') + ')' : ''));
        vlanConflictItems.push({ id: 'VLAN ' + v, meta: (decl && !declMeasured ? decl + ' ↔ ' : '') + parts.join(' ↔ '), vid: Number(v) });
      } else if (!decl && netName) {
        // lacuna colmabile: nessun nome dichiarato ma la rete ne conosce uno (concorde).
        vlanFromSnmp++;
        vlanFillItems.push({ id: 'VLAN ' + v, meta: netName, tag: 'measured', vid: Number(v) });
      }
    }
    ...
    if (proof && typeof proof.cableProof === 'function' && nodes.some((n) => n && n.proof)) {
      ...
      // Ogni contatore va SEMINATO a 0: `undefined++` fa NaN, e un NaN in questo
      // oggetto non esplode — sparisce, perché a valle `pc.shut || 0` lo legge come
      // zero e la voce non compare mai. Un bug che non si vede è peggio di uno che
      // rompe: aggiungendo uno stato, aggiungere anche la sua riga qui.
      cableProofCounts = { declared: 0, derivedStrong: 0, derivedWeak: 0, ghost: 0, review: 0, shut: 0 };
      ...
      const els = links.map((l) => {
        ...
        // Contato a parte, non dentro `review`: nel tile «Cavi» ha una sua voce, e
        // sommarlo ai riesami lo avrebbe reso invisibile proprio dove si guarda.
        else if (st === 'declared-shut') cableProofCounts.shut++;
    ...
    const rows = [
      ...
      _row('cables', { value: links.length, total: null,
        ...
        extra: Object.assign({
          ...
          // Cavi dichiarati su porta in `shutdown`: il numero viene DIRITTO dal report
          // della Verifica, che è dove il fatto nasce. Prima passava per `l.portShut`
          // scritto sui link, cioè uno stato intermedio che vive solo in memoria: al
          // primo ricarico spariva e la tessera taceva. Meno anelli, meno cose da
          // perdere — ed è la stessa fonte da cui la Dashboard legge tutti gli altri
          // conteggi del Drift.
          // La chiave compare SOLO se c'è almeno un cavo così: `extra` è una forma che
          // i lettori confrontano, e aggiungerci uno zero perenne cambierebbe il
          // contratto per raccontare il nulla.
        }, cableProofCounts ? { proofCounts: cableProofCounts, proofScore: cableTruth } : {},
           _num(_obj(_obj(m.driftLive).counts).shutCable) > 0
             ? { shutCable: _num(_obj(_obj(m.driftLive).counts).shutCable) } : {},
           // Capi di cavo su una porta che sull'apparato NON esiste (tipicamente:
           // qualcuno ha abbassato il conteggio porte e i cavi sono rimasti). Sono
           // nel documento ma nessuno li disegna: invisibili, e quindi non
           // correggibili. Il conteggio lo fa il glue con lib/port-inventory.js —
           // qui si COMPONE, non si calcola. Stessa disciplina di `shutCable`: la
           // chiave compare SOLO se il fatto c'è, perché uno zero perenne cambia la
           // forma che i lettori confrontano per raccontare il nulla.
           _num(m.orphanCables) > 0 ? { orphan: _num(m.orphanCables) } : {}),
        // Il click elenca TUTTI i cavi, divisi in schede per COME LO SAPPIAMO. Prima
        // mostrava i soli dedotti: il numero grande diceva 17 e il dettaglio si apriva
        // su 15, due popolazioni diverse sotto la stessa riga.
        // Le parole sono quelle della notazione unica e le da' `_cableGrade`: fino a
        // cinque schede — dichiarato · misurato · dedotto · contraddetto (la rete
        // smentisce il documento) · non risulta (il fantasma: un dedotto che ha perso
        // l'evidenza). Le ultime due esistono solo DOPO una Verifica, quindi senza
        // Verifica le schede sono tre e nessuna promette cio' che nessuno ha guardato.
        // ⚠️ La pastiglia per-voce qui NON si mette piu': la scheda dice gia' quella
        //    parola, e ripeterla accanto sarebbe lo stesso difetto — due volte lo
        //    stesso alfabeto nella stessa riga — che la notazione unica e' venuta a
        //    chiudere. I due capi li risolve a nome il renderer.
        items: links.map((l) => {
          const it = { id: _nodeIdOf(l.src, nodeIds), peer: _nodeIdOf(l.dst, nodeIds) };
          // Il link-id viaggia con la voce: al click il renderer evidenzia il
          // PERCORSO fisico del cavo sul floor (store.highPath), non un solo capo.
          if (l && l.id != null) it.lid = _str(l.id);
          const st = cableStateById && l && l.id != null ? cableStateById[_str(l.id)] : null;
          const g = _cableGrade(l, st);
          if (g) it.group = g;
          return it;
        }).sort((a, b) => _gradeRank(a.group) - _gradeRank(b.group)),
      }),
      _row('subnets', {
        ...
        items: subGroups.map((g) => ({ id: g.cidr, meta: g.used, tag: g.prov === 'declared' ? 'declaredNet' : 'undeclared', net: g.cidr })),
      }),
      ...
      _row('gateways', {
        value: declaredSubnets - gwMissingArr.length, total: declaredSubnets,
        prov: declaredSubnets ? 'declared' : 'none',
        items: gwOrdered.map((p) => Object.assign({ id: p.cidr, meta: p.gateway || '—', net: p.cidr }, p.gateway ? {} : { tag: 'noGateway' })),
      }),
      _row('vlanNames', {
  ...
  function _truth(m) {
    ...
    // ⚠️ «Ha risposto» e' snmpStatus === 'ok' — la definizione DICHIARATA in testa a
    // lib/subbar-stats.js e usata anche da src/app-drift.js. Qui si contava
    // «polled meno gli err», cioe' «non ha fallito», che e' un'ALTRA domanda: un
    // apparato configurato e mai interrogato non ha uno stato, quindi passava per
    // uno che risponde. MISURATO prima di toccarlo, sullo stesso documento: due ok
    // e uno mai sondato davano 2/3 nella sotto-header e 3/3 nella Panoramica — due
    // superfici della stessa app che si contraddicono, e la verde era quella falsa.
    const snmpAnswered = polled.filter((n) => _str(n.snmpStatus) === 'ok');
    const snmpErr = polled.filter((n) => _str(n.snmpStatus) === 'err');
    // Il TERZO stato, che non aveva un nome: configurato, ne' ok ne' err. Non e' un
    // fallimento (nessuno l'ha interrogato) e non e' una risposta: e' un'attesa, e
    // la si dice. Tenerlo dentro al numero dei buoni era un ripiego travestito da
    // misura — a valle nessuno lo distingueva da un apparato che ha davvero parlato.
    const snmpSilent = polled.filter((n) => {
      const s = _str(n.snmpStatus);
      return s !== 'ok' && s !== 'err';
    });
    ...
    const neverAnswered = [];
    const neighborItems = [];
    // Un vicino LLDP/CDP che coincide con un CAVO del progetto e' cliccabile come
    // il cavo: il click ne evidenzia il percorso (riusa `lid` -> overview-goto-link
    // nel renderer). Mappa coppia-di-nodi (non ordinata) -> link id, sui link con
    // entrambi gli estremi nel progetto; il primo vince (per un LAG basta uno).
    const _linkIdByPair = new Map();
    for (const l of _arr(m.links)) {
      if (!l || l.id == null) continue;
      const la = _nodeIdOf(l.src, nodeIds), lb = _nodeIdOf(l.dst, nodeIds);
      if (!la || !lb) continue;
      const pk = la < lb ? la + '\u0000' + lb : lb + '\u0000' + la;
      if (!_linkIdByPair.has(pk)) _linkIdByPair.set(pk, _str(l.id));
    }
    for (const n of polled) {
      ...
      for (const nb of adj) {
        ...
        if (peer) {
          const it = { id: n.id, peer, meta: port };
          const a = _str(n.id), b = _str(peer);
          const pk = a < b ? a + '\u0000' + b : b + '\u0000' + a;
          const lid = _linkIdByPair.get(pk);
          if (lid) it.lid = lid;
          neighborItems.push(it);
        }
        else neighborItems.push({ id: n.id, meta: remote ? (port ? remote + ' · ' + port : remote) : port });
      }
    }

    // LAG — due fatti diversi, e per anni sono usciti dalla stessa stringa.
    // DA DOVE ARRIVA lo dice la chiave: `snmp-lag-…` misurato, `lldp-lag-…` dedotto
    // dall'auto-link, qualunque altra forma l'ha scritta una PERSONA. Va bene: la
    // chiave la scrive chi crea il gruppo, ed e' una dichiarazione di origine.
    // ⭐ I CAPI no. Quelli li dicono le PORTE MEMBRO — `ports[pid].lagGroup`, che e'
    //    l'unico posto in cui sono un fatto — e leggerli dalla chiave funzionava solo
    //    finche' la chiave conteneva un id di nodo. Misurato sul progetto del banco il
    //    2026-09-24: SEI righe su sedici uscivano con il nome del capo VUOTO, tutte
    //    quelle di gruppi creati a mano. Un id non e' un dato: e' una targa.
    // ⚠️ Un gruppo senza porte membro non ha capi perche' non e' un LAG: e' un residuo
    //    (un giro vecchio, o una cancellazione a meta'). Esce detto, non indovinato.
    const lagGroups = _obj(m.lagGroups);
    const lagMembers = _obj(m.lagMembers);
    const lagKeys = Object.keys(lagGroups);
    const _lagOrigin = (k) => (k.indexOf('snmp-') === 0 ? 'measured'
      : (k.indexOf('lldp-') === 0 ? 'derived' : 'declared'));
    const lagMeasured = lagKeys.filter((k) => _lagOrigin(k) === 'measured').length;
    const lagDerived  = lagKeys.filter((k) => _lagOrigin(k) === 'derived').length;
    const lagItems = lagKeys.map((k) => {
      const capi = _arr(lagMembers[k]).map(_str).filter(Boolean);
      const nm = lagGroups[k];
      const it = { id: capi[0] || null, peer: capi[1] || null, group: _lagOrigin(k),
                   meta: _str(typeof nm === 'string' ? nm : (nm && nm.name)) };
      // La scheda dice gia' l'origine: ripeterla in pastiglia sarebbe la stessa
      // parola due volte nella stessa riga. Il tag resta per cio' che la scheda
      // NON dice — un gruppo rimasto senza porte.
      if (!capi.length) it.tag = 'noMembers';
      return it;
    }).sort((x, y) => _gradeRank(x.group) - _gradeRank(y.group));

    // Lo stato operativo DICHIARATO messo davanti alla misura (lib/device-status.js).
    // Due numeri, e il secondo è quello che tiene onesto il primo:
    //   • le CONTRADDIZIONI — dichiarato fuori servizio, e invece risponde. È il
    //     numero grande della riga: documentazione scaduta, che nient'altro vedeva.
    //   • le assenze SPIEGATE — quante volte la dichiarazione ha fatto NON accendere
    //     un rosso sul pavimento. Un silenziamento che non si conta è un
    //     silenziamento invisibile, ed era l'obiezione più seria a questo campo.
    // Il perimetro sono gli apparati DOCUMENTATI, non i soli interrogabili: uno
    // switch a magazzino non ha un IP, e lasciarlo fuori dal censimento farebbe
    // sparire proprio i casi per cui il campo esiste.
    const documented = nodes.filter((n) => !_obj(types[n && n.type]).isStructural);
    const statusCensus = (dsApi && typeof dsApi.countByStatus === 'function')
      ? dsApi.countByStatus(documented) : null;
    const statusConflictItems = [];
    let absentExpected = 0;
    if (dsApi && typeof dsApi.statusFinding === 'function') {
      for (const n of documented) {
        const f = dsApi.statusFinding(n && n.status, _obj(n && n.proof).status);
        if (!f) continue;
        if (f.code === 'status.aliveNotInService') statusConflictItems.push(Object.assign(item(n), { tag: f.status }));
        else if (f.code === 'status.absentAsDeclared') absentExpected++;
      }
    }
    const statusDeclared = statusCensus ? (statusCensus.total - statusCensus.undeclared) : 0;
    // Solo gli stati che qualcuno dichiara davvero: una riga «pianificati: 0» è
    // rumore, e un elenco di sette zeri nasconde l'unico numero che conta.
    const statusBreakdown = {};
    if (statusCensus) {
      for (const s of _arr(dsApi && dsApi.STATUSES)) if (_num(statusCensus[s]) > 0) statusBreakdown[s] = statusCensus[s];
    }
    ...
    const lvActionable = _num(lvc.stateDrift) + _num(lvc.macOrphan) + _num(lvc.undocumented)
      + _num(lvc.ghostCable) + _num(lvc.shutCable) + _num(lvc.ipChanged) + _num(lvc.identityDrift);
    ...
    const _driftCats = [
      ...
      // `shutCable` NON ha una riga sua: il suo numero vive dentro il tile «Cavi»,
      // accanto a fantasma/dedotti/dichiarati. È pur sempre un modo in cui un cavo può
      // stare, non un'anomalia di un'altra specie — e una riga in più a fianco di
      // «Cavi» avrebbe raccontato lo stesso oggetto in due posti.
    ...
    // La sovrapposizione è fra PREFISSI, e la VLAN di ciascuno è facoltativa: una
    // rete senza VLAN si scrive '—', mai «VLAN 0» (`+null === 0`). Se nessuna delle
    // due ha una VLAN non c'è niente da nominare e il meta resta il solo marcatore.
    for (const o of overlaps) {
      const a = _obj(o.a), b = _obj(o.b);
      const vid = (x) => (x.vlan == null ? '—' : String(_num(x.vlan)));
      const vlanPart = (a.vlan == null && b.vlan == null) ? '' : 'VLAN ' + vid(a) + '/' + vid(b);
      conflictItems.push({
        id: _str(a.cidr) + ' ⇄ ' + _str(b.cidr),
        meta: vlanPart + (o.identical ? (vlanPart ? ' ·=' : '=') : ''),
      });
    }
    ...
    const rows = [
      ...
      // Nessun bersaglio con uno stato = la misura non e' mai stata fatta. Il
      // valore resta NULL, non 0: un 0 a valle e' indistinguibile da «nessuno ha
      // risposto», che e' un esito, mentre qui non c'e' stato nemmeno il tentativo.
      _row('verifiable', {
        value: (polled.length && (snmpAnswered.length + snmpErr.length)) ? snmpAnswered.length : null,
        total: polled.length || null,
        // Nessun bersaglio ha uno stato = MAI interrogato: assenza, non zero. E'
        // lo stesso gating della riga sorella lastSync — «0 su 15 rispondono»
        // sarebbe una misura mai fatta scritta come un risultato pessimo, cosi'
        // come «15 su 15» era la stessa misura mai fatta scritta come un trionfo.
        prov: (polled.length && (snmpAnswered.length + snmpErr.length)) ? 'derived' : 'none',
        tone: (snmpErr.length || snmpSilent.length) ? 'alert' : 'normal',
        extra: { unverifiable: notVerifiable.length, errors: snmpErr.length, silent: snmpSilent.length },
        items: []
          .concat(snmpErr.map((n) => Object.assign(item(n), { tag: 'snmpErr' })))
          .concat(snmpSilent.map((n) => Object.assign(item(n), { tag: 'noReading' })))
      ...
      ...driftRows,
      // Stato dichiarato ↔ misura. `prov:'none'` finché NESSUNO dichiara uno stato:
      // la riga esiste e dichiara la lacuna, invece di stampare uno zero che si
      // legge come «nessun problema». Zero è un'affermazione, tratteggiato è una
      // domanda — ed è la stessa regola del limite di visibilità (P1).
      _row('statusDeclared', {
        value: statusDeclared ? statusConflictItems.length : null,
        total: null,
        prov: statusDeclared ? 'measured' : 'none',
        tone: statusConflictItems.length > 0 ? 'alert' : (statusDeclared ? 'normal' : 'muted'),
        extra: {
          declared: statusDeclared,
          undeclared: statusCensus ? statusCensus.undeclared : 0,
          absentExpected,
          byStatus: statusBreakdown,
        },
        items: statusConflictItems,
      }),
      _row('neighbors', {
      ...
      _row('lags', {
        ...
        // Tre secchi, non due: un gruppo creato a mano non e' ne' misurato ne'
        // dedotto — e' DICHIARATO, e finiva contato fra i dedotti.
        extra: { measured: lagMeasured, derived: lagDerived,
                 declared: lagKeys.length - lagMeasured - lagDerived },
        items: lagItems,
      }),
        ...
        ? _row('verify', {
            ...
            extra: {
              ...
              counts: Object.assign({}, lvc),
                ...
                // ⚠️ `unread` e non `none`: qui non manca una tua dichiarazione — manca una
        // VERIFICA. L'etichetta «non dichiarato» era falsa su questa riga, ed è il
        // motivo per cui le due assenze sono state separate.
        : _row('verify', { value: null, total: null, prov: 'unread', tone: 'muted' }),
    ];
    ...
    return {
      rows,
      health,
      // La contraddizione di stato guida il titolo quando c'è: un apparato che
      // risponde mentre il documento lo dà fuori servizio è la bugia più netta che
      // questa lente sappia trovare, ed è rara — quando compare, è quella la notizia.
      headline: _headline(rows, [
        ['statusDeclared', (r) => r.value > 0],
        ['suspectPorts', (r) => r.value > 0],
        // r.value puo' essere null (mai interrogato): senza il controllo esplicito
        // null < total e' vero, e un'assenza si prenderebbe il titolo come se fosse
        // un guasto misurato.
        ['verifiable', (r) => r.total > 0 && r.value != null && r.value < r.total],
        ['lastSync', () => true],
      ]),
    };
  }
  ...
  function _recovery(m) {
    ...
    const _identity = (n) => {
      ...
      // Il MISMATCH è un'accusa («apparato sostituito») e vuole una misura
      // RICONFERMATA dall'ultimo poll: un'inventory rimasta «ultimo noto» perché
      // la walk ENTITY-MIB si è troncata non basta per dire che l'hardware è
      // cambiato (lib/identity-reconcile.js, stessa regola della Verifica).
      // Il modello e il firmware invece restano: per sapere cosa ricomprare
      // l'ultima misura nota vale, l'accusa no.
      const confirmed = idApi && typeof idApi.isConfirmedMeasure === 'function'
        ? idApi.isConfirmedMeasure(inv) : (!!inv && inv.stale !== true);
      return {
        model, fw: _str(n.firmwareVer) || _str(inv.firmwareVer),
        mismatch: !!(confirmed && declS && measS && declS !== measS),
  ...
  function _security(m) {
    ...
    const rows = [
      ...
      _row('secWifiVlan', {
        ...
        items: wifiIssues.map((is) => (is.kind === 'client-ip-vlan-mismatch'
          // Client su un SSID di VLAN X ma con IP nella subnet di VLAN Y (dato incoerente).
          ? { id: _str(is.client) + ' · SSID VLAN ' + _str(is.vlan) + ' ≠ IP VLAN ' + _str(is.ipVlan), meta: _str(is.ap) || null }
          : { id: 'SSID «' + _str(is.ssid) + '» · VLAN ' + _str(is.vlan), meta: _str(is.ap) || null }   // SSID·VLAN + meta AP
        )),
      }),
    ];
  ...
  //   'elsewhere' il dato e' documentato o MISURATO altrove (una card device, il
  //               pannello «Sedi e collegamenti»…) ma non entra in un
  //               verdetto qui. Quando una dimensione verra' modellata, la si TOGLIE
  //               da questa lista: e' un TODO vivo, non una scritta fissa.
  const _BLIND_SPOTS = [
    // ⚠️ 'elsewhere', non 'unmodeled': dal 30/08 le linee WAN, la banda
    // contrattuale e i collegamenti fra sedi SONO modellati — ma nel pannello
    // «Sedi e collegamenti», che e' UNO per INSTALLAZIONE, mentre questo motore
    // legge UN progetto. Toglierlo dalla lista direbbe che la Panoramica li
    // guarda (falso); lasciarlo 'unmodeled' diceva che il prodotto non li
    // modella (falso anche quello). Il terzo tier esisteva gia' apposta.
    { key: 'wan',       tier: 'elsewhere' },   // linee WAN / banda contrattuale / collegamenti fra sedi
    ...
    // Controllo d'accesso alla porta: 802.1X, MAB, port-security. La lente
    // «Sicurezza» giudica VLAN guest, wireless e management — chi sia AUTORIZZATO a
    // parlare su una presa non lo legge nessuno, e in quella lente il silenzio si
    // legge come assoluzione. Gli OID esistono (IEEE8021-PAE-MIB) ma leggerne meta'
    // sarebbe peggio di non leggerli: «authorized» da solo non distingue un
    // supplicant autenticato da una porta forzata aperta che non controlla nessuno.
    // Finche' non si legge intero, si dichiara.
    { key: 'nac',       tier: 'unmodeled' },
  ];
   ...
   */
  // ── IL VERDETTO, in un posto solo ──────────────────────────────────────────
  // Ogni sezione aveva il suo: la Sintesi una frase da i18n, le tre lenti un
  // numero grande costruito a mano DENTRO il renderer. Due implementazioni dello
  // stesso concetto — ed e' esattamente il difetto che questa Panoramica esiste
  // per non avere. L'idea migliore delle due, «non lo so ha il suo colore e non
  // si traveste da verde», viveva in UNA sezione su sei.
  //
  // ⚠️ `none` NON vuol dire «zero problemi»: vuol dire **non ho letto niente**.
  // La regola di base e' DERIVATA e vale per qualunque sezione, anche una scritta
  // domani: se ogni riga ha provenienza 'none', non c'e' un dato su cui giudicare.
  // Le eccezioni sotto esistono perche' quella sezione sa qualcosa di piu' — e
  // sono POCHE e dichiarate, non un elenco parallelo da mantenere.
  function _senzaDati(key, sec) {
    const rows = _arr(sec && sec.rows);
    // Sezioni che portano un totale esplicito: il conteggio e' piu' preciso delle
    // righe, perche' distingue «nessun apparato da guardare» da «righe vuote».
    if (key === 'recovery') return _num(sec.total) === 0;
    if (key === 'security') return _num(sec.snmpTotal) === 0;
    if (key === 'health') return _num(sec.measured) === 0;
    // ⚠️ «Mai sincronizzato» era ROSSO. Ma il rosso e' un guasto, e non aver mai
    // letto non e' un guasto: e' non sapere. Il rosso qui prometteva una diagnosi
    // che nessuno aveva fatto — la stessa famiglia del ripiego che afferma.
    if (key === 'truth') {
      const ls = rows.find((r) => r && r.key === 'lastSync');
      return !ls || ls.prov === 'none';
    }
    return rows.length > 0 && rows.every((r) => !r || r.prov === 'none');
  }

  // I due numeri del verdetto, DOVE la risposta e' un numero: «19 di 31» risponde
  // a «se cade stanotte lo rimetti in piedi?» molto meglio di «margine risicato».
  // Le altre tre sezioni non hanno una coppia sola che le riassuma: li' il
  // verdetto resta la frase, e va bene cosi' — inventare un numero per simmetria
  // sarebbe la simmetria che mente.
  function _cifreVerdetto(key, sec) {
    if (key === 'recovery') return { value: _num(sec.recoverable), total: _num(sec.total) };
    if (key === 'security') return { value: _num(sec.secured), total: _num(sec.snmpTotal) };
    if (key === 'health') return { value: _num(sec.healthy), total: _num(sec.measured) };
    return null;
  }

  // Attacca a una sezione il suo verdetto. Il glue ci mette le PAROLE (i18n) e
  // basta: livello, numeri e conteggio arrivano da qui, come per `health`.
  function _conVerdetto(key, sec) {
    if (!sec || typeof sec !== 'object') return sec;
    const h = _obj(sec.health);
    const vuota = _senzaDati(key, sec);
    const level = vuota ? 'none' : (h.level || 'ok');
    const cifre = vuota ? null : _cifreVerdetto(key, sec);
    sec.verdict = {
      level,
      issues: vuota ? 0 : _num(h.issues),
      value: cifre ? cifre.value : null,
      total: cifre ? cifre.total : null,
      stale: !vuota && !!h.stale,
      staleDays: _num(h.staleDays),
    };
    return sec;
  }
  ...
  function buildOverview(model) {
    ...
    // ⚠️ Si itera sulle sezioni EFFETTIVE, non su un elenco scritto a mano: una
    // sezione aggiunta domani prende il verdetto da sola invece di restarne fuori
    // in silenzio, che e' il modo in cui un elenco duplicato si guasta.
    for (const k of Object.keys(out)) if (k !== 'blindSpots') _conVerdetto(k, out[k]);
    return out;
  }
  ...
  return { buildOverview, _isAddressable, _hasRealName, _rackFill, _BLIND_SPOTS,
    vlanNameKey, BACKUP_FRESH_DAYS, LIFECYCLE_SOON_DAYS };
});
