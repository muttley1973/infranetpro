  ...
  function _complete(m) {
    ...
    const rows = [
      ...
      _row('addr', { value: addressable.length - noAddr.length, total: addressable.length,
        prov: 'declared', items: noAddr.map(item) }),
      _row('name', { value: addressable.length - noName.length, total: addressable.length,
        prov: 'declared', items: noName.map(item) }),
      // Un MAC non si scrive: si osserva. Arriva da ARP/discovery (node.mac) o da
      // ifPhysAddress sulle porte — nessuno digita 48 bit a memoria. Chiamarlo
      // «dichiarato» era il default che parlava al posto del dato.
      _row('mac', {
        value: addressable.length - noMac.length, total: addressable.length,
        prov: (addressable.length - noMac.length) ? 'measured' : 'none',
  ...
  function _margin(m) {
    ...
    const rows = [
      ...
      _row('freePorts', { value: freeHonest, total: _num(totals.ports),
        prov: _num(totals.ports) ? 'derived' : 'none', extra: { raw: free, suspect },
