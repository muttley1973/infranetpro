...
(function (root, factory) {
  ...
  function _recovery(m) {
    ...
    // 'mismatch' serial dichiarato ≠ misurato (apparato sostituito) · 'known' almeno
    // un serial c'è e concordano · 'unknown' nessun serial (non sai cosa rimpiazzare).
    const _identityState = (n) => {
      const decl = _str(n.serialNumber).trim().toLowerCase();
      const meas = _str(_obj(_obj(n.integration).inventory).serialNumber).trim().toLowerCase();
      if (decl && meas && decl !== meas) return 'mismatch';
      return (decl || meas) ? 'known' : 'unknown';
    };
    ...
    let okBackup = 0, okIdentity = 0, okLocation = 0, staleBackup = 0, idMismatch = 0;
    let recoverable = 0, withObs = 0, haProtected = 0;

    for (const n of pop) {
      const b = _backupState(n), id = _identityState(n), lo = _locState(n);
      ...
      if (id === 'known') okIdentity++;
      else { if (id === 'mismatch') idMismatch++; noIdentity.push(Object.assign(item(n), id === 'mismatch' ? { tag: 'mismatch' } : {})); }
      if (lo === 'known') okLocation++; else noLocation.push(item(n));
      if (_haProtected(n)) { haProtected++; haItems.push(item(n)); }
      if (b === 'fresh' && id === 'known' && lo === 'known') recoverable++;
    ...
    const rows = [
      ...
      _row('drIdentity', { value: okIdentity, total, prov: total ? 'measured' : 'none', extra: { mismatch: idMismatch }, items: noIdentity }),
  ...
  function buildOverview(model) {
    const m = _obj(model);
    return { complete: _complete(m), truth: _truth(m), margin: _margin(m), recovery: _recovery(m) };
  }
