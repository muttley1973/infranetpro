  ...
  function _complete(m) {
    ...
    for (const v of vlansInUse) {
      ...
      if (netName && ((decl && !declMeasured) || disagree)) {
        ...
        vlanConflictItems.push({ id: 'VLAN ' + v, meta: (decl && !declMeasured ? decl + ' ↔ ' : '') + parts.join(' ↔ ') });
      } else if (!decl && netName) {
        // lacuna colmabile: nessun nome dichiarato ma la rete ne conosce uno (concorde).
        vlanFromSnmp++;
        vlanFillItems.push({ id: 'VLAN ' + v, meta: netName, tag: 'measured' });
      }
    }
    ...
    const rows = [
      ...
      _row('gateways', {
        ...
        items: gwOrdered.map((p) => Object.assign({ id: p.cidr, meta: p.gateway || '—' }, p.gateway ? {} : { tag: 'noGateway' })),
      }),
      _row('vlanNames', {
