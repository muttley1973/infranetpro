  ...
  function _complete(m) {
    ...
    const rows = [
      ...
      _row('subnets', {
        ...
        items: subGroups.map((g) => ({ id: g.cidr, meta: g.used, tag: g.prov === 'declared' ? 'declared' : 'undeclared' })),
      }),
      _row('vlanNames', {
  ...
  function _margin(m) {
    ...
    const rows = [
      ...
      _row('ipFree', {
        ...
        items: groups.map((g) => ({ id: g.cidr, meta: Math.max(0, g.usable - g.used), of: g.usable, tag: g.prov === 'declared' ? 'declared' : 'undeclared' })),
      }),
    ];
    return {
      rows,
