...
(function (root, factory) {
  ...
  function _complete(m) {
    ...
    const rows = [
      ...
      // value = subnet dichiarate CON gateway; il click elenca quelle SENZA.
      _row('gateways', {
        value: declaredSubnets - gwMissing.length, total: declaredSubnets,
        prov: declaredSubnets ? 'declared' : 'none',
        items: gwMissing.map((k) => ({ id: _str(_obj(ipamV[k]).subnet), meta: 'VLAN ' + k })),
      }),
      _row('vlanNames', {
  ...
  function _margin(m) {
    ...
    // A che VELOCITA' puoi crescere: «166 porte libere» non risponde alla domanda
    // della colonna finche' non sai se sono 1G o 10G, ed era la meta' del dato che
    // il motore raccoglieva senza mai aggregarla. Sta QUI dentro e non in un
    // riquadro suo: e' un altro modo di guardare le stesse porte libere, quindi
    // e' una terza scheda del dettaglio accanto a «in rack» / «fuori rack».
    // Le classi escono dalla piu' veloce — l'ordine in cui uno le guarda quando
    // deve decidere dove attaccare la prossima cosa. Le porte senza velocita' nota
    // restano una voce A PARTE: non sono porte lente, sono porte di cui nessuno
    // ha scritto la velocita'. `value` = Mbps grezzi, la parola la mette il
    // renderer — qui dentro non entrano stringhe d'interfaccia.
    const bySpeed = _obj(totals.freeBySpeed);
    const bySpeedSorted = Object.keys(bySpeed)
      .map((k) => ({ mbps: _num(k), n: _num(bySpeed[k]) }))
      .filter((x) => x.mbps > 0 && x.n > 0)
      .sort((a, b) => b.mbps - a.mbps);
    const noSpeed = _num(totals.freeNoSpeed);
    const speedItems = bySpeedSorted.map((x) => ({ id: x.n, metric: 'speedClass', value: x.mbps, group: 'speed' }));
    if (noSpeed > 0) speedItems.push({ id: noSpeed, metric: 'speedNone', value: null, group: 'speed' });
    ...
    for (const it of speedItems) freePortItems.push(it);
    ...
    const rows = [
      ...
      _row('freePorts', { value: freeHonest, total: _num(totals.ports),
        prov: _num(totals.ports) ? 'derived' : 'none',
        extra: { raw: free, suspect, bySpeed: bySpeedSorted, noSpeed },
  ...
  // La community NON esce MAI dal motore: qui si CONTA, mai la si espone (anti-leak).
  ...
  function _security(m) {
    ...
    for (const n of pop) {
      ...
      // Community indovinabile: default noti o vuota. Solo il CONTEGGIO+nome device,
      // MAI il valore della community (anti-leak). Nessun tag: la riga già segnala.
      const comm = _str(integ.community).trim().toLowerCase();
      if (_DEFAULT_COMMUNITIES.indexOf(comm) !== -1) defaultComm.push(item(n));
    }
    ...
    const rows = [
      ...
      // Segmentazione della gestione: quante VLAN sono marcate «di gestione».
      // Advisory (non gate): averne una dedicata è buona pratica, non un obbligo.
      _row('secMgmtVlan', { value: mgmtVlans.length, total: null, prov: mgmtVlans.length ? 'declared' : 'none' }),
    ];
    ...
    return { rows, health: { level, issues: exposed + defaultComm.length }, secured: v3, snmpTotal, managed: pop.length };
  }
  ...
  function _health(m) {
    ...
    const targets = _arr(m.nodes).filter(_hasSnmp).length;
