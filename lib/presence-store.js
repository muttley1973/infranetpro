...
(function (root, factory) {
  ...
  // Fonde due mappe di presenza: vince la misura più FRESCA, e a parità vince la
  // BASE (il sidecar è l'ultimo che ha scritto) — stessa regola di mergePresence.
  //
  // ⚠️ Serve alla MIGRAZIONE. Fino alla 2.8.0 la presenza finiva anche dentro il
  // `<id>.json`: quando quello stato torna indietro per essere salvato, la sua
  // presenza CONFLUISCE qui invece di essere buttata via. Così un progetto vecchio
  // non perde i suoi rossi nel momento in cui il documento smette di ospitarli.
  function foldPresence(base, incoming) {
    const a = sanitizePresence(base).nodes;
    const b = sanitizePresence(incoming).nodes;
    const out = Object.assign({}, a);
    for (const id of Object.keys(b)) {
      if (!out[id] || _ms(b[id].lastCheckedAt) > _ms(out[id].lastCheckedAt)) out[id] = b[id];
    }
    return sanitizePresence({ nodes: out });   // ri-sanifica: applica il tetto all'unione
  }

  // Toglie la presenza dallo stato che sta per essere scritto sul disco: nel
  // DOCUMENTO non ci va. Ritorna quanti nodi ne avevano una (per i test e la doc).
  function stripPresence(state) {
    const nodes = (state && Array.isArray(state.nodes)) ? state.nodes : [];
    let n = 0;
    for (const node of nodes) {
      if (!node || typeof node !== 'object') continue;
      if (Object.prototype.hasOwnProperty.call(node, 'proof')) { delete node.proof; n++; }
    }
    return n;
  }
  ...
  return { sanitizeProof, sanitizePresence, mergePresence, foldPresence, stripPresence, collectPresence, PRESENCE_STATUSES: STATUSES };
});
