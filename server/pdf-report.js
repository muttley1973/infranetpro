...
function _addRecoveryPages(doc, recovery, projName, date, lang = 'it', now = Date.now()) {
  ...
  let recoverable = 0, noBackup = 0, noLoc = 0;
  const rows = list.map((d, i) => {
    ...
    const ident = !!(_s(d.serial) || _s(d.model));
    const loc = !!_s(d.rack);
    ...
    return [
      ...
      _s(d.serial) || '-', _s(d.firmware) || '-', _s(d.rack) || '-',
    ];
  });
  ...
  const cols = [
    { label: _rt(L, 'col.num'),        w: 18  },
    { label: _rt(L, 'col.device'),     w: 84, wrap: true },
    { label: _rt(L, 'col.backupRef'),  w: 130, wrap: true },
    { label: _rt(L, 'col.method'),     w: 48  },
    { label: _rt(L, 'col.backupDate'), w: 82, wrap: true },
    { label: _rt(L, 'col.serial'),     w: 66, shrink: true },
    { label: 'Firmware',               w: 71, wrap: true },
    { label: _rt(L, 'col.rack'),       w: 40, wrap: true },
  ]; // 18+84+130+48+82+66+71+40 = 539
  _rTable(doc, cols, rows, y, T, projName, date);
}

module.exports = { _loadPdfDeps, _addReportPages, _addCoverPage, _addNotesPages, _addChangelogPages, _addSparePages, _addAssetRegisterPages, _addRecoveryPages, _assetDeviceLabel, _fmtRevised, _rt, _fit, _wrapFit };
