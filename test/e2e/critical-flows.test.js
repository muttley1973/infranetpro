  ...
  try {
    ...
    await t.test('Sync: materializza gateway annunciato; intermediario inferito → segmento L2 (ruolo suggerito)', async () => {
      ...
      await page.evaluate(() => {
        ...
        state.ports['e2eGs-23'] = { ifName: 'Gi23' };
        state.ports['e2eGs-24'] = { ifName: 'Gi24' };
        if (typeof _invalidateIdx === 'function') _invalidateIdx();
      });
      const POLL = [{
        nodeId: 'e2eGs',
        fdbTable: { 'ec:71:db:49:58:05': 'Gi24', 'ec:71:db:4f:bd:51': 'Gi24' }, // 2 webcam su Gi24
        neighbors: [{ localPort: 'Gi23', remoteDevice: 'd4:1a:d1:82:11:20', remoteMac: 'd4:1a:d1:82:11:20', remoteIP: '', protocol: 'LLDP' }],
        suggestedSharedSegments: [],
      }];
      const first = await page.evaluate((poll) => {
